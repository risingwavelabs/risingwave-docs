...
You can use the [CREATE INDEX](/sql/commands/sql-create-index) command to construct an index on a table or a materialized view. Indexes can be standard for accelerating lookups and sorts or vector indexes for similarity search in machine learning, AI applications, etc. The syntax is as follows:
...
RisingWave supports creating indexes on expressions. Indexes on expressions are normally used to improve the performance of queries for frequently used expressions.

### Standard indexes on expressions

To create an index on an expression, use the syntax:
...
If you often perform queries like this:
...
**Example 2: JSONB field extraction**
...
### Vector index on function expressions

<Note>
Creating vector index on function expression is added in v2.6.0 and is currently in **[technical preview](/changelog/product-lifecycle#product-release-lifecycle)** stage.
</Note>

RisingWave supports creating vector indexes on the results of function expressions.

```sql Syntax
CREATE INDEX index_name ON table_name USING flat (function_expression(column))
[INCLUDE (other_columns)]
WITH (distance_type = '<type>');
```

**Example: text embedding with a function**

Suppose you have a `text` column and a function `get_embedding(text)` that converts text into a vector. You can create a vector index directly on the output of this function:

```sql
-- Create a vector index on the result of get_embedding(text)
CREATE INDEX i ON items USING flat (get_embedding(text))
INCLUDE (text)
WITH (distance_type = 'l2');

-- Query the most similar text entries using the indexed embedding
SELECT id, text
FROM items
ORDER BY get_embedding(text) <-> '[0.1,0.2,0.3]'::vector(3)
LIMIT 5;
```
