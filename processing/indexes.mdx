...
If your queries only access certain columns, you can create an index that includes only those columns. The RisingWave optimizer will automatically select the appropriate index for your query. If a partial index does not cover all referenced columns, RisingWave may use an additional lookup join when that plan is valid. Otherwise, it falls back to scanning the base table or materialized view.
...
If RisingWave cannot convert a non-covering index access into a valid lookup join, it falls back to scanning the base table or materialized view instead of forcing the lookup plan.
