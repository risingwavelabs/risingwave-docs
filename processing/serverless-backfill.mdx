...
<Note>
Serverless backfilling is available starting from v2.8.0 and is disabled by default. Support for materialized views that use non-default resource groups is available starting from v3.1.0.
</Note>
...
3. Once all backfill fragments report completion, the MV transitions to the standard incremental streaming phase and is migrated to its steady-state resource group for normal streaming execution.
...
After backfill finishes, the materialized view resumes steady-state streaming in its configured target resource group. If the materialized view specifies a resource group at creation time, that resource group is used. Otherwise, the materialized view uses the database's resource group, or the default resource group if the database is not assigned to one.
