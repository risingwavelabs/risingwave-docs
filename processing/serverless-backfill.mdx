...
<Note>
Serverless backfilling is available starting from v2.8.0 and is disabled by default. Support for materialized views whose parent database uses a non-default resource group is delivered via [risingwave-extensions#335](https://github.com/risingwavelabs/risingwave-extensions/pull/335) and is available on RisingWave v2.7, v2.8, and v3.0.
</Note>
...
3. Once all backfill fragments report completion, the MV transitions to the standard incremental streaming phase and is migrated to its **steady-state resource group** — the resource group of its parent database — for normal streaming execution.
...
During backfill, RisingWave Cloud runs the job in a temporary backfiller resource group that is provisioned from the Backfiller SKU and replica settings in the Cloud console.

After backfill finishes, the materialized view resumes steady-state streaming in its parent database's resource group. If the database is not assigned to a resource group, the materialized view uses the default resource group. A materialized view does not have its own resource group setting — it always inherits from its parent database.
