...
After backfill finishes, the materialized view resumes steady-state streaming in its parent database's resource group. If the database is not assigned to a resource group, the materialized view uses the default resource group.
