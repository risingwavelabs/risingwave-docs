...
description: "Use serverless backfilling to run the backfill phase of materialized view, sink, and index creation on dedicated backfiller resource groups, reducing resource contention on your main streaming cluster."
---
...
When you create a materialized view, sink, or index on existing data, RisingWave must first process all historical records, a phase called **backfill**. By default, this runs on the same compute nodes as your regular streaming workloads, which can cause resource contention and increased latency for existing pipelines.
...
You must enable serverless backfilling in the RisingWave Cloud console (in the **Backfilling** section of the **Resources** page) before using it. If it isn't enabled, statements that request serverless backfilling return an error.
...
- [CREATE SINK](/sql/commands/sql-create-sink) — Full syntax and options for creating sinks.
- [CREATE INDEX](/sql/commands/sql-create-index) — Full syntax and options for creating indexes.
