...
RisingWave supports cross-database queries in both streaming mode and ad-hoc mode, allowing you to access and combine data from multiple databases within your instance for a more comprehensive view. To get started, 

- There must be at least one subscription on the upstream table or materialized view in the external database.

- Users must be a superuser or have `CONNECT` permissions for the upstream databases, and must also have the corresponding relation's query permissions.

Here are some implementation details of cross-database queries:

- **Cross-database binding**: Supported in batch operations, `CREATE MATERIALIZED VIEW`, and `CREATE SINK`.
- **Privilege checks**: The system enforces database CONNECT privilege checks for all cross-database queries.
- **Snapshot backfill**: An integrated executor handles initial data loading from external databases.
- **Safety measures**: The system prevents `DROP MATERIALIZED VIEW/SINK CASCADE` operations that involve cross-database dependencies to avoid unintended consequences.
- **Log store verification**: During DDL operations, it verifies that log stores exist for all cross-database upstreams to maintain data consistency.
