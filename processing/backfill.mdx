...
If you monitor the job through `rw_catalog.rw_ddl_progress`, the consumed row count for snapshot backfill also resumes from the persisted backfill state after recovery instead of restarting from zero.
