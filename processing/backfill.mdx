...
To reduce streaming stalls during snapshot backfill completion, RisingWave gates the merge step on the pending barrier backlog and drains catch-up barriers gradually when the job starts consuming the log store. You can tune the merge threshold with `snapshot_backfill_finish_max_lagged_barriers` and the catch-up barrier throttle with `snapshot_backfill_barrier_amplification_factor` in the `[streaming]` section of `risingwave.toml`. For details, see [Node-specific configurations](/deploy/node-specific-configurations#streaming-configurations).
