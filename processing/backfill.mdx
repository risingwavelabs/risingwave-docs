...
5. Waits until the pending barrier backlog is within the configured limit before merging into the main streaming graph
6. Transitions to incremental mode once the snapshot is fully processed and merged
...
You can control when snapshot backfill merges into the main streaming graph with `snapshot_backfill_finish_max_lagged_barriers`. The default is `100`.
