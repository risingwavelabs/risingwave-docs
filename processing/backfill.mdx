...
5. Waits until the job's pending barrier backlog drops below the snapshot-backfill merge limit before merging into the database graph and transitioning to incremental mode. This reduces streaming stalls when backfill finishes. The default limit is `100`, controlled by `snapshot_backfill_finish_max_lagged_barriers`.
