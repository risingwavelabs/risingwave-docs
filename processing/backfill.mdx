...
### Legacy no-shuffle backfill

New streaming jobs no longer use no-shuffle backfill. RisingWave now uses snapshot backfill when available and otherwise falls back to arrangement backfill.

Legacy persisted jobs that already completed no-shuffle backfill remain compatible, but if RisingWave recovers an unfinished legacy no-shuffle backfill, you must cancel and recreate the streaming job.
...
| `0` | Pauses the backfill, which stays at `0.00%` progress until the value is changed. |
| positive integer | Rows per second, per parallelism. |

For [locality backfill](/performance/best-practices#locality-backfill), the limit also applies while the `LocalityProvider` drains snapshot rows. While a limit is configured, `rw_catalog.rw_rate_limit` lists this part of the backfill with the node name `LOCALITY_PROVIDER`.
...
Setting the parameter only affects jobs created afterwards. To change an existing job, use `ALTER`: [materialized views](/sql/commands/sql-alter-materialized-view#set-backfill-rate-limit), [CDC tables](/sql/commands/sql-alter-table#set-backfill-rate-limit), [sinks](/sql/commands/sql-alter-sink#set-backfill-rate-limit), and [fragments](/sql/commands/sql-alter-fragment#set-backfill-rate-limit). To see the value in effect for existing jobs, query `rw_catalog.rw_rate_limit`. For related parameters, see [runtime parameters](/operate/view-configure-runtime-parameters).
