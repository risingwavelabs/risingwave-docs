...
The `description` column stores the text data. The embedding column is used to store vector embeddings generated from description. If you create a vector index directly on description using a function expression, you do not need to store the embedding separately, saving storage and reducing maintenance.
