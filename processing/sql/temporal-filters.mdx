...
<Note>
**Important**: RisingWave's optimizer cannot always push temporal filters down to the source.
To guarantee the temporal filter is applied early (reducing state size), wrap the source with
the filter in a subquery in the `FROM` clause, rather than placing the filter in the outer `WHERE` clause.
This pattern ensures the filter is evaluated before the join, not after.
