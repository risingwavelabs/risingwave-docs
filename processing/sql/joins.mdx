...
<Note>
**Performance Tip**: In streaming queries, the optimizer can apply more aggressive optimizations
when filters are part of the join condition itself (in the `ON` clause) rather than added as
separate filters afterward. Consider restructuring your query to include filters in `ON` when possible.
</Note>
