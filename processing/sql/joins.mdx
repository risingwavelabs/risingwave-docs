...
- **Watermark behavior**: When interval joins include inequality conditions, only the watermark from the side with larger values is propagated downstream. For instance, in `left.ts >= right.ts`, the left side is larger, so only the left-side watermark is emitted. This ensures correct state cleanup and proper behavior for downstream operators relying on watermarks (such as `EMIT ON WINDOW CLOSE`).
