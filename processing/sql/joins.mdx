...
- High join amplification means there's a large amount of rows produced from the join. While barrier latency is mitigated with this approach, we still need to scale the cluster, to provide more resources for processing the large amount of rows.
</Note>
