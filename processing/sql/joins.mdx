...
- The join condition must include the key columns that form the distribution key of the lookup table (right table expression). Use `DESCRIBE <table_name>` to view the table's key information. You can create an index on the lookup table to facilitate the temporal join if the required key columns are not part of the primary key.
