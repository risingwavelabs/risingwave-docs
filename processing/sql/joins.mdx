...
Materialized views that contain temporal joins support background DDL. If you run `SET BACKGROUND_DDL = true` before `CREATE MATERIALIZED VIEW`, the creation can continue and finish successfully after compute node or Meta recovery.
