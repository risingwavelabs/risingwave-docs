...
<Note>
When both input sources define watermarks on the columns used in the equality join condition, RisingWave propagates an **aligned watermark** — the minimum of the two input watermarks — through the window join. Both sides' internal join-state tables are cleaned up as the aligned watermark advances, keeping the join state bounded. The output watermark advances only when watermarks from both input sides have progressed past the same timestamp.
</Note>
...
State cleaning is asymmetric: only the side whose watermark column appears in a lower-bound inequality condition has its join-state table cleaned. For example, given the condition `l.ts >= r.ts - INTERVAL '5' SECOND`, the left-side state is cleaned as the watermark advances, while the right-side state is not.
</Note>
...
<Note>
Starting from v3.0.2, materialized views that contain process-time temporal joins support background creation. If you run `SET BACKGROUND_DDL = true` before `CREATE MATERIALIZED VIEW`, RisingWave continues the backfill in the background and can resume creation after recovery. For details, see [SET BACKGROUND_DDL](/sql/commands/sql-set-background-ddl).
</Note>
