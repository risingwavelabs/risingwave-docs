...
[ BROADCAST ] [ LEFT | INNER ] JOIN <table_expression> FOR SYSTEM_TIME AS OF PROCTIME() [AS <alias>]
ON <join_conditions>;
```
...
- You can add `BROADCAST` before `JOIN` or `LEFT JOIN` to preserve the left-side distribution and broadcast lookup-table changes to each temporal join actor. This can reduce actor hot spots when lookup keys are skewed.
- `BROADCAST` temporal joins only support equality-based `INNER` and `LEFT` joins.
...
```sql Syntax
<table_expression> [ BROADCAST ] [ LEFT | INNER ] JOIN <table_expression> FOR SYSTEM_TIME AS OF PROCTIME() ON <join_conditions>;
```
...
<Note>
`BROADCAST` remains a contextual word. If you have an existing alias named `broadcast` immediately before `JOIN`, `INNER JOIN`, or `LEFT JOIN`, write it as `AS broadcast` or `"broadcast"` to keep it as an identifier.
</Note>
