...
**Watermark propagation**: The interval join operator propagates the watermark from the side with larger values in the inequality condition. For example, in a condition like `left.ts >= right.ts`, the left side has larger values, so only the left-side watermark is emitted downstream. This ensures correct watermark semantics for state cleaning and downstream operators like `EMIT ON WINDOW CLOSE`.
