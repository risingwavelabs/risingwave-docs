...
This approach is most effective when downstream is the majority bottleneck. It introduces a trade-off: improved pipeline stability at the cost of slightly increased end-to-end latency due to the buffering step. Monitoring for potential data lag in the buffer is also recommended.