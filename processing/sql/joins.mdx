...
<Note>
In streaming queries, it’s usually more efficient to put filters in the JOIN ON clause rather than filtering after the join. Consider restructuring your query to include filters in `ON` when possible.
</Note>
...
<Note>
**Watermark propagation and state cleaning for equality joins**

When both tables in an equality join have a `WATERMARK` defined on the join key column(s), RisingWave automatically propagates watermarks through the join and cleans up stale join state based on the watermark. Only the leftmost join key column with watermarks on both sides is used for state cleaning.

This enables the `EMIT ON WINDOW CLOSE` pattern with regular joins, without requiring explicit window functions.

```sql Example
CREATE TABLE t_left (
    id INT,
    ts TIMESTAMPTZ,
    -- Watermark is defined on `ts`, the join key column
...
CREATE TABLE t_right (
    id INT,
    ts TIMESTAMPTZ,
    -- Same watermark column on both sides enables state cleaning
    WATERMARK FOR ts AS ts - INTERVAL '5' SECOND
) APPEND ONLY;

-- `ts` is the join key column and has watermarks on both sides,
-- so this join propagates watermarks and cleans up stale state
CREATE MATERIALIZED VIEW my_join AS
SELECT l.id, r.id AS r_id, l.ts
FROM t_left l
JOIN t_right r ON l.ts = r.ts;

-- EMIT ON WINDOW CLOSE works because watermarks are propagated through the join
CREATE MATERIALIZED VIEW my_join_eowc AS
SELECT * FROM my_join
...
The internal watermark handling mechanism for interval joins (joins combining equality conditions with watermark-based inequality or range conditions) was redesigned in v2.9.0. Existing interval joins created before upgrading to v2.9.0 will have their in-memory cache state cleaning invalidated. Drop and recreate these joins after upgrading to restore correct watermark-based state cleaning.
</Warning>
...
- The join condition must include the key columns that form the distribution key of the lookup table (right table expression).
- Use `DESCRIBE <table_name>` to view the table's key information.
- You can create an index on the lookup table to facilitate the temporal join if the required key columns are not part of the primary key.
...
<Warning>
**Temporal joins are asymmetric**

Temporal joins only produce output when data changes on the **left-hand (stream) side**. Updates or inserts to the right-hand (lookup) side alone do **not** generate any new join output — they only update the reference data that will be used for future left-side lookups.
</Warning>
...
<Note>
Added in v2.5.0. It is currently in **[technical preview](/changelog/product-lifecycle#product-release-lifecycle)** stage, with some known performance issues and bugs tracked [here](https://github.com/risingwavelabs/risingwave/issues/20408).
</Note>
