...
For wide tables that use `ON CONFLICT DO UPDATE IF NOT NULL`, RisingWave preserves row-level changes automatically so partial updates for the same target primary key are applied correctly.
