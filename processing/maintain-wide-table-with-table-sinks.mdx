...
To prevent `DELETE` events from removing rows in the wide table, set `ignore_delete = 'true'`:
...
For RisingWave v2.6 and earlier, use `type = 'append-only'` and `force_append_only = 'true'` for this pattern.
</Note>
...
/* Dimension sinks ignore DELETE to avoid propagating them into the wide table. */
...
<Frame>
  <img src="/images/maintain_wide_table_with_table_sink.drawio.png"/>
</Frame>
