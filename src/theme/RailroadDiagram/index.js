...
export const Options = {
  ...
  CHAR_WIDTH: 7.5, // width of each monospace character. play until you find the right value for your font
...
export class Choice extends DiagramMultiContainer {
  constructor(normal, ...items) {
    ...
    for (var i = first; i < normal; i++) {
      ...
      this.up += Math.max(
        arcs,
        this.items[i].height + this.items[i].down + Options.VS + this.items[i + 1].up
      );
    }
    ...
    for (i = normal + 1; i <= last; i++) {
      ...
      this.down += Math.max(
        arcs,
        this.items[i - 1].height + this.items[i - 1].down + Options.VS + this.items[i].up
      );
    }
  ...
  format(x, y, width) {
    ...
    for (var i = this.normal - 1; i >= 0; i--) {
      ...
      distanceFromY += Math.max(
        Options.AR,
        item.up + Options.VS + (i === 0 ? 0 : this.items[i - 1].down + this.items[i - 1].height)
      );
    }
    ...
    for (i = this.normal + 1; i <= last; i++) {
      ...
      distanceFromY += Math.max(
        Options.AR,
        item.height + item.down + Options.VS + (i == last ? 0 : this.items[i + 1].up)
      );
    }
...
export class Group extends FakeSVG {
  ...
  format(x, y, width) {
    ...
    if (this.label) {
      this.label
        .format(x, y - (this.boxUp + this.label.down + this.label.height), this.label.width)
        .addTo(this);
    }
