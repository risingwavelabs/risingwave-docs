...
export class Stack extends DiagramMultiContainer {
  ...
  toTextDiagram() {
    var [corner_bot_left, corner_bot_right, corner_top_left, corner_top_right, line, line_vertical] =
...
export class OptionalSequence extends DiagramMultiContainer {
  constructor(...items) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (i > 0) {
        this.down =
          Math.max(this.height + this.down, heightSoFar + Math.max(arc * 2, item.down + Options.VS)) - this.height;
      }
  ...
  toTextDiagram() {
    ...
    for (var [itemNum, itemTD] of enumerate(itemTDs)) {
      ...
      var separator = hasSeparator
        ? [line_vertical + " ".repeat(TextDiagram._maxWidth(diagramTD, itemTD) - 2) + line_vertical]
        : [];
...
export class HorizontalChoice extends DiagramMultiContainer {
  ...
  format(x, y, width) {
    ...
    // upper track
    var upperSpan =
      sum(allButLast, (x) => x.width + (x.needsSpace ? 20 : 0)) + (this.items.length - 2) * Options.AR * 2 - Options.AR;
    new Path(x, y)
      .arc("se")
...
export class OneOrMore extends FakeSVG {
  ...
  toTextDiagram() {
    var [line, repeat_top_left, repeat_left, repeat_bot_left, repeat_top_right, repeat_right, repeat_bot_right] =
      TextDiagram._getParts([
        "line",
        "repeat_top_left",
        "repeat_left",
...
export class Terminal extends FakeSVG {
  constructor(text, { href, title, cls } = {}) {
    ...
    this.cls = cls;
    this.width =
      this.text.length * Options.CHAR_WIDTH + 20; /* Assume that each char is .5em, and that the em is 16px */
    this.height = 0;
    this.up = 11;
    this.down = 11;
    this.needsSpace = true;
  ...
  format(x, y, width) {
    ...
    new FakeSVG("rect", { x: x, y: y - 11, width: this.width, height: this.up + this.down, rx: 10, ry: 10 }).addTo(
      this
    );
...
export class Block extends FakeSVG {
  ...
  format(x, y, width) {
    ...
    new FakeSVG("rect", { x: x, y: y - this.up, width: this.width, height: this.up + this.height + this.down }).addTo(
      this
    );
    return this;
  }
...
export class TextDiagram {
  ...
  static _rectish(rectType, data, dashed = false) {
    ...
    var [topLeft, ctrLeft, botLeft, topRight, ctrRight, botRight, topHoriz, botHoriz, line, cross] =
      TextDiagram._getParts([
        rectType + "_top_left",
...
function SVG(name, attrs, text) {
  ...
  for (var attr in attrs) {
    if (attr === "xlink:href") el.setAttributeNS("http://www.w3.org/1999/xlink", "href", attrs[attr]);
