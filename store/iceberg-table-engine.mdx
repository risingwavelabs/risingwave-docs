...
<Note>For better security, avoid hardcoding credentials like S3 access keys. Instead, store them as secrets and reference the secret name. See [Manage secrets](/operate/manage-secrets) for details.</Note>
