...
description: "Learn how to ingest data into RisingWave: continuous streaming, one-time batch loads, ad-hoc full reloads, and periodic ingestion."
---
...
## Ad-hoc batch ingestion

**What it is**: Loading or reloading a batch table on demand with `REFRESH TABLE`, without configuring a refresh schedule.

**When to use**: For controlled backfills, one-off reloads, or external tables that change occasionally and should only be pulled when you decide.

This pattern is available for tables created with `refresh_mode = 'FULL_RELOAD'`. If you omit `refresh_interval_sec`, the table loads only when you manually run `REFRESH TABLE`. See [Ingest data from Iceberg tables](/iceberg/ingest-from-iceberg), [Ingest data from Snowflake](/ingestion/sources/snowflake), and [`REFRESH TABLE`](/sql/commands/sql-refresh-table) for connector-specific details.

### Example: Manual refresh for an Iceberg table

```sql
CREATE TABLE iceberg_manual_snapshot (
    PRIMARY KEY (id)
) WITH (
    connector = 'iceberg',
...
REFRESH TABLE iceberg_manual_snapshot;
```

## Periodic ingestion

**What it is**: For most connectors, RisingWave doesn't provide a general-purpose scheduler for arbitrary ingestion jobs, so recurring loads are typically orchestrated with external tools like Cron or Airflow. Some refreshable batch tables, such as Iceberg and Snowflake, can also refresh natively when `refresh_interval_sec` is set.
...
| **Apache Iceberg** | ❌ | ✅ | ✅ | Manual one-time load via `REFRESH TABLE`; native periodic refresh when `refresh_interval_sec` is set |
| **Snowflake** | ❌ | ✅ | ✅ | Manual one-time load via `REFRESH TABLE`; native periodic refresh when `refresh_interval_sec` is set |
...
- **Periodic**: Use native refresh intervals for refreshable Iceberg or Snowflake tables, or external orchestration for other connectors.
