...
To perform a one-time snapshot of a PostgreSQL (or MySQL, SQL Server) table, use the shared source + derived table pattern. The derived table performs an initial snapshot of the upstream table when created. You can drop the table and source after the snapshot is complete if you only need a one-time load.
...
-- 2. Create a table from the source — this performs the initial snapshot
CREATE TABLE historical_sales (
  ...
  sale_date DATE,
  amount DECIMAL
) FROM pg_source TABLE 'public.sales_history';

-- 3. After the snapshot is complete, drop the table and source if no ongoing CDC is needed
