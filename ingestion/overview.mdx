...
For PostgreSQL and MySQL, use the [`postgres_query`](/integrations/sources/postgresql-table) or [`mysql_query`](/integrations/sources/mysql-table) table-valued function to query the upstream database on demand. Unlike a CDC connector, each invocation executes the upstream query once and does not continue reading database changes.

```sql
-- Run the PostgreSQL query once and store its result in RisingWave.
CREATE TABLE historical_sales AS
SELECT *
FROM postgres_query(
  'localhost',
  '5432',
  ...
  '<password>',
  'analytics',
  'SELECT sale_id, product_id, quantity, sale_date, amount FROM public.sales_history;'
);
```
...
CREATE SOURCE s3_logs (
  ...
  s3.credentials.access = '<your-access-key>',
  s3.credentials.secret = '<your-secret-key>',
  match_pattern = 'logs/2024/*.csv'
...
**What it is**: Reloading or incrementally importing external data on a schedule.

**When to use**: For scheduled data updates, daily or hourly batch processing, or when you need precise control over ingestion timing.

S3, GCS, Iceberg, and Snowflake tables can use `refresh_mode = 'FULL_RELOAD'`. Set `refresh_interval_sec` for native periodic refresh, or omit it and run [`REFRESH TABLE`](/sql/commands/sql-refresh-table) when needed. Full-reload refresh is currently in [technical preview](/changelog/product-lifecycle#product-release-lifecycle).

Use an external orchestrator such as Cron or Airflow for connectors without native refresh support or for custom incremental-loading logic.
...
| **PostgreSQL** | ✅ | ✅ | ⚠️ | CDC for streaming; `postgres_query` for one-time batch loads |
| **MySQL** | ✅ | ✅ | ⚠️ | CDC for streaming; `mysql_query` for one-time batch loads |
| **SQL Server CDC** | ✅ | ❌ | ⚠️ | CDC for streaming; no native one-time batch query |
| **MongoDB CDC** | ✅ | ❌ | ⚠️ | CDC for streaming; no native one-time batch query |
| **AWS S3** | ✅ | ✅ | ✅ | Append-only file discovery; batch queries; optional full reload |
| **Google Cloud Storage** | ✅ | ✅ | ✅ | Append-only file discovery; batch queries; optional full reload |
| **Azure Blob** | ✅ | ✅ | ⚠️ | Append-only file discovery and batch queries; external orchestration for scheduled reloads |
| **Apache Iceberg** | ✅ | ✅ | ✅ | Append-only continuous ingestion; batch queries; optional full reload |
...
- [What is a Source?](/reference/what-is-source) — Source concepts, connectors, and source vs. table
- [What is CDC?](/reference/what-is-cdc) — Native CDC connectors for PostgreSQL, MySQL, SQL Server, MongoDB
- [Data processing](/processing/overview) — Transform ingested data with materialized views
- [Data delivery](/delivery/overview) — Send processed results to downstream systems
