...
If you do not need to backfill existing rows, you can instead expose one table from a shared MySQL, PostgreSQL, or SQL Server CDC source as a catalog-only upsert source. Each downstream streaming job reads changes independently, starting when that job is created:
...
CREATE MATERIALIZED VIEW orders_state AS
SELECT * FROM orders_cdc;
```

This source cannot be queried directly and does not store the current table state. For the complete syntax and limitations, see [CDC table sources](/sql/commands/sql-create-source#cdc-table-sources).
