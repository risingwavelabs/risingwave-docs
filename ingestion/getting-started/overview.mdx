...
*   **Table:** Represents data stored within RisingWave's internal storage. There are two main types of tables:
    *   **Standard (batch) tables:** Created using a regular `CREATE TABLE` statement *without* a `WITH` clause. You populate these tables manually (for example, using `INSERT` statements).
        ...
        ```
    *   **Streaming (connector-backed) tables:** Created using a `CREATE TABLE ... WITH (connector=...)` statement. These tables are automatically and continuously populated by ingesting data from an external source connected via a *connector*.
        ```sql
        ...
        ```
*   **Connector:** A component that enables RisingWave to interface with a specific type of data source or sink (for example, Kafka or PostgreSQL).
*   **Materialized View:** A materialized view represents the results of a query, similar to a view. However, unlike a view, the results of a materialized view are stored and continuously updated in the background, enabling low-latency access to precomputed data.
*   **Sink:** A connection to an *external system* where RisingWave can *deliver* processed data (for example, Kafka, PostgreSQL, or cloud storage). **Creating a sink is optional.** Applications can also directly query data within RisingWave.
...
1.  **`CREATE SOURCE` (data not stored):**
    ...
    *   **Learn more:** [Connecting with `CREATE SOURCE`](/path/to/create-source)
...
2.  **`CREATE TABLE ... WITH (connector=...)` (data stored, continuous ingestion):**
    ...
    *   **Learn More:** [Connecting with `CREATE TABLE`](/path/to/create-table)
...
2.  **(Optional) Create *materialized views* to transform the data.** Materialized views continuously process data from sources or tables and store the *results* in RisingWave. This is an efficient way to build streaming pipelines without storing all the raw data.
3.  **(Optional) Create *sinks* using `CREATE SINK` to deliver processed data to external systems.** Applications can also connect *directly* to RisingWave and query the results of materialized views or tables *without* a sink.
...
Whether a CREATE SOURCE provides continuous data or a one-time snapshot depends on the source connector.
...
*   **Databases (via CDC):** PostgreSQL, MySQL, SQL Server, MongoDB, Citus
*   **Cloud storage:** Amazon S3, Google Cloud Storage, Azure Blob Storage
*   **Others:** NATS JetStream, MQTT, Apache Iceberg

See [Supported data sources](/path/to/supported-sources) for a complete list. RisingWave uses *connectors* to interface with each source.

## Supported data formats and encodings

RisingWave supports various data formats and encodings. See [Supported data formats and encodings](/path/to/supported-formats) for details.

## Next steps

*   Explore the [Supported data sources](/path/to/supported-sources) and [Supported data formats and encodings](/path/to/supported-formats).
*   Learn how to connect to specific sources using the individual connector guides (e.g., [Connect to Kafka](/path/to/kafka-connect)).
*   Learn more about using [`CREATE SOURCE`](/path/to/create-source) and [`CREATE TABLE`](/path/to/create-table).