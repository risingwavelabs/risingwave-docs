...
*   `FORMAT format_type ENCODE encode_type (...)`: This section specifies how the data is formatted and encoded.
    ...
    *   `(...)`: These are *format- and encoding-specific* options. The available options depend on the chosen `FORMAT` and `ENCODE`. See [Data formats and encoding options](/ingestion/getting-started/data-formats-and-encoding-options) for details.
...
- Learn about [Data formats and encoding options](/ingestion/getting-started/data-formats-and-encoding-options).
- See the documentation for specific connectors (e.g., [Connect to Kafka](/ingestion/sources/apache-kafka)).