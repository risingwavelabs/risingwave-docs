...
Each Kafka source also exposes the virtual column `_rw_kafka_timestamp`, which contains the Kafka message timestamp.

You can include this column in views or materialized views:
...
If you query the source directly, you can use `_rw_kafka_timestamp` to filter messages by time:
