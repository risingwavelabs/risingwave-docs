---
title: "Ingest from Apache Kafka"
...
### Query the implicit Kafka timestamp

If a Kafka source does not explicitly include the Kafka timestamp with
`INCLUDE timestamp`, RisingWave adds the implicit `_rw_kafka_timestamp` column.
This column contains the timestamp of each Kafka message. RisingWave does not
add this implicit column to a Kafka table; use `INCLUDE timestamp` when creating
the table to expose the timestamp under the specified column name.

You can use the column in a view or materialized view:

```sql
CREATE MATERIALIZED VIEW recent_events AS
SELECT _rw_kafka_timestamp, user_id, product_id
FROM my_kafka_source;
```

When querying a source directly, you can also use the column to filter messages
by time. For example, this query selects messages from the past 10 minutes:

```sql
SELECT *
FROM my_kafka_source
WHERE _rw_kafka_timestamp > now() - INTERVAL '10 minutes';
```

If you explicitly add `INCLUDE timestamp AS kafka_timestamp`, use the specified
alias instead; RisingWave does not add `_rw_kafka_timestamp` in that case.
