...
*   **`CREATE SOURCE`:** Creates a connection *without* storing data in RisingWave. See [Connecting with `CREATE SOURCE`](/path/to/ingest-data-create-source.md) for details.
*   **`CREATE TABLE`:** Creates a connection *and* stores the ingested data in RisingWave. See [Connecting with `CREATE TABLE`](/path/to/ingest-data-create-table.md) for details.
...
## Ingesting additional fields with the INCLUDE clause
