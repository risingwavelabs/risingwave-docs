...
Use the following syntax to create a Kafka source or table:

```sql
CREATE { SOURCE | TABLE } [ IF NOT EXISTS ] source_or_table_name
[ (
    { * | column_name data_type [ column_constraint ] }, ...
    [ PRIMARY KEY ( column_name, ... ) ]
    [ WATERMARK FOR column_name AS expression ]
) ]
[ INCLUDE kafka_metadata [ AS column_name ] ] [...]
WITH (
    connector = 'kafka',
    topic = 'topic_name',
    properties.bootstrap.server = 'broker:port',
...
For `kafka_metadata`, use one of the following forms:

```sql
{ key | timestamp | partition | offset | payload | header }
header '<header_key>' [ data_type ]
```

You can repeat the `INCLUDE` clause to extract multiple metadata fields. For
example:

```sql
INCLUDE key AS kafka_key
INCLUDE timestamp AS kafka_timestamp
INCLUDE header 'trace_id' VARCHAR AS trace_id
```

Column constraints differ between sources and tables. See
[`CREATE SOURCE`](/sql/commands/sql-create-source) and
[`CREATE TABLE`](/sql/commands/sql-create-table) for their complete
column-definition options. For Avro and Protobuf, you can omit the column
definitions when RisingWave derives the schema from the configured schema
location or registry. See [Format and encoding options](#format-and-encoding-options)
for the supported `FORMAT`, `ENCODE`, and encoding options.
...
`scan.startup.mode` only applies to streaming jobs, such as when building a table or creating a materialized view on a Kafka source. When directly querying a source, RisingWave will always read from the earliest available messages. For more information about querying sources, see [Querying a source](/ingestion/create-source-vs-create-table).

<Note>
RisingWave reads Kafka messages compressed with [zstd](https://www.zstd.net/)
without additional connector configuration.
</Note>
...
| `group.id.prefix`                     | Specifies a custom prefix for the consumer group ID. Can be modified at runtime without recreating the source. RisingWave uses consumer groups to track progress, but does *not* rely on committed offsets. The default prefix is `rw-consumer`. The full consumer group ID is formed as `{group_id_prefix}-{fragment_id}`. | string  |
...
| `properties.auto.commit.interval.ms`   | The frequency in milliseconds that the consumer offsets are committed. This is useful for external Kafka consumer-group monitoring only; RisingWave does not rely on committed offsets for correctness.                                                                             | int     |
...
| `properties.reconnect.backoff.ms`      | The initial backoff time in milliseconds before reconnecting to a broker after a connection closes.                                                                                                                                                                                 | int     |
| `properties.reconnect.backoff.max.ms`  | The maximum backoff time in milliseconds before reconnecting to a broker after a connection closes.                                                                                                                                                                                 | int     |
| `properties.retry.backoff.ms`          | The initial backoff time in milliseconds before retrying a failed Kafka protocol request.                                                                                                                                                                                           | int     |
| `properties.retry.backoff.max.ms`      | The maximum backoff time in milliseconds before retrying a failed Kafka protocol request.                                                                                                                                                                                           | int     |
| `properties.socket.connection.setup.timeout.ms` | The maximum time in milliseconds allowed for broker connection setup, including TCP setup and SSL/SASL handshakes.                                                                                                                                                        | int     |
...
<Tip>
**Troubleshoot `GroupAuthorizationFailed`**

If Kafka returns `GroupAuthorizationFailed` while RisingWave consumes a topic, add the consumer groups that use your `group.id.prefix` (the default is `rw-consumer`) to the Kafka ACL allowlist. For example:

```bash
./kafka-acls.sh --bootstrap-server <broker-address> \
  --add --allow-principal User:<user-or-service-account> \
  --group rw-consumer- --resource-pattern-type prefixed \
  --operation READ
```
</Tip>

<Note>
During source enumerator cleanup, RisingWave deletes the Kafka consumer groups derived from `group.id.prefix`. Cleanup is best-effort. Failures appear in warning logs and in the Prometheus metric `source_kafka_consumer_group_delete_failure_count{source_id,consumer_group}`. In the Grafana development dashboard, see **Kafka Metrics** > **Kafka Consumer Group Delete Failures** for the failure rate grouped by source and consumer group.
</Note>
...
<Warning>
RisingWave's `OAUTHBEARER` support enables only unsecured client-side JWTs and
does not support OpenID Connect (OIDC). Do not use unsecured tokens in
production.
</Warning>

## Format and encoding options

These parameters are used with `FORMAT` and `ENCODE` to specify how RisingWave should interpret the data in your Kafka messages. See [Data formats and encoding options](/ingestion/formats-and-encoding-options) for details on available formats and encodings.

| Parameter             | Notes                                                                                                                                                                                                                                                                              |
| :-------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| *data_format*         | Data format. Supported formats: `DEBEZIUM`, `UPSERT`, `PLAIN`.                                                                                                                                                                                                                    |
...
RisingWave supports Confluent Schema Registry and compatible registries such as [Karapace](https://aiven.io/docs/products/kafka/karapace). When you create a source, RisingWave uses the configured subject naming strategy (`TopicNameStrategy` by default), retrieves the subject's latest schema, and derives the source columns and data types from it.

Specify the registry in the `ENCODE` options:

```sql
ENCODE <encoding> (
  schema.registry = 'https://<schema-registry-address>'
)
```

<Note>
For Protobuf data, RisingWave bundles only protobuf [well-known types](https://protobuf.dev/reference/protobuf/google.protobuf/), such as `google/protobuf/timestamp.proto`, `duration.proto`, `any.proto`, `empty.proto`, `struct.proto`, and wrappers. Other imports, including Google common types such as `google/type/date.proto`, `google/type/latlng.proto`, `google/rpc/*`, `google/api/*`, and `confluent/meta.proto`, must be available as schema references in Confluent Schema Registry.

If an error reports missing schema references, the upstream Confluent serializer or converter might be treating these imports as known types and skipping their registration. Set `skip.known.types = false` on the serializer or converter so that Schema Registry stores the dependencies as references. See Confluent's [known-types list](https://github.com/confluentinc/schema-registry/blob/v8.4.0-124/protobuf-provider/src/main/java/io/confluent/kafka/schemaregistry/protobuf/ProtobufSchema.java#L271).
</Note>

### Schema evolution

RisingWave continues to use the schema version retrieved when the source is created. To use a newer writer schema after a compatible schema evolution, drop and recreate the source.
...
The following examples show how to align `properties.bootstrap.server`, `privatelink.endpoint`, and `privatelink.targets`:

**Multiple endpoints (1:1 mapping)**

`privatelink.endpoint` is a JSON array and must align 1:1 by order with the addresses in `properties.bootstrap.server`. The `privatelink.targets` port list must also align by order.
...
WITH (
  connector = 'kafka',
  topic = 'your_topic',
  properties.bootstrap.server = 'b-1.my-prod-kafka.abcd12.c2.kafka.us-east-1.amazonaws.com:9092,b-2.my-prod-kafka.abcd12.c2.kafka.us-east-1.amazonaws.com:9093,b-3.my-prod-kafka.abcd12.c2.kafka.us-east-1.amazonaws.com:9094',
  privatelink.endpoint = '[{"host": "vpce-0abc111111111111-1111.us-east-1.vpce.amazonaws.com"}, {"host": "vpce-0abc222222222222-2222.us-east-1.vpce.amazonaws.com"}, {"host": "vpce-0abc333333333333-3333.us-east-1.vpce.amazonaws.com"}]',
  privatelink.targets = '[{"port": 9092}, {"port": 9093}, {"port": 9094}]'
)
```

The JSON arrays map 1:1 to the broker addresses in `properties.bootstrap.server` by position.

**Single endpoint**

You can specify a single value for `privatelink.endpoint`. RisingWave automatically applies this endpoint to all broker addresses in `properties.bootstrap.server`. The `privatelink.targets` port list must still align by order.
  
This works whether brokers are in the same AZ or in different AZs, as long as the endpoint supports cross-AZ routing without restrictions.
...
WITH (
  connector = 'kafka',
  topic = 'your_topic',
  properties.bootstrap.server = 'nlb-kafka-use1a-1234567890abcdef.elb.us-east-1.amazonaws.com:9092,nlb-kafka-use1a-1234567890abcdef.elb.us-east-1.amazonaws.com:9093,nlb-kafka-use1a-1234567890abcdef.elb.us-east-1.amazonaws.com:9094',
  privatelink.endpoint = 'vpce-0abc111111111111-1111.us-east-1.vpce.amazonaws.com',
  privatelink.targets = '[{"port": 9092}, {"port": 9093}, {"port": 9094}]'
)
```
