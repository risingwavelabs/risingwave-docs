...
Additional mechanism-specific notes:

* `SASL/GSSAPI` is supported only with `SASL_PLAINTEXT`.
* For `SASL/PLAIN`, `SASL/SCRAM`, and `SASL/OAUTHBEARER` with SSL enabled, also provide the relevant `properties.ssl.*` parameters.
...
RisingWave supports reading schemas from a Web location in `http://...`, `https://...`, or `S3://...` format for Kafka data in Protobuf format. For Avro, use Confluent Schema Registry or AWS Glue Schema Registry instead.

For Protobuf, if a schema location is specified, the schema file must be a `FileDescriptorSet`, which can be compiled from a `.proto` file like this:

```bash
protoc -I=$include_path --include_imports --descriptor_set_out=schema_descriptor.pb schema.proto
```

Specify the schema location in the `ENCODE` clause:

```sql
ENCODE data_encode (
   schema.location = 'https://.../schema_descriptor.pb'
)
```

For example, you can load a descriptor from `https://example.com/schema_descriptor.pb` or `S3://bucket/schema_descriptor.pb`.

If a primary key is needed, define it in the table schema:
...
Confluent and [Karapace](https://aiven.io/docs/products/kafka/karapace) Schema Registry provide a serving layer for schema metadata over a REST interface.

RisingWave supports reading schemas from Confluent Schema Registry. When `CREATE SOURCE` runs, RisingWave retrieves the latest schema using the `TopicNameStrategy` strategy and infers the columns and data types from it.

Specify the schema registry in the `ENCODE` clause:
...
For Protobuf data, RisingWave bundles only protobuf [well-known types](https://protobuf.dev/reference/protobuf/google.protobuf/), such as `google/protobuf/timestamp.proto`, `duration.proto`, `any.proto`, `empty.proto`, `struct.proto`, and wrappers. Other imports must be available as schema references in Confluent Schema Registry.

If you see an error about missing schema references, Confluent's Protobuf serializer or converter may be treating those imports as known types and skipping registration. Set `skip.known.types = false` on the upstream Confluent serializer or converter so Schema Registry stores the dependencies as references.
...
Based on the compatibility type configured for the schema registry, some changes are allowed without changing the schema to a different version. In this case, RisingWave continues using the original schema definition. To use a newer version of the writer schema in RisingWave, drop and recreate the source.

To learn about compatibility modes and the changes they allow, see Confluent Schema Registry's [Compatibility Types](https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html#compatibility-types).
