...
- Multiple endpoints (1:1 mapping): `privatelink.endpoint` is a JSON array and must align 1:1 by order with the addresses in `properties.bootstrap.server`. The `privatelink.targets` port list must also align by order.
...
- Single endpoint: You can specify a single value for `privatelink.endpoint`. RisingWave automatically applies this endpoint to all broker addresses in `properties.bootstrap.server`. The `privatelink.targets` port list must still align by order.
