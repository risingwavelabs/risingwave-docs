...
- Different Availability Zones (multiple endpoints): `privatelink.endpoint` is a JSON array and must align 1:1 by order with the addresses in `properties.bootstrap.server`. The `privatelink.targets` port list must also align by order.
...
- Same Availability Zone (single endpoint): `privatelink.endpoint` can be a single string. RisingWave automatically applies this endpoint to all addresses in `properties.bootstrap.server`. The `privatelink.targets` port list must still align by order.
