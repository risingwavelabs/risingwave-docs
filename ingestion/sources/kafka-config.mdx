...
| `group.id.prefix`                     | Specifies a custom prefix for the consumer group ID. Can be modified at runtime without restarting the source. RisingWave uses consumer groups to track progress, but does *not* rely on committed offsets. The default prefix is `rw-consumer`. The full consumer group ID is formed as `{group_id_prefix}-{fragment_id}`. | string  |
