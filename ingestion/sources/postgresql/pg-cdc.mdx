...
RisingWave uses `string` for `debezium.decimal.handling.mode` to preserve decimal precision. Omit this property or set it to `string`; other values, including `precise` and `double`, are rejected.
...
* `ADD COLUMN [DEFAULT expr]`: Allows you to add a new column to an existing table. Constant defaults are backfilled to existing rows. If RisingWave cannot parse the upstream default expression, such as `now()` or `gen_random_uuid()`, it still applies the schema change, but existing rows in the new column are filled with `NULL`.
