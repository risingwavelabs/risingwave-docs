...
<Note>
RisingWave does not support directly creating tables from PostgreSQL composite types. If you want to read scalar composite type data, you will need to use a source and create a materialized view based off that source. Arrays of user-defined composite types are supported and are ingested as `varchar[]`, with each element preserved in PostgreSQL text format.
</Note>
