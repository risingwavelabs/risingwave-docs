...
<Note>
To ingest user-defined composite types, create a shared PostgreSQL CDC source and then create a table with `CREATE TABLE ... FROM source`. Scalar composite columns map to `varchar`, and composite array columns map to `varchar[]` with elements preserved in PostgreSQL text format. Set `debezium.include.unknown.datatypes = 'true'` in the source configuration. You can then create materialized views from that table if needed.
</Note>
