...
<Note>
If `publication.create.enable = 'true'`, the PostgreSQL user must have `CREATE` on the database and must own the tables that RisingWave adds to the publication. If the tables are owned by another user, create the publication in advance with a table owner account and set `publication.create.enable = 'false'`.
</Note>
