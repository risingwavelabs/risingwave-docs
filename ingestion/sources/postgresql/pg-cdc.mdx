...
<Warning>
This support applies to the standard CDC table path with materialized row state. [CDC upsert sources](/sql/commands/sql-create-source#cdc-table-sources) created with `snapshot = 'false'` cannot reconstruct unchanged TOAST values omitted from change records. Adding a downstream sink or `AS CHANGELOG` does not recover those values.
</Warning>
