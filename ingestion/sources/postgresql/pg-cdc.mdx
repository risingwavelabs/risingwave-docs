...
<Note>
PostgreSQL CDC supports `pgvector` columns only when the upstream type includes a dimension, such as `vector(3)` or `vector(768)`. Dimension-less `vector`, `halfvec`, and `sparsevec` are not supported.
</Note>
