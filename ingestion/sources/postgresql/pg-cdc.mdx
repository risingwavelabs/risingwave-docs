...
<Note>
PostgreSQL user-defined composite types are supported when the source is created with `debezium.include.unknown.datatypes = 'true'`. RisingWave ingests composite columns as `CHARACTER VARYING` using PostgreSQL's text representation, such as `(USD,12.34)`. If `auto.schema.change = 'true'` is enabled, newly added composite columns are also mapped to `CHARACTER VARYING`.
</Note>
