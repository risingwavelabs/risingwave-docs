...
RisingWave's PostgreSQL CDC connector is compatible with any PostgreSQL-compliant database that supports logical replication, and supports **PostgreSQL versions 10 through 18**. PostgreSQL 18 support was added in RisingWave v3.0.0.
...
If an upstream PostgreSQL schema or table name is a quoted identifier, preserve the PostgreSQL double quotes inside the string literal. For example, use `TABLE 'public."QuotedTable"'`.
...
For example, the following statement derives its schema from an upstream mixed-case table named `public."QuotedNote"`:

```sql
CREATE TABLE quoted_note (*)
FROM shared_source TABLE 'public."QuotedNote"';
```
...
<Warning>
A PostgreSQL replication slot can retain WAL while it remains on the upstream
database. Monitor the
[`pg_replication_slots`](https://www.postgresql.org/docs/current/view-pg-replication-slots.html)
view. When a slot is no longer used by a RisingWave source, remove it with
[`pg_drop_replication_slot()`](https://www.postgresql.org/docs/current/functions-admin.html#FUNCTIONS-REPLICATION)
to prevent WAL accumulation.
</Warning>
...
<Note>
To ingest user-defined composite types, create a shared PostgreSQL CDC source with `debezium.include.unknown.datatypes = 'true'`, then create a table with `CREATE TABLE ... FROM source`. Scalar composite columns map to `varchar` values in PostgreSQL text format. You can create materialized views from that table if needed.

Composite array columns map to `varchar[]`, with each element preserved in PostgreSQL text format.
</Note>

The connector uses a nullable text representation for composite columns. With PostgreSQL `REPLICA IDENTITY DEFAULT`, DELETE events can carry `NULL` for non-key composite columns even when the upstream column is declared `NOT NULL`.
...
For `INCLUDE timestamp`, historical rows read during the initial snapshot use
`1970-01-01 00:00:00+00:00`. Incremental changes use the upstream transaction's
commit timestamp.
...
Keep double quotes around quoted upstream identifiers in the `TABLE` value. For example: `TABLE 'public."QuotedTable"'`.
...
as `mood_enum[]`, and arrays of user-defined composite types to `varchar[]`.
Composite array elements are preserved in PostgreSQL text format.
