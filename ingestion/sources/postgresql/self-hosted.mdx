...
If RisingWave creates the publication automatically (`publication.create.enable = 'true'`), the user must also own each captured table. If the tables are owned by another user, create the publication ahead of time with a table owner account and then set `publication.create.enable = 'false'` in RisingWave.
