...
| :-------- | :---------- | :------- | :------ |
| server.id | Unique numeric ID for the database client. Must be unique across all database connections. | No | Auto-generated |
| snapshot.mode | Debezium snapshot mode for initial data capture. Options: initial, initial_only, never. | No | initial |
| snapshot.fetch.size | Number of rows to fetch per batch during snapshot. | No | 2000 |
| snapshot.isolation.mode | Transaction isolation level during snapshot. | No | repeatable_read |
| snapshot.interval | Interval between snapshots. | No | 1 |
| snapshot.batch_size | Batch size for snapshot operations. | No | 1000 |
| auto.schema.change | Enable automatic schema change detection. | No | false |
| ssl.mode | SSL connection mode: disabled, preferred, or required. | No | disabled |
| transactional | Enable transactional support for shared CDC source. | No | true |
| cdc.source.wait.streaming.start.timeout | Timeout for waiting for streaming to start (in milliseconds). | No | 60000 |
| table.whitelist | Comma-separated list of tables to include. Alternative to specifying single table.name. | No |  |
| table.blacklist | Comma-separated list of tables to exclude. | No |  |
| column.whitelist | Comma-separated list of columns to include. | No |  |
| column.blacklist | Comma-separated list of columns to exclude. | No |  |
...
```sql
-- Check current LSN (Log Sequence Number)
SELECT sys.fn_cdc_get_max_lsn() as max_lsn;

-- Check CDC changes for a specific table
DECLARE @begin_lsn binary(10), @end_lsn binary(10);
SET @begin_lsn = sys.fn_cdc_get_min_lsn('dbo_your_table');
SET @end_lsn = sys.fn_cdc_get_max_lsn();

SELECT * FROM cdc.fn_cdc_get_all_changes_dbo_your_table(@begin_lsn, @end_lsn, 'all');
```
