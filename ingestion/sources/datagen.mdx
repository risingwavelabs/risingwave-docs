...
description: "The built-in load generator can generate mock data, which can be used in demos and tests. It provides an easy way to simulate the data stream without connecting to an actual external data source."
sidebarTitle: "Datagen"
---

Use the built-in load generator to create synthetic streams for local development, demos, and connector-free testing.

The datagen connector emits insert-only rows. It does not generate updates or deletes, so it is not suitable for testing CDC, upsert, or delete-driven workloads.
...
WITH (
   ...
   datagen.split.num = ' split_integer '       -- Optional. Specify how many generator splits to use.
) FORMAT PLAIN ENCODE JSON;
```

## Connector options

Specify these options in the `WITH` clause.

| Option | Description |
| :----- | :---------- |
| `datagen.rows.per.second` | Total number of rows generated per second across all datagen splits. Default: `10`. |
| `datagen.split.num` | Number of generator splits. Use this when you want the generated workload to be partitioned across multiple splits. If you set `datagen.split.num = '3'` and `datagen.rows.per.second = '10'`, RisingWave distributes the output across the three splits. |
