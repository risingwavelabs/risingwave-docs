...
For MySQL 8 servers that use the default `caching_sha2_password` authentication plugin, RisingWave sets `driver.allowPublicKeyRetrieval=true` for the embedded Debezium connection. The corresponding pass-through option is `debezium.driver.allowPublicKeyRetrieval`; you do not need to set it to enable public key retrieval.
