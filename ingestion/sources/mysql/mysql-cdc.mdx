...
RisingWave sets `driver.allowPublicKeyRetrieval=true` by default for embedded Debezium MySQL CDC connections, enabling public key retrieval when required by authentication plugins such as `caching_sha2_password`. The corresponding pass-through option is `debezium.driver.allowPublicKeyRetrieval`; you do not need to set it to enable public key retrieval.

Public key retrieval does not enable TLS. Set `ssl.mode = 'required'` to require encrypted MySQL connections; this mode does not verify the server certificate. To turn off retrieval for the embedded Debezium streaming connection, set `debezium.driver.allowPublicKeyRetrieval = 'false'`. This override does not affect the separate connection used for source validation, which still enables retrieval.
