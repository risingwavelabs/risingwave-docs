...
The following parameters are available for advanced CDC configuration:

| Parameter | Description | Required | Default |
| :-------- | :---------- | :------- | :------ |
| `snapshot` | Enable or disable initial snapshot. Set to `false` to disable CDC backfill. | No | `true` |
| `snapshot.mode` | Debezium snapshot mode. See [Debezium documentation](https://debezium.io/documentation/reference/stable/connectors/mysql.html#mysql-connector-properties) for options. | No | `initial` |
| `snapshot.interval` | Interval between snapshots. <!-- TODO: Add detailed description and units --> | No |  |
| `snapshot.batch_size` | Batch size for snapshot operations. <!-- TODO: Add detailed description --> | No |  |
| `backfill.parallelism` | Parallelism for backfill operations. <!-- TODO: Add detailed description and limits --> | No |  |
| `backfill.num_rows_per_split` | Number of rows per split for backfill. <!-- TODO: Add detailed description --> | No |  |
| `backfill.as_even_splits` | Whether to create even splits for backfill. <!-- TODO: Add detailed description --> | No |  |
| `backfill.split_pk_column_index` | Primary key column index for backfill splitting. <!-- TODO: Add detailed description --> | No |  |
| `transactional` | Enable transactional support for shared CDC source. | No | `true` |
| `cdc.source.wait.streaming.start.timeout` | Timeout for waiting for streaming to start. <!-- TODO: Add units and detailed description --> | No |  |
