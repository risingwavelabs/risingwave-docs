...
At present, combined with the [ALTER SOURCE command](/sql/commands/sql-alter-source#format-and-encode-options), you can refresh the schema registry of a source by refilling its [FORMAT and ENCODE options](/ingestion/formats-and-encoding-options). The syntax is:
...
In addition, when the [FORMAT and ENCODE options](/ingestion/formats-and-encoding-options) are not changed, the `REFRESH SCHEMA` clause of `ALTER SOURCE` can also be used to refresh the schema of a source.
