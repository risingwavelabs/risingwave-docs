...
| `UPSERT`        | `PROTOBUF`  | Protobuf for updates/deletes (requires schema location/registry). |
...
<Note>
When using `FORMAT UPSERT` with `CREATE SOURCE`, be aware that upsert sources cannot be used directly with stateful operations like JOIN, aggregations, or window functions. For these use cases, either materialize the source as a view first, or use `CREATE TABLE` instead. See [Upsert sources](/sql/commands/sql-create-source#upsert-sources) for details.
</Note>
...
For Kafka topics with updates and deletes encoded as Protobuf values, use `FORMAT UPSERT ENCODE PROTOBUF`. As with other upsert formats, you must include the Kafka key and use it as the primary key.
...
If you need key-based upsert semantics with bytes payloads, RisingWave does not provide a separate `FORMAT UPSERT ENCODE BYTES` syntax. Instead, use `FORMAT PLAIN ENCODE BYTES`, add `INCLUDE KEY`, and declare the key column as the primary key.
...
| PLAIN | AVRO | `schema.registry.username` | The username for schema registry. Optional, but if set it must be used together with `schema.registry.password`. | No |
| PLAIN | AVRO | `schema.registry.password` | The password for schema registry. Optional, but if set it must be used together with `schema.registry.username`. | No |
...
| UPSERT | AVRO | `schema.registry.username` | The username for schema registry. Required if `schema.registry.password` is set. | No |
| UPSERT | AVRO | `schema.registry.password` | The password for schema registry. Required if `schema.registry.username` is set. | No |
...
| DEBEZIUM | AVRO | `schema.registry.username` | The username for schema registry. Required if `schema.registry.password` is set; configure together with `schema.registry.password`. | No |
| DEBEZIUM | AVRO | `schema.registry.password` | The password for schema registry. Required if `schema.registry.username` is set; configure together with `schema.registry.username`. | No |
...
| PLAIN | PROTOBUF | `schema.location` | URL of the schema file (alternative to `schema.registry`). | Yes (one of) |
| PLAIN | PROTOBUF | `schema.registry` | URL(s) of the schema registry (alternative to `schema.location`). | Yes (one of) |
...
| UPSERT | PROTOBUF | `message` | Fully qualified name of the Protobuf message. | Yes |
| UPSERT | PROTOBUF | `schema.location` | URL of the schema file (alternative to `schema.registry`). | Yes (one of) |
| UPSERT | PROTOBUF | `schema.registry` | URL(s) of the schema registry (alternative to `schema.location`). | Yes (one of) |
| UPSERT | PROTOBUF | `schema.registry.ca_pem_path` | Path to a custom root CA certificate file (PEM) for the schema registry TLS connection, or `'ignore'` to skip certificate verification (not recommended for production). Supports `SECRET ... AS FILE`. | No |
...
You cannot set this parameter for `FORMAT DEBEZIUM_MONGO ENCODE JSON`.
