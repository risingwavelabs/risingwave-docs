...
For CDC tables created with parallelized backfill enabled (`backfill.parallelism` > 0), you can monitor the overall progress using the system catalog:

### Using the rw_cdc_progress system catalog

The `rw_cdc_progress` system catalog provides a high-level overview of CDC backfill progress:
