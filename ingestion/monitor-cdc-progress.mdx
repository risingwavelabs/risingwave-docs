...
2.  **Query the internal table:**  Use a `SELECT` statement to query the internal table. The specific columns and their meanings may vary slightly depending on the source, but generally, you'll find information about:
    ...
    *   `row_count`: The total number of rows processed so far. This value is persisted, so after recovery it continues to reflect the full backfill row count instead of restarting from zero.
    ...
    **Example (PostgreSQL):**
    ...
    RisingWave also uses this persisted backfill row count when reporting snapshot backfill progress in `rw_catalog.rw_ddl_progress`, so the reported count remains cumulative after recovery.
