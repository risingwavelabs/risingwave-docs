...
<Note>
The `rwadmin` user cannot be altered, renamed, or dropped to ensure system security. Only the `rwadmin` user itself can change its password. For existing clusters, `rwadmin` is not automatically created but can be manually created by other superusers if needed.
</Note>
