...
**Note that the `progress` given here is just an estimate.** For statements that use snapshot backfill, if the cluster recovers while the statement is still running, the reported consumed row count resumes from the persisted backfill state instead of restarting from zero.
