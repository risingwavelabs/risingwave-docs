...
### Limit Parallelism for less intensive Stream Jobs

If you want to limit the parallelism for certain types of jobs, you can use session parameters like `streaming_parallelism_for_table` to set specific limits.
...
By setting the parallelism strategy for a specific streaming job, you can specify the exact parallelism when creating the corresponding job.

If you want to reset to the default strategy, simply set it to "default" and it will revert to the global streaming_parallelism.
...
You can also use this syntax to sync this configuration globally.
