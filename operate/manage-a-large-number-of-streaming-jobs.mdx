...
### Configure unified parallelism values

Streaming parallelism is configured through `streaming_parallelism` and per-job-type overrides such as `streaming_parallelism_for_table`. These parameters accept:

- `adaptive` or `0` to use the full available worker set
- A fixed integer such as `4`
- `bounded(n)` to cap adaptive parallelism
- `ratio(r)` to use a fraction of the available parallelism

For example:

```sql
SET streaming_parallelism = 'bounded(4)';
SET streaming_parallelism_for_table = 'ratio(0.5)';
SET streaming_parallelism_for_materialized_view = 'adaptive';
SET streaming_parallelism_for_sink = 'bounded(2)';
```

Create jobs with specific parallelism values:

```sql Example
SET streaming_parallelism_for_table = 'bounded(2)';
CREATE TABLE my_table (id INT, value VARCHAR);

SET streaming_parallelism_for_materialized_view = 'ratio(0.5)';
CREATE MATERIALIZED VIEW my_mv AS SELECT * FROM my_table;
```

For more details on supported values and default resolution, see [Unified streaming parallelism values](/operate/view-configure-runtime-parameters#unified-streaming-parallelism-values).
...
----+----------+---------------+-------------+-----------------
  7 | t        | table         | 2           |             256
  6 | t_normal | table         | 1           |             256
  8 | i        | index         | 3           |             256
(3 rows)
```

By setting the parallelism value for a specific job type, you can control the exact parallelism policy used when the job is created.

If you want to reset to the default value, simply set it to `default`. The parallelism will then fall back to the global `streaming_parallelism` setting.
