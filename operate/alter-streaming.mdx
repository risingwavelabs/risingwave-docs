...
```sql
CREATE MATERIALIZED VIEW m1 as select v2, count(*) from t group by v2;
```

Modify this materialized view:

```sql
CREATE MATERIALIZED VIEW m2 as select v2, count(*), max(v1) from t group by v2;
```
  </Step>
...
CREATE TABLE m1_store(
    v2 varchar primary key, 
    cnt int, 
    -- The version column is not required, but recommended. See Notes below.
    version int,
) ON CONFLICT OVERWRITE WITH VERSION COLUMN(version);

CREATE SINK m1_stream_v1 INTO m1_store AS SELECT v2, count(*) as cnt, 1 as version from t group by v2;
```
  </Step>
...
SELECT v2, count(*) as cnt, max(v1) as max, 2 as version
from t group by v2;
```

Drop the old streaming job:
```sql
