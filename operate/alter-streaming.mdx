...
If your sink is created from a table or materialized view, you can specify `snapshot = false` to exclude existing data. This applies to both `CREATE SINK ... FROM ...` and sink queries created with `CREATE SINK ... AS SELECT ...`, as long as the sink query only uses operators that can consume upstream-only changes, such as projection and filtering.

```
CREATE SINK ... FROM ... WITH (snapshot = false);
CREATE SINK ... AS SELECT ... WITH (snapshot = false);
```
