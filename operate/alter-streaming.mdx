...
Here we create a new materialized view `cust_sales_new` with the new column `sales_vount`:
...
CREATE MATERIALIZED VIEW cust_sales_new AS
    SELECT
        customer_id,
        SUM(total_price) AS sales_amount,
        COUNT(*) AS sales_vount -- The new column
    FROM orders
    GROUP BY customer_id;
...
To add a new column `sales_vount` to `cust_sales`, we need to create the new materialized views `cust_sales_new` and `orders_new` first:
<Frame>
...
CREATE MATERIALIZED VIEW cust_sales_new AS
    SELECT
        customer_id,
        SUM(total_price) AS sales_amount,
        SUM(item_vount) AS sales_vount -- The new column
