...
Resource groups are defined by assigning compute nodes to them at startup. Each compute node can be independently assigned to a named group, and all workers launched with the same resource group name collectively form that logical group.

This assignment is done when launching a worker by using the `--resource-group` command-line flag or by setting the `RW_RESOURCE_GROUP` environment variable.
