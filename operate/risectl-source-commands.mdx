...
Unlike [`ALTER SOURCE ... CONNECTOR WITH`](/sql/commands/sql-alter-source) (which restricts updates to a predefined set of runtime-alterable fields), the `risectl` command bypasses this restriction and can update any connector property for any connector type. Use with caution.
...
- **Kafka**: Split ID is the partition number. Example: `{"0": "1000", "1": "2000"}`
- **Pulsar**: Split ID is the partition number. Offset is in the format `"{ledger_id}:{entry_id}:{partition}:{batch_index}"` (e.g., `"1234:56:0:-1"`).

<Warning>
For CDC sources, the offset format is complex and connector-specific (e.g., MySQL CDC uses `{"file": "...", "pos": ..., "gtid": ...}`). Refer to [Monitor CDC progress](/ingestion/monitor-cdc-progress) for the actual offset structure before using this command with CDC sources.
</Warning>
...
Inject Kafka offsets (keys are partition IDs, values are offsets):
...
1. **Offset injection**: Delivers the specified offsets to the source via an internal coordination mechanism
2. **State overwrite**: Completely overwrites the stored split state with the new offsets
3. **Resumption**: Source consumption resumes from the injected offsets
