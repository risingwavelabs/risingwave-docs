...
Currently, only Kafka connector properties can be updated. Unlike [`ALTER SOURCE ... CONNECTOR WITH`](/sql/commands/sql-alter-source), the `risectl` command bypasses the runtime-alterable check, so it can update a broader set of properties. The commonly used properties are:
