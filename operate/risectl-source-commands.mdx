...
- **Kafka**: Split ID is the partition number (e.g., `"0"`, `"1"`). Offset is the Kafka offset as a string. Example: `{"0": "1000", "1": "2000"}`
- **Pulsar**: Split ID is the topic name. Offset is in the format `"{ledger_id}:{entry_id}:{partition}:{batch_index}"` (e.g., `"1234:56:0:-1"`).
...
3. **Reader rebuild**: Source readers are rebuilt automatically and consumption resumes from the injected offsets
