...
<Warning>
Resetting splits may cause data duplication depending on connector behavior. Use with caution in production environments.
</Warning>
...
The offset format varies by connector type:

- **Kafka**: `{"partition_id": "offset_value"}`
- **CDC**: `{"binlog_file": "position"}` or `{"lsn": "value"}`
...
- Currently only Kafka connector properties are supported. Support for other connectors will be added in future releases.
