...
2. Follow [the instruction](https://github.com/risingwavelabs/risingwave/blob/main/grafana/README.md) to set up the toolchain.

3. Run the following command to generate the Grafana dashboard JSON model.
```bash
...
4. Import the generated JSON models (*.gen.json) into Grafana.


### Troubleshooting

If the namespace filter is not working, it is probably because the metrics collected does not contain the label `risingwave_name`. In that case, you need to
use `metricRelabelings` and `relabelings` fields in the [endpoint spec](https://github.com/prometheus-operator/prometheus-operator/blob/main/Documentation/api-reference/api.md#endpoint) to add this label to the metrics. 
