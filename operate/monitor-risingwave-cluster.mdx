...
3. **RisingWave Console** (separate product): An enhanced on-premise UI with additional features like risectl command execution, automated diagnostics, and metadata snapshot management. See [RisingWave Console documentation](/web-ui/introduction) for details.

For the complete Prometheus metric catalog, see [Prometheus metrics reference](/operate/prometheus-metrics-reference).
...
4. Import the recommended pre-generated dashboard JSON into Grafana from [risingwave-dev-dashboard.json](https://github.com/risingwavelabs/risingwave/blob/main/grafana/risingwave-dev-dashboard.json). You can also use [risingwave-user-dashboard.json](https://github.com/risingwavelabs/risingwave/blob/main/grafana/risingwave-user-dashboard.json) if you prefer a higher-level dashboard.
5. From **Dashboards**, select **RisingWave** \-> **risingwave_dev_dashboard**. You can edit metrics in the dashboard and add new metrics.

<Frame>
  <img src="/images/current/monitor-risingwave-cluster/risingwave-grafana-k8s.png"/>
</Frame>

## Advanced: regenerate a Grafana dashboard for multiple RisingWave clusters

This section explains how to regenerate a Grafana dashboard for monitoring multiple RisingWave clusters deployed within a Kubernetes environment. These steps only generate the dashboard's JSON model and can be performed on any machine with the necessary tools.

<Note>
Most users should import the pre-generated JSON dashboards above and can ignore the Python generator scripts. The Python scripts under `grafana/` are the source of truth used by RisingWave developers with `grafanalib` to generate the JSON dashboards.
</Note>

If you need custom filters such as namespace and RisingWave cluster name filters, follow the steps below.

<Note>
We recommend using `risingwave_dev_dashboard` for monitoring because it is more comprehensive and better organized than `risingwave_dashboard`. Note that `generate.sh` produces both `risingwave-dev-dashboard.gen.json` and `risingwave-user-dashboard.gen.json` using the same `DASHBOARD_UID`; to avoid UID collisions on import, import only `risingwave-dev-dashboard.gen.json`.
</Note>
