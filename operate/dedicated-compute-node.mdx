...
After enabling the embedded serving mode, the frontend component will operate as the Serving Node, while the compute component will be dedicated to streaming operations only. This means:

- The frontend pods will accept SQL connections and execute serving (batch) queries.
...
Spilling temporarily writes data to the directory specified by `RW_BATCH_SPILL_DIR` (default: `/tmp/`). While spilling prevents OOM, it significantly impacts query performance due to disk I/O overhead.

If spilling is insufficient to keep memory usage within limits, the query will terminate. To prevent this, either reduce cache sizes to increase compute memory, or distribute the query across multiple nodes using `SET QUERY_MODE TO distributed`.
