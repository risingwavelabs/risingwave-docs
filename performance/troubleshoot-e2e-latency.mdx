...
In contrast, sink outputs are eagerly written to the downstream system based on sink buffer conditions, without waiting for barriers to commit. If the sink buffer fills up before the next barrier, the data is flushed immediately; otherwise, it may be held until the next barrier.
