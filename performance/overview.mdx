...
-   **Backpressure:** A mechanism that prevents RisingWave from being overwhelmed by data. When a downstream component cannot keep up with the upstream data flow, backpressure signals the upstream to slow down, ensuring system stability. Backpressure is most useful as a *secondary diagnostic signal*: start with direct job/fragment resource metrics to identify the hot fragment, then use backpressure to confirm downstream-propagation effects. This is a natural and essential part of stream processing.
-   **Resource utilization:** The consumption of resources like CPU, memory, and disk I/O. Monitoring resource utilization is key to identifying bottlenecks. RisingWave exposes these metrics at the streaming job and fragment level, enabling direct identification of resource-saturated fragments without requiring backpressure tracing.
