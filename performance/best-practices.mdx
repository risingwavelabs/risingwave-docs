...
In this example, since the query groups by column `b`, RisingWave’s optimizer can automatically use the index `idx_bc(b, c DESC)` to scan data in a locality-friendly order, improving cache utilization and backfilling speed.

#### Locality backfill

While index selection improves data reading order at the table scan level, locality backfill extends this optimization to the entire backfill pipeline. It makes joins and aggregations process data in a locality-aware order, reducing data shuffling and improving overall performance.
...
Once enabled, the optimizer automatically inserts `LocalityProvider` operators into the query plan to maintain data locality across joins and aggregations.
