...
- `auto` (default) enables locality backfill only for snapshot backfills whose estimated scan size reaches `auto_locality_backfill_min_size`, which defaults to `10737418240` bytes (10 GiB).
...
The default `auto` mode requires the locality backfill premium feature. In `always` mode, plans that use more than five `LocalityProvider` operators are considered complex queries and require the same premium feature; otherwise, RisingWave falls back to regular backfill. For a comprehensive overview of all premium features and their usage, please see [RisingWave premium features](/get-started/premium-features).
</Tip>
