...
Enable state table memory preloading for workloads with frequent state access and sufficient memory resources, particularly effective for small to medium-sized state tables experiencing high cache miss rates. See [State table memory preloading](/performance/streaming-optimizations#state-table-memory-preloading) for configuration details.
