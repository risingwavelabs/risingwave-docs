...
When enabled, the query optimizer automatically rewrites batch (ad-hoc) queries to scan a matching materialized view instead of the raw base tables. This is particularly useful for backfilling workloads that rely on large one-off or periodic batch queries over historical data, because they can reuse pre-computed results instead of repeatedly rescanning base tables. If a query's `SELECT` list, `WHERE` clause, and `GROUP BY` match an existing MV, RisingWave reads the pre-computed result directly—avoiding a full table scan and significantly reducing query latency.
