...
For workloads with frequent state access patterns, you can enable state table memory preloading to significantly improve performance by keeping state data in memory. This is particularly effective for:

- Complex stateful computations with large internal states
- Workloads with frequent random access to state data  
- Small to medium-sized state tables that fit in available memory

Consider enabling memory preloading when you have sufficient memory resources and observe high cache miss rates in state operations. See [State table memory preloading](/performance/streaming-optimizations#state-table-memory-preloading) for configuration details.

**Memory considerations:** Monitor memory usage carefully when enabling this feature, as it increases memory consumption but can provide substantial performance improvements for suitable workloads.
