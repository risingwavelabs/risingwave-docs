...
RisingWave provides two pre-generated Grafana dashboards that you can import directly:

1. [Dev dashboard (`risingwave-dev-dashboard.json`)](https://github.com/risingwavelabs/risingwave/blob/main/grafana/risingwave-dev-dashboard.json) — finer-grained metrics covering all components. **Recommended** for most monitoring needs.
2. [User dashboard (`risingwave-user-dashboard.json`)](https://github.com/risingwavelabs/risingwave/blob/main/grafana/risingwave-user-dashboard.json) — summarizes high-level metrics such as source and sink throughput, barrier latency, cluster alerts, and compute/source/sink streaming error rates.

The Python scripts under [`grafana/`](https://github.com/risingwavelabs/risingwave/tree/main/grafana) are the source of truth used by RisingWave developers with `grafanalib` to generate these JSON dashboards. Most users should import the pre-generated JSON above and can ignore the Python. If you need to customize or regenerate the dashboards, see the [Grafana README](https://github.com/risingwavelabs/risingwave/blob/main/grafana/README.md).
