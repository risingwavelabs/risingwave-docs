...
<AccordionGroup>
  ...
  <Accordion title="When should I deploy a dedicated batch-serving cluster?">
    ...
    If you have batch queries that require sub-second latency, a dedicated batch-serving cluster is recommended. This isolates batch query workloads, preventing resource contention with streaming jobs.  It also improves the availability of batch query processing, as failures of compute nodes used for stream processing won't affect batch queries. See [Best practices - Resource allocation](l/performance/best-practices#resource-allocation).
  </Accordion>
