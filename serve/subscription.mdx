...
sidebarTitle: "Subscribe to real-time updates"
description: Subscribe to real-time updates from tables and materialized views without external event stores like Kafka. Retrieve full or incremental data changes with minimal maintenance overhead.
---
...
- **I want incremental updates from now on**: declare a subscription cursor with `SINCE now()/proctime()`.
- **I want the full snapshot first, then incremental updates**: declare a subscription cursor with `FULL`.
- **I want to resume after a restart (no duplicates, no data loss)**: persist `rw_timestamp` and resume with `SINCE <rw_timestamp>`.

> Tip: Incremental data is only available within the subscription's `retention` window. If you resume from an old timestamp outside the window, the data is no longer accessible.
...
To rename a subscription, change the owner, set a new schema, or change its retention period, use the syntax below:
...
Increasing the retention period preserves incremental data for longer. Data that has already expired cannot be restored by increasing retention.
...
If you specify `FULL` instead of the `since_clause`, the subscription cursor starts consuming from the existing snapshot (the data that already exists at the time you declare the cursor), then continues to return incremental changes.

### What does the cursor return?

Each row returned by `FETCH` contains:

- The columns of the subscribed table/materialized view
- An `op` column describing the change type (`Insert`, `UpdateInsert`, `UpdateDelete`, `Delete`)
- A `rw_timestamp` column (Unix timestamp in milliseconds) indicating when the change was written

`rw_timestamp` is also the **progress token** you should persist if you want to resume consumption (see [Exactly-once delivery](#exactly-once-delivery)).
...
Before running the code below, make sure you have created a subscription, for example:

```sql
-- Replace the table name and subscription name with yours.
...
If you are consuming subscriptions from Python, [`risingwave-py`](/python-sdk/intro) provides a simpler exactly-once flow. Set `persist_progress=True` in `on_change()` and the SDK will automatically persist the last consumed `rw_timestamp` and resume from it after a restart.
...
Use the manual approach below if you are consuming subscriptions through a Postgres driver directly or need to manage progress storage yourself.
...
def update_progress(conn, sub_name, progress):
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO subscription_progress (sub_name, progress) VALUES (%s, %s);",
            (sub_name, progress),
        )
        # Ensure the progress is durable and visible for subsequent reads.
        cur.execute("FLUSH;")
        conn.commit()
```
...
with conn.cursor() as cur:
    ...
    while True:
        ...
        # 'rw_timestamp' is the progress indicator.
        # In the examples above, it's the last column of the returned row.
        last_progress = row[-1]
