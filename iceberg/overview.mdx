...
- **Hosted catalog:** RisingWave's built-in catalog service that eliminates the need to set up and manage an external catalog. Perfect for getting started quickly. See [Hosted Iceberg Catalog](/iceberg/hosted-iceberg-catalog).
- **External catalogs:** JDBC, AWS Glue, REST, and S3 Tables catalogs for integration with existing data infrastructure.

For complete instructions on setup, configuration options (like catalog types, commit intervals), features (like time travel), and limitations, see [Iceberg table engine](/iceberg/iceberg-table-engine).
...
*   [Use Amazon S3 Tables with RisingWave Iceberg](/iceberg/configuration/object-storage#amazon-s3-tables)
*   [Use S3 Tables with the Iceberg table engine](/iceberg/iceberg-table-engine#use-amazon-s3-tables-with-the-iceberg-table-engine)
