...
## Use Amazon S3 Tables
 ...
 This enables **Iceberg Source, Iceberg Sink, and Iceberg Table Engine** in RisingWave to connect with S3 Tables. Specifically, S3 Tables offer automatic compaction, so when using the Iceberg table engine with AWS S3 Tables, they provide an alternative solution until Iceberg compaction becomes available in future releases.
 
 The following examples show how to use S3 tables as the storage for Iceberg table engine, Iceberg source, and Iceberg sink. For details, see [Use S3 Tables for Iceberg source](/ingestion/sources/iceberg-config#use-amazon-s3-tables),[S3 tables for Iceberg sink](/integrations/destinations/apache-iceberg#use-amazon-s3-tables), and []().
