...
<Tabs>
  ...
  To write to a Databricks Unity Catalog table, set `catalog.type = 'rest'` with `vended_credentials = true` and provide your Unity Catalog REST endpoint and credentials.
  Note that currently, only `append-only` sinks are supported for this integration.
