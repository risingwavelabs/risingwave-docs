...
The deployment step above creates only the `analytics` table bucket. The `default.orders` table must already exist in the catalog before you create a source — seed it with a tool such as [PyIceberg](https://py.iceberg.apache.org/) or another Iceberg client writing to the same SeaweedFS S3 gateway.
...
Rows are committed to the Iceberg table at intervals controlled by `commit_checkpoint_interval` (default: about every 60 seconds), and other engines can read them through the same catalog.
