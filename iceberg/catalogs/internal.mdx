...
You create it once with `CREATE CONNECTION ...`, and then reuse it by name — either for the current session (`SET iceberg_engine_connection = 'schema.connection_name'`) or globally for all sessions (`ALTER SYSTEM SET iceberg_engine_connection = 'schema.connection_name'`).
</Note>
...
When you create `ENGINE = iceberg` tables, RisingWave needs to know **which Iceberg `CONNECTION` to use**. Set the connection before creating the table — either for the current session or globally for all sessions.

The connection name format is `schema_name.connection_name` (for example, `public.built_in_catalog_conn`).

```sql
-- Set the connection for the current session only
SET iceberg_engine_connection = 'public.built_in_catalog_conn';

-- Or set it globally for all sessions
ALTER SYSTEM SET iceberg_engine_connection = 'public.built_in_catalog_conn';
```
