...
You create it once with `CREATE CONNECTION ...`, and then reuse it by name by setting a session default: `SET iceberg_engine_connection = 'schema.connection_name'`
</Note>
...
When you create `ENGINE = iceberg` tables, RisingWave needs to know **which Iceberg `CONNECTION` to use**. Set a session default first, then create the table.
...
-- 2) Create an internal Iceberg table
