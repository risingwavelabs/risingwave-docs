...
For more information about the two catalog types, see [About the Apache Iceberg REST catalog endpoint](https://docs.cloud.google.com/lakehouse/docs/about-iceberg-rest-catalog-endpoint).

## Before you begin

Create a [single-bucket catalog in end-user credentials mode](https://docs.cloud.google.com/lakehouse/docs/create-catalog), and make a Google identity available to the RisingWave connector processes through ADC. The catalog ID and GCS bucket name must match.
...
`catalog.security = 'google'` uses ADC to authenticate catalog requests independently of `enable_config_load`. The `enable_config_load` setting controls environment/local credential loading for GCS file I/O. `gcs.credential` also configures only GCS file I/O and does not replace catalog ADC.
...
If ADC comes from `GOOGLE_APPLICATION_CREDENTIALS` or a local ADC file, add `enable_config_load = 'true'` to the statement's `WITH` clause.

For Iceberg source behavior and limitations, see [Ingest data from Iceberg tables](/iceberg/ingest-from-iceberg).

## Write to a table

Use an append-only upstream table or materialized view for the following sink. It writes to an existing Iceberg table or creates it if it does not exist:
