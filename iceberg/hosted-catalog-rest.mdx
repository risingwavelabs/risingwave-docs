---
title: "Hosted REST Catalog"
sidebarTitle: "REST"
description: "Deploy and use the REST hosted catalog, powered by Lakekeeper, to manage internal Iceberg tables in RisingWave."
---

This guide shows how to deploy RisingWave with Lakekeeper, a hosted REST catalog service. This setup enables you to create and manage internal Iceberg tables that use a REST-compliant catalog.
...
1. Create an Iceberg connection. You can authenticate using two methods: S3 access keys or credential vending.

   <Tabs>
   <Tab title="Using S3 access keys">
      For this method, simply provide your S3 access and secret keys in the connection string.
   ...
   <Tab title="Using Credential Vending (AWS STS Role)">
      For this method, you need to:
      1. Prepare an AWS S3 bucket and an IAM Role with permissions to access that S3 bucket.
      2. In Lakekeeper, add a warehouse by configuring the corresponding S3 bucket and enabling STS with the associated STS Role ARN.
         <Columns cols={3}>
            <Frame><img src="/images/lakekeeper_warehouses.png" /></Frame>
            <Frame><img src="/images/lakekeeper_add_warehouse.png" /></Frame>
            <Frame><img src="/images/lakekeeper_enable_sts.png" /></Frame>
         </Columns>
      3. Create the connection in RisingWave using the following configuration.
         ```sql Using credential vending
         CREATE CONNECTION lakekeeper_catalog_conn
         WITH (
            type = ‘iceberg’,
            catalog.type = ‘rest_rust’,
            catalog.uri = ‘http://lakekeeper:8181/catalog/’,
            catalog.header=‘X-Iceberg-Access-Delegation=vended-credentials’,
            warehouse.path = ‘<your_warehouse_name>’,
         );
         ```
   </Tab>
   </Tabs>
...
9. Connect to RisingWave and run the test queries.
   ...
   Next, create an Iceberg connection using one of the following methods: S3 access keys or credential vending.
   ...
   You can then run the SQL test queries:
