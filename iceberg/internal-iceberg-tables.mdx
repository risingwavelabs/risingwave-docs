...
One important difference: **new writes become queryable only after an Iceberg commit**. By default, Iceberg commits happen about every **60 seconds** (controlled by `commit_checkpoint_interval`), or earlier when the buffered write size exceeds `commit_checkpoint_size_threshold_mb` (default: `128` MB).
