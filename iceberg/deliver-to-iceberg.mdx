...
| `default_table_location_from_namespace` | No | For a REST catalog, set to `true` to derive the location of an automatically created table from the namespace's nonempty `location` property when no location can be derived from `warehouse.path`. RisingWave trims trailing slashes from the namespace location and appends the table name. A location derived from `warehouse.path` takes precedence; if the namespace location is missing or empty, server-side location assignment remains unchanged. Default: `false`. |
