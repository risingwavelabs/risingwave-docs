...
| `auto.schema.change` | No | Set to `true` to enable automatic schema evolution. When enabled, RisingWave automatically propagates schema changes (e.g., adding columns) from the upstream source to the Iceberg table. This feature works with exactly-once semantics (`is_exactly_once = 'true'`). Currently supports `ADD COLUMN` operations. Default: `false`. |
...
You can configure `commit_checkpoint_interval`, `commit_checkpoint_size_threshold_mb`, and `commit_retry_num` to manage commit frequency and retry behavior.

## Schema evolution

RisingWave supports automatic schema evolution for Iceberg sinks, allowing schema changes from upstream sources to be automatically propagated to the target Iceberg table. This feature works with exactly-once semantics.

To enable schema evolution, set `auto.schema.change = 'true'` when creating the sink:
...
2. Schema changes are committed to the Iceberg table atomically
3. The sink handles both data commits and schema commits, ensuring consistency
4. In exactly-once mode, RisingWave ensures that schema changes are applied exactly once, even in the presence of failures

Schema changes are committed separately from data changes but are coordinated to maintain consistency. The system checks whether schema changes have already been applied to avoid duplicate schema updates during recovery.
