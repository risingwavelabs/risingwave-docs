...
| `is_exactly_once`| No | If `true`, enables exactly-once delivery semantics. This provides stronger consistency but may impact performance. Default: `true`. <br/><br/>**Note**: Exactly-once delivery requires [sink decoupling](/delivery/overview#sink-decoupling) to be enabled (the default behavior). If you explicitly disable sink decoupling (`SET sink_decouple = false;`), exactly-once semantics will be automatically disabled for the sink.|
