...
| `ignore_delete`| No | If `true`, RisingWave drops `DELETE` messages and converts `UPDATE` messages to `INSERT` messages before writing to the sink. This is commonly used to force append-only output from a non-append-only upstream. Default: `false`. |
