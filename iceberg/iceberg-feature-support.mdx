...
| Per-table metadata relations | Supported | Query `<name>$snapshots`, `<name>$manifests`, and `<name>$files` for one Iceberg engine table, Iceberg source, or standalone Iceberg sink. |
...
| External catalogs | Supported | Supports storage, REST, Glue, JDBC, Hive, S3 Tables, Lakekeeper, SeaweedFS, Snowflake, and Databricks Unity Catalog with catalog-specific limits. |
...
| SeaweedFS | Supported | Self-hosted REST catalog and Parquet storage in one S3 gateway. Uses SigV4 signing. |
