...
You can enable automatic maintenance to run periodically in the background for your Iceberg sinks and internal tables. Automatic manifest rewrite is currently available for Iceberg sinks. Periodic manifest rewrite runs in the meta-node maintenance loop, controlled by `iceberg_gc_interval_sec` (default: `3600` seconds), rather than by the per-sink `compaction_interval_sec` parameter.
...
| Parameter | Description |
|:--|:--|
| `enable_manifest_rewrite` | Optional. Set to `true` to periodically rewrite fragmented data manifests for an Iceberg sink. Default: `false`. |
