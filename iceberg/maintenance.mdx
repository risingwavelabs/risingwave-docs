...
You can enable automatic maintenance to run periodically in the background for your Iceberg sinks and internal tables. Automatic manifest rewrite is currently available for Iceberg sinks.
...
`VACUUM` rewrites eligible manifests immediately and then expires snapshots. `VACUUM FULL` first compacts data files, then rewrites manifests, and finally expires snapshots.

```sql
-- Rewrite manifests and expire snapshots
VACUUM my_iceberg_table;

-- Compact files, then rewrite manifests and expire snapshots
