...
<Note>
`VACUUM FULL` waits for the manual compaction task to finish before returning. If the target already has a pending or in-flight Iceberg compaction task, RisingWave returns an error instead of scheduling another compaction task.
</Note>
