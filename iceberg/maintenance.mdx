...
| Parameter | Description |
|:--|:--|
| `enable_manifest_rewrite` | Optional. Set to `true` to periodically rewrite fragmented Iceberg manifests. Default: `false`. This option is supported in Iceberg sinks and in `CREATE TABLE ... ENGINE = iceberg` `WITH (...)` clauses. |
| `manifest_rewrite_target_size_bytes` | Optional. The target size in bytes for rewritten manifests. Must be greater than `0`. If not set, RisingWave uses the Iceberg default target manifest size. |
| `manifest_rewrite_min_count_to_merge` | Optional. The minimum number of small manifests required before RisingWave merges them. Must be greater than `0`. If not set, RisingWave uses the Iceberg default minimum merge count. |

<Note>
`enable_manifest_rewrite` is not supported when `format_version = 3`, because manifest rewriting cannot preserve row lineage for Iceberg v3 tables.
</Note>
...
#### Manifest rewrite for internal Iceberg tables

You can also enable manifest rewriting on internal Iceberg tables through the table `WITH` clause:
