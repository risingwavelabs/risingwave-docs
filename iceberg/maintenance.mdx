...
You can enable automatic maintenance to run periodically in the background for your Iceberg sinks and internal tables. Periodic manifest rewrite runs in the meta-node maintenance loop, controlled by `iceberg_gc_interval_sec` (default: `3600` seconds), rather than by the per-sink `compaction_interval_sec` parameter.
...
In addition to automatic background maintenance, you can trigger maintenance manually at any time using the [`VACUUM`](/sql/commands/sql-vacuum) command.

`VACUUM` immediately checks whether manifests are eligible for rewrite, and then checks for snapshots to expire. Manifest rewrite runs only when `enable_manifest_rewrite = true` and the configured rewrite thresholds select an eligible batch. Snapshot expiration runs only when `enable_snapshot_expiration = true`.

`VACUUM FULL` first compacts data files, then performs the same gated manifest rewrite and snapshot expiration checks. The manifest rewrite and snapshot expiration steps do not override their corresponding maintenance settings.

```sql
-- Check for eligible manifests to rewrite, then check for snapshots to expire
VACUUM my_iceberg_table;

-- Compact files, then run the same manifest rewrite and snapshot expiration checks
