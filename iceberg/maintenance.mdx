...
- **Manifest Rewrite**: Rewrites fragmented Iceberg manifests to reduce metadata overhead.
...
| `enable_manifest_rewrite` | Optional. Periodically rewrites fragmented Iceberg data manifests. For newly created internal Iceberg tables using Iceberg format V2, RisingWave enables this by default unless you explicitly set it to `false`. External Iceberg sinks remain disabled by default. |
...
| `manifest_rewrite_target_size_bytes` | Optional. The target size in bytes for rewritten Iceberg data manifests. |
| `manifest_rewrite_min_count_to_merge` | Optional. The minimum number of manifests required to rewrite an under-filled batch. |
...
```sql
-- Expire old snapshots for a table
VACUUM my_iceberg_table;

-- Compact files, then expire old snapshots
VACUUM FULL my_iceberg_table;
```

<Note>
`VACUUM FULL` waits for the manual compaction task to finish before returning. If the target already has an automatic or manual compaction task waiting to be dispatched or running, the new `VACUUM FULL` request fails instead of scheduling another task.
</Note>
