...
You can enable automatic maintenance to run periodically in the background for your Iceberg sinks and internal tables. Periodic manifest rewrite runs in the meta-node maintenance loop, rather than using the per-sink `compaction_interval_sec` parameter. Configure the interval with `iceberg_gc_interval_sec` in the `[meta]` section of `risingwave.toml` (default: `3600` seconds).
