...
| `snapshot_expiration_max_age_millis` | Optional. The maximum age (in milliseconds) for a snapshot to be retained. To keep only the latest snapshot, set this to `0`. Default: `3600000` (1 hour). |
| `snapshot_expiration_retain_last` | Optional. The minimum number of snapshots to retain, regardless of their age. Default: `12`. |
