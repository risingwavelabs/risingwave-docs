...
description: "Learn how to use the Iceberg sink in RisingWave to stream data into your existing Iceberg tables."
---

This guide explains how to write processed data from RisingWave into existing Iceberg tables. Use this when you have Iceberg tables managed by external systems and want RisingWave to deliver processed results into them.
...
| `type` | Sink mode. Use `'append-only'` to add new records to the target table, suitable for event logs. Use `'upsert'` to handle updates and deletes based on a `primary_key`. |
...
| `force_append_only` | When set to `'true'`, converts an upstream `upsert` stream into an `append-only` stream for the sink. Updates are written as new rows and deletes are ignored. | `false` |
...
- **Ingest from Iceberg**: Set up sources with [Ingest from Iceberg](/iceberg/ingest-from-iceberg).
