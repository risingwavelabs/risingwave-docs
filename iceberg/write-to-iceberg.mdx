...
sidebarTitle: "Write to tables (sinks)"
description: "Learn how to use the Iceberg sink in RisingWave to stream data into your existing Iceberg tables."
---

This guide explains how to stream processed data from RisingWave into existing Iceberg tables. Use this when you have Iceberg tables managed by external systems and want RisingWave to write processed results into them.

To write data to an Iceberg table, you create an Iceberg sink. This involves two main steps:
...
To configure Iceberg compaction when creating a sink, specify the following parameters in the `WITH` clause:
...
- **Read from Iceberg**: Set up sources with [Read from Iceberg](/iceberg/read-from-iceberg).
