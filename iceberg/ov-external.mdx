...
| Periodic loading | Supported (experimental, v2.7+) | Supported (experimental, v2.7+) |
| Continuous loading | Supported | No support (Iceberg CDC is immature) |

<Note>
Periodic loading with `refresh_mode = 'FULL_RELOAD'` is an experimental feature. For details, see [Ingest data from Iceberg tables](/iceberg/ingest-from-iceberg).
</Note>
