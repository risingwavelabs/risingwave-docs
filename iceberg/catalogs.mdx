...
| `catalog.header` | Custom HTTP headers for REST catalog requests, formatted as `key1=value1;key2=value2`. For Google Cloud Lakehouse Iceberg REST catalogs, set `catalog.header = 'x-goog-user-project=PROJECT_ID'` to specify the billing project. |
| `catalog.io_impl` | Custom Iceberg `FileIO` implementation class. For example, use `org.apache.iceberg.aws.s3.S3FileIO` for Amazon S3, `org.apache.iceberg.gcp.gcs.GCSFileIO` for Google Cloud Storage, or `org.apache.iceberg.azure.adlsv2.ADLSFileIO` for Azure Data Lake Storage Gen2. Google Cloud Lakehouse Iceberg REST catalogs with credential vending require `org.apache.iceberg.gcp.gcs.GCSFileIO`. |
...
| `vended_credentials` | Enable vended credentials for a REST catalog. When set to `true` with `catalog.type = 'rest'`, RisingWave requests credentials from the REST catalog server instead of managing them directly. For Google Cloud Lakehouse Iceberg REST catalogs, this sends `X-Iceberg-Access-Delegation: vended-credentials`. Default: `false`. |
