...
You can ingest data from Iceberg tables using three approaches:
...
2. **[Manual refresh](#manual-refresh-with-create-table)**: Create an Iceberg table with `refresh_mode = 'FULL_RECOMPUTE'` for on-demand full table refreshes. Note that you must use `CREATE TABLE` (not `CREATE SOURCE`), and data will only be loaded after you manually trigger a refresh using `REFRESH TABLE`.
3. **[Periodic full reload](#periodic-full-reload-with-create-table)**: Create an Iceberg table with `refresh_mode = 'FULL_RELOAD'` for scheduled full table refreshes. Note that you must use `CREATE TABLE` (not `CREATE SOURCE`), and data will only be loaded after you trigger a refresh—either manually or via the configured schedule.
...
## Manual refresh with CREATE TABLE

<Note>
Added in v2.7.0. It is currently in **[technical preview](/changelog/product-lifecycle#product-release-lifecycle)** stage.
</Note>

For batch-style workloads where you need to manually reload an entire Iceberg table on demand, you can create a table with `refresh_mode = 'FULL_RECOMPUTE'`. This mode is useful when:

- The external Iceberg table supports mutable data (updates and deletes).
- You need explicit control over when data is synced from Iceberg.
...
CREATE TABLE iceberg_manual_table (
    ...
    s3.region = 'us-west-2',
    refresh_mode = 'FULL_RECOMPUTE'     -- Required for manual refresh
);
```
...
|:----------|:------------|:---------|:--------|
| `refresh_mode` | Must be set to `'FULL_RECOMPUTE'` to enable manual refresh functionality | Yes | `'FULL_RECOMPUTE'` |

When `refresh_mode = 'FULL_RECOMPUTE'` is set, the table will only be populated when you explicitly execute `REFRESH TABLE`. The refresh operation pulls the latest Iceberg snapshot and applies any delete files (PositionDeletes and EqualityDeletes) to ensure query results match the latest table state in Iceberg.

### Trigger a refresh

To load or reload data from the external Iceberg table, use the `REFRESH TABLE` command:

```sql
REFRESH TABLE iceberg_manual_table;
```

This command pulls the latest Iceberg snapshot and applies all delete files, ensuring the table reflects the current state in Iceberg.
...
WHERE source_name = '__iceberg_source_iceberg_manual_table';
```
