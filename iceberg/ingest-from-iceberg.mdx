...
### Inspect Iceberg metadata through per-table relations

Append a metadata suffix to the Iceberg source name to inspect metadata for that table:

```sql
-- View all snapshots
SELECT * FROM "my_iceberg_source$snapshots";

-- View manifests for the current snapshot
SELECT * FROM "my_iceberg_source$manifests";

-- View live data and delete files for the current snapshot
SELECT * FROM "my_iceberg_source$files";
```

The `$manifests` and `$files` relations support time travel by timestamp or snapshot ID. The `$snapshots` relation does not support time travel.
...
These metadata relations are also available for Iceberg engine tables and standalone Iceberg sinks. For details, see [Iceberg feature support](/iceberg/iceberg-feature-support#per-table-metadata-relations).
