...
When reading from Iceberg, `uuid` and `fixed[L]` columns are exposed as `BYTEA` in RisingWave. For the full mapping, see the [Data type mapping guide](/iceberg/data-types).
