...
| idle_in_transaction_session_timeout | 60000 | Terminate any session that has been idle (waiting for a client query) within an open transaction for longer than the specified amount of time in milliseconds. The default value is 60 seconds (60,000 milliseconds). |
