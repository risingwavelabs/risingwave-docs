...
You can monitor the progress of background DDL operations using the [SHOW JOBS](/docs/current/sql-show-jobs/) command.

### Cancel jobs

Running jobs in the background can be canceled using the [CANCEL JOBS](/docs/current/sql-cancel-jobs/) command followed by the job ID.
