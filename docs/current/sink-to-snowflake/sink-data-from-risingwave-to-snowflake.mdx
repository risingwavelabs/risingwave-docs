...
| s3.path               | Optional. The S3 path to be specified. If specified, the actual file location would be \<s3\_bucket>://\<s3\_path>/\<rw\_auto\_gen\_file\_name>. Otherwise, it would be \<s3\_bucket>://\<rw\_auto\_gen\_file\_name>. |
