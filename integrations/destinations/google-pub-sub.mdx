...
| pubsub.endpoint       | **Required**. The Pub/Sub endpoint. For real Google Cloud usage, set this to `pubsub.googleapis.com:443`. For a local emulator, set this to the emulator host and port (e.g., `localhost:8900`). Do not include a scheme prefix (`https://`) or angle brackets.                                                                                                                                                                                                                                                                                                       |
...
You can test the connector locally before deploying to Google Cloud. See [Test locally with the Pub/Sub emulator](https://cloud.google.com/functions/docs/local-development).
...
- `pubsub.endpoint` is set to `pubsub.googleapis.com:443` for real GCP (no `https://` prefix, no angle brackets).
...
| Transport error / connection refused | `pubsub.endpoint` contains an `https://` prefix, angle brackets, or another unsupported format. | Set `pubsub.endpoint` to the plain host-and-port value `pubsub.googleapis.com:443`. |
