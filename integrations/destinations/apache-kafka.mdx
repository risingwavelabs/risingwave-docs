...
| ignore\_delete            | **Optional**. When set to `true`, prevents DELETE operations from being sent to the sink. INSERT and UPDATE operations are processed normally. Supported for `PLAIN` (append-only) and `UPSERT` sinks, but not for `DEBEZIUM` (retract) sinks. The default value is `false`. For backward compatibility, `force_append_only` can be used as an alias. |
...
- `ignore_delete='true'` must be specified
