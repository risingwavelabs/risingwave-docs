...
<Note>
RisingWave introduced the native Postgres sink connector in version 2.2, and the JDBC sink connector for Postgres will be deprecated in a future release. Existing JDBC sinks will be maintain in a backwards compatible way. There are some known issues for native Postgres Sink Connector tracked here: https://github.com/risingwavelabs/risingwave/issues/24183. If you encounter these, it's recommended you use JDBC sink first. You can try Postgres Native sink in-place for your JDBC sinks by setting `stream_switch_jdbc_pg_to_native = true` under `[streaming.developer]`.
</Note>
