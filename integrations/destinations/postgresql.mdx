...
| connector           | Sink connector should be `jdbc`. To switch from `jdbc` to `postgres`, set `switch_jdbc_pg_to_native = true` under `[streaming.developer]`. |
