...
Auto schema change works best when paired with CDC sources that also support schema evolution, such as [MySQL CDC](/ingestion/sources/mysql/mysql-cdc#automatic-schema-changes) or [PostgreSQL CDC](/ingestion/sources/postgresql-cdc#automatic-schema-changes). This enables end-to-end schema evolution from source to sink.
</Note>
...
4. **Intermediate Table Updates**: For upsert sinks, both target and intermediate CDC tables are updated with new columns
5. **Processing Continuity**: Schema changes don't interrupt ongoing data processing
