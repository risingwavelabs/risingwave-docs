...
| password                      | Snowflake password (required when using password authentication) |
...
For integration with secret management systems (like AWS Secrets Manager, HashiCorp Vault, etc.), you can provide the PEM-formatted private key content directly. This is the recommended approach for production environments:
...
MIIEv...your_private_key_content_here...
-----END PRIVATE KEY-----',
    ...
);
```
...
<Note>
To set up key-pair authentication in Snowflake, you need to:
...
For detailed instructions, see [Snowflake's key-pair authentication documentation](https://docs.snowflake.com/en/user-guide/key-pair-auth).
</Note>
...
1. Snowflake v2 sink with S3 writer (Append-only mode) using password authentication
...
CREATE SINK snowflake_sink FROM test_table WITH (
    ...
    -- Snowflake authentication: password (default)
...
2. Snowflake v2 sink with S3 writer (Append-only mode) using key-pair authentication
...
CREATE SINK snowflake_sink FROM test_table WITH (
    ...
    -- Snowflake authentication: key-pair with PEM content
    jdbc.url = 'jdbc:snowflake://...',
    username = '...',
    auth.method = 'key_pair_object',
    private_key_pem = '-----BEGIN PRIVATE KEY-----
...
3. Snowflake v2 sink with S3 writer (Upsert mode):
