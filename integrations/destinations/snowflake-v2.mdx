...
For integration with [secret management](/operate/manage-secrets) systems, you can provide the PEM-formatted private key content directly. This is the recommended approach for production environments:

First, create a secret containing your private key:

```sql
CREATE SECRET snowflake_pk AS '-----BEGIN PRIVATE KEY-----
...
-----END PRIVATE KEY-----';
```

Then, reference the secret in your sink definition:
...
CREATE SINK snowflake_sink FROM mv_name WITH (
  ...
  private_key_pem = secret snowflake_pk,
  ...
);
```
