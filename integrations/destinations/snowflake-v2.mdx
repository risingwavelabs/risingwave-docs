...
| warehouse                     | Snowflake warehouse name for the merge task in `upsert` mode. Do not set this when `task.serverless = true`. |
| task.serverless               | Creates the Snowflake merge task as a serverless task in `upsert` mode. When set to `true`, omit `warehouse`. |
| task.target_completion_interval | Optional `TARGET_COMPLETION_INTERVAL` value for a serverless Snowflake merge task. Requires `task.serverless = true`. |
...
| Parameter                     | Description |
|:------------------------------|:------------|
| auth.method                   | Authentication method: `password` (default), `key_pair_file`, or `key_pair_object`. If not specified, the method is inferred from the provided credentials |
| private_key_file              | Path to the private key file for key-pair authentication (required when `auth.method = 'key_pair_file'`) |
| private_key_file_pwd          | Password for encrypted private key file (optional, used with `private_key_file`) |
| private_key_pem               | PEM-formatted private key content for key-pair authentication (required when `auth.method = 'key_pair_object'`). This is the recommended method for secret management systems |
...
CREATE SINK snowflake_sink FROM mv_name WITH (
    ...
    private_key_file_pwd = 'your_key_passphrase',  -- Optional, only if key is encrypted
    ...
);
```
...
Alternatively, you can store the private key securely using [`CREATE SECRET`](/operate/manage-secrets) and reference the secret in the sink configuration.
...
- The merge task can run either on a user-managed warehouse (`warehouse`) or as a Snowflake serverless task (`task.serverless = true`). If you use a serverless task, you can optionally set `task.target_completion_interval`.
...
Before connecting RisingWave to Snowflake, you need to prepare a dedicated user, role, and schema in Snowflake. For `upsert` sinks, also prepare either a warehouse for the merge task or use Snowflake serverless tasks with `task.serverless = true`. If you already have a Snowflake user with the necessary privileges, you can skip this step.

Follow the steps below to create and configure the required resources.

1. Set `database_name`, `warehouse_name`, and `rw_schema` accordingly to use an existing database, warehouse, and/or schema. If you plan to use `task.serverless = true`, you can skip `warehouse_name` and the warehouse setup steps below.
...
2. Configure the following additional trusted entities for your IAM Role to allow RisingWave to assume the role. Obtain the **Workload Identity (IAM Role ARN)** from the [Cloud metadata](/cloud/cloud-metadata) page (**Connection** → **Cloud Meta** tab). See [Set up IAM role assume](/cloud/iam-role-assume) for a complete walkthrough.
