...
Starting in v2.5, RisingWave does not create NATS streams automatically by default (`allow_create_stream = false`). This is a behavior change from previous versions that prevents accidental stream creation in production environments due to typos or misconfigurations. 
