...
<Note>
**Stream Creation Security**

By default, RisingWave will not create NATS streams automatically (`allow_create_stream = false`). This prevents accidental stream creation in production environments due to typos or misconfigurations. 

To enable automatic stream creation, explicitly set `allow_create_stream = 'true'`. Only enable this in development environments or when you specifically want RisingWave to manage stream creation.
</Note>
...
| allow\_create\_stream | **Optional**. Whether to allow RisingWave to create the NATS stream if it doesn't exist. Default: `false`. When `false`, RisingWave will return an error if the specified stream doesn't exist. When `true`, RisingWave will create the stream if it doesn't exist. For production environments, it's recommended to keep this as `false` to prevent accidental stream creation. |
