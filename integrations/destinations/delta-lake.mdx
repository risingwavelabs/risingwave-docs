...
| is\_exactly\_once            | **Optional**. Set to `true` to enable exactly-once coordination for Delta Lake sinks. RisingWave uses the Delta transaction log to deduplicate replayed epoch commits so recovery does not append duplicate files. Default: `false`.                                                                                                                                                                                                                   |
...
To enable exactly-once coordination for the sink, add `is_exactly_once = 'true'`.
