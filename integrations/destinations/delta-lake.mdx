...
[Delta Lake](https://delta.io) is an open-source storage framework that enables building a lakehouse architecture with compute engines including Spark, PrestoDB, Flink, Trino, and others. It provides ACID transactions, scalable metadata handling, and unifies streaming and batch data processing.

The Delta Lake sink connector provides:
- **Real-time data delivery**: Stream data changes directly to Delta Lake tables
- **Append-only operations**: Optimized for high-throughput append operations
- **Multi-storage support**: Works with S3, GCS, Azure Blob, and local file systems
- **Schema validation**: Automatic validation of RisingWave and Delta Lake schema compatibility
- **Checkpoint-based committing**: Configurable commit intervals for optimal performance
- **Coordinated sink**: Ensures consistency with coordinated commit mechanism
- **Secret management**: Secure credential handling through RisingWave secrets
...
* The Delta Lake table must exist before creating the sink (auto-creation is not supported).
* Appropriate storage credentials are configured (AWS, GCS, Azure, etc.).
...
The following query creates a source using the built-in load generator, which creates mock data. For more details, see [CREATE SOURCE](/sql/commands/sql-create-source) and [Generate test data](/ingestion/sources/datagen). You can transform the data using additional SQL queries if needed.
