...
* `changelog_op`: the change type (`1` = INSERT, `2` = UPDATE-BEFORE, `3` = UPDATE-AFTER, `4` = DELETE)  
...
1. **Merge on read**: run an explicit `MERGE` (or similar SQL) that keeps the newest `_changelog_row_id` per primary key.  
...
4. Approach A: Use `MERGE INTO` on reads

```sql
-- 1. Create a table to store the results
CREATE TABLE your_table_name(xxx, xxx);

-- 2. Create a table to store the offset.
CREATE TABLE offset_table (table_name string, offset int); 
...
BEGIN
    ...
    -- 2. Query the offset from the table
    SELECT offset INTO :my_offset 
    FROM t3 
    WHERE table_name = 't3'
    LIMIT 1;

    -- 3. Use MERGE INTO statement for aggregation
    MERGE INTO user_behaviors AS target
    USING (
        SELECT *
        FROM (
            SELECT *, 
                   ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY __row_id DESC) AS dedupe_id
            FROM t3
            WHERE __row_id > :my_offset
        ) AS subquery
        WHERE dedupe_id = 1
    ) AS source
    ON target.user_id = source.user_id
    WHEN MATCHED AND source.__op IN (2, 4) THEN DELETE
    WHEN MATCHED AND source.__op IN (1, 3) THEN UPDATE SET
    target.col1 = source.col1,
    target.col2 = source.col2
    WHEN NOT MATCHED AND source.__op IN (1, 3) THEN INSERT (col1, col2, col3) VALUES (source.col1, source.col2, source.col3);
...
4. Approach B: Create a dynamic table that materializes the upsert view in Snowflake
...
FROM (
    ...
    FROM t3              -- t3 is the table populated by the sink
) AS sub
WHERE dedupe_id = 1
