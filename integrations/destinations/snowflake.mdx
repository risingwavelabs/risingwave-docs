...
4. Approach A: Use `MERGE INTO` on reads. With this approach, we need an intermediate table (cdc_table), a task(multi_statement_task), an offset storage table (offset_table) and a table to store the results (target_table).
...
BEGIN
    ...
    -- 2. Use MERGE INTO statement for aggregation
