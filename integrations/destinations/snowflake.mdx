...
With this approach, we need three tables in Snowflake. Their purposes are as follows:

- **target table:** stores the final data.
- **CDC table:** holds intermediate data during processing.
- **offset table:** keeps track of the offsets for each cdc table. If you have multiple cdc tables, they can share the same offset table.
...
4. Approach B: Create a dynamic table that materializes the upsert view in Snowflake. We need an intermediate table (cdc_table) and a dynamic table to store the results (target_table).
