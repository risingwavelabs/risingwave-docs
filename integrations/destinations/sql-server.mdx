...
| sqlserver.schema    | **Optional**. The SQL Server schema name. If not specified, the default schema `dbo` is used.                                         |
...
| bigint / int / smallint / tinyint | timestamp with time zone |
| No support      | interval                    |
| No support      | struct                      |
| No support      | array                       |
| varbinary       | bytea                       |
