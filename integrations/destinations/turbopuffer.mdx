...
| `num_shards` | **Optional**. Number of shards to configure for the Turbopuffer namespace. Must be greater than `0`. Set this only when writing to a new namespace, because Turbopuffer does not allow sharding to be added or changed after the namespace is created. |
...
* If you set `num_shards`, configure it before the target namespace receives its first write.
