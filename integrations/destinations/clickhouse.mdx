...
If you have an upsert source and want to create an append-only sink, set `type = append-only` and `ignore_delete = true`. This will ignore delete messages in the upstream, and turn upstream update messages into insert messages.
