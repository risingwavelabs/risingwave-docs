...
|  redis_value_type     | Optional. Controls how data is written to Redis. <ul><li>`string` (default): Key-value storage. Required parameters are `key_format` and `value_format`.</li><li>`pubsub`: Publishes messages to Redis Pub/Sub channels. </li><li>`geospatial`: Stores data using Redis' geospatial index.</li></ul>                                                                                                 |
