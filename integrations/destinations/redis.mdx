...
|  redis_value_type     | Optional. If set to `pubsub`, data will be published to a Redis Pub/Sub channel instead of being written as key-value pairs. This requires `FORMAT UPSERT ENCODE TEMPLATE`. For more information, see [example](#sinking-data-to-redis-pub-sub) below.                                                                                                        |
|  channel     | Optional. The name of the Redis Pub/Sub channel to publish messages to. Required if `redis_value_type = 'pubsub'` and `channel_column` is not set.                                                                                                        |
|  channel_column     | Optional. The values from this column will be used as the names of the Redis Pub/Sub channels to publish messages to. Required if `redis_value_type = 'pubsub'` and `channel` is not set. The column type must be `VARCHAR`.                                                                                                        |

## Example

This section provides examples for sinking data from RisingWave to Redis, covering both key-value storage and Pub/Sub messaging.

### Sinking data as key-value pairs

This approach is suitable when you want to use Redis as a fast data store or cache, storing data as key-value pairs.
...
### Sinking data to Redis Pub/Sub

This approach allows RisingWave to publish messages to Redis channels. Make sure to use `FORMAT UPSERT ENCODE TEMPLATE` and specify `redis_value_type = 'pubsub'`. At least one of `channel` or `channel_column` must also be specified.
