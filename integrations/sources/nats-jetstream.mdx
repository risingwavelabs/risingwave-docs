...
| consumer.ack\_policy             | **Optional**. Acknowledgment policy for message processing (e.g., None, All, Explicit). Default: `explicit`.                                                                                                                                                                                                                                                                     |
...
Starting in v2.5, RisingWave does not create NATS streams automatically by default (`allow_create_stream = false`). This prevents accidental stream creation in production environments due to typos or misconfigurations. 
