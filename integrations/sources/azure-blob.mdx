...
| match\_pattern                   | **Conditional**. Set to find object keys in `azblob.container_name` that match the given pattern. Standard Unix-style [glob](https://en.wikipedia.org/wiki/Glob%5F%28programming%29) syntax is supported. Typical usage follows the `prefix/*.suffix` form; for example, `your_directory/*.parquet` matches all Parquet files under `your_directory/`. If no `/` appears in `match_pattern`, the scan is performed from the container root. |
