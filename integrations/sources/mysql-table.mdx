---
title: "Query MySQL tables (non-CDC)"
description: "Fetch data from MySQL tables on demand via the mysql_query table-valued function (non-CDC)."
sidebarTitle: MySQL tables (non-CDC)
---
...
<Note>
**Not CDC.** This approach fetches rows on demand via a table-valued function. For continuous change replication, use [MySQL CDC](/ingestion/sources/mysql/mysql-cdc).
</Note>
...
Define `mysql_query` in either of the following forms:
...
```sql
mysql_query(
    cdc_source_name varchar, -- Existing MySQL CDC source name
    query varchar            -- SQL query to execute
)
```

The source-based form requires an existing [MySQL CDC](/ingestion/sources/mysql/mysql-cdc) source. It reuses the connection properties already defined on that source, including credentials managed as secrets. Added in v2.3.0.

The `cdc_source_name` argument accepts either a bare identifier (resolved against the session `search_path`) or a schema-qualified name (for example `'my_schema.my_cdc_source'` or `'"my_schema"."my_cdc_source"'`). Use the schema-qualified form when the CDC source lives outside the default `search_path`.
...
If you already have a MySQL CDC source, you can reuse its connection properties instead of passing them inline again.
...
-- Bare name, resolved against the session search_path
SELECT *
FROM mysql_query('mysql_source', 'select * from test;');

-- Schema-qualified reference (required when the CDC source lives outside the search_path)
SELECT *
FROM mysql_query('my_schema.mysql_source', 'select * from test;');
```
