...
| :--------- | :-----------|
| `data JSONB` | Stores the full JSON payload from the webhook. This example uses a single `JSONB` column, but webhook tables can also use typed multi-column schemas. |
