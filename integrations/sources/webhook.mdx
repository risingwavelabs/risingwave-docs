...
The same `webhook` table can also accept WebSocket ingest at `ws://<HOST>:4560/ingest/<database>/<schema>/<table>`. Each DML batch is sent as one WebSocket text frame, so keep each batch below 8 MiB to stay under the managed Cloud 10 MiB limit and the typical self-hosted frontend 16 MiB limit. Delivery is at least once: if the connection closes before you receive an ack, reconnect and replay every batch sent after the last acked `dml_batch_id`. For managed RisingWave Cloud routing and protocol details, see [Ingest data from WebSocket](/cloud/ingest-data-from-websocket).
