...
RisingWave can serve as a webhook destination, directly accepting HTTP requests from external services and storing the incoming data in its tables. By default, the Compute Node listens for webhook events on port `4560`. You can reconfigure this depending on how your cluster is deployed. When a webhook is triggered, RisingWave processes and ingests the data in real-time.
...
If `RW_SSL_CERT` and `RW_SSL_KEY` are configured, the same endpoint is served over HTTPS instead:

`https://<HOST>:4560/webhook/<database>/<schema>/<table>`

The default listening port is `4560`. Replace `<schema>` with `public` if you didn't create the table under a custom schema.

<Note>
If your cluster already sets `RW_SSL_CERT` and `RW_SSL_KEY`, existing webhook senders must switch from HTTP to HTTPS after upgrading to a version that includes this feature.
</Note>

### Choose a table shape

Webhook tables support two patterns:

- A legacy single-`JSONB` column that stores the full request body.
- A typed multi-column table that maps top-level JSON fields directly into RisingWave columns.

Use a single `JSONB` column when you want to keep the raw payload and extract fields later in SQL:
...
Use typed columns when you want RisingWave to decode the JSON payload into normal table columns directly:
...
CREATE TABLE orders (
  ...
  'sha1=' || encode(hmac('TEST_WEBHOOK', payload, 'sha1'), 'hex')
);
```

For typed multi-column webhook tables:

- Each top-level JSON field is decoded into the matching column by name.
- Missing non-primary-key fields are inserted as `NULL`.
- Missing primary-key fields are rejected.
- For batched requests, set `is_batched = true` and send newline-delimited JSON objects.
...
For providers that sign payloads (for example, GitHub/Segment), the second argument of `secure_compare(...)` is typically an expression that computes the expected signature from the secret/raw string and the request body.

- For single-`JSONB` webhook tables, reference the `JSONB` payload column name (for example, `data`).
- For typed multi-column webhook tables, reference the reserved `payload` identifier, which exposes the raw request body bytes during validation.

See the provider-specific guides below for copy/paste-ready examples.
