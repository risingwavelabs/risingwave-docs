...
The same `webhook` table can also accept WebSocket ingest at `ws://<HOST>:4560/ingest/<database>/<schema>/<table>`. WebSocket ingest is a premium feature and requires a license key on self-managed RisingWave deployments. Each DML batch is a single WebSocket text frame. Keep each batch below 8 MiB to stay under the managed Cloud 10 MiB limit (RWProxy default) and the self-hosted frontend 16 MiB limit (the `tungstenite` library default, not overridden by RisingWave). Delivery is at least once: if the connection closes before you receive an ack, reconnect and replay every batch sent after the last acked `dml_batch_id`. For managed RisingWave Cloud routing and protocol details, see [Ingest data from WebSocket](/cloud/ingest-data-from-websocket).
