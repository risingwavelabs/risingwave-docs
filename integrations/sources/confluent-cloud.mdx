...
3. Add a new **service account** API key for the cluster you just created, and set up the ACL to allow READ operation for consumer groups with `rw-consumer` (aligned with `group.id.prefix` in the WITH clause).
