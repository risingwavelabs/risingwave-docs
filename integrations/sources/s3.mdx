...
| `enable_config_load`   | **Optional**. Set to `true` if you need to load warehouse credentials from environment variables. You must also set `DISABLE_DEFAULT_CREDENTIAL=false` as an environment variable on the Meta Node and Streaming Nodes.                                            |
