...
| `enable_config_load`   | **Optional**. Set to `true` to load warehouse credentials from environment variables. This is for self-hosted environments only. You must also set `DISABLE_DEFAULT_CREDENTIAL=false` as an environment variable on the Meta Node and Streaming Nodes.                                            |
