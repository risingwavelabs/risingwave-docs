...
| match\_pattern        | **Conditional**. This field is used to find object keys in `s3.bucket_name` that match the given pattern. Standard Unix-style [glob](https://en.wikipedia.org/wiki/Glob%5F%28programming%29) syntax is supported. Typical usage follows the `prefix/*.suffix` form; for example, `your_directory/*.parquet` matches all Parquet files under `your_directory/`. If no `/` appears in `match_pattern`, the scan is performed from the bucket root. |
