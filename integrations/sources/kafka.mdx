...
<Note>
**Kafka consumer group cleanup visibility**

During source enumerator cleanup, RisingWave deletes the Kafka consumer groups derived from `group.id.prefix`. Cleanup remains best-effort, but failures are now surfaced in warning logs and in the Prometheus metric `source_kafka_consumer_group_delete_failure_count{source_id,consumer_group}`. If you use the Grafana dev dashboard, check **Kafka Metrics** > **Kafka Consumer Group Delete Failures** to see the failure rate grouped by source and consumer group.
</Note>
