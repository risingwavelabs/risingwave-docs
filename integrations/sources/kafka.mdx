...
Confluent's Protobuf serializer and converter can treat a broader set of imports as known types and skip registering them as references. If your schema imports any of these files, set `skip.known.types = false` on the upstream Confluent serializer or converter so Schema Registry stores the dependencies as references. For the current known-types list, see Confluent's [`ProtobufSchema.java`](https://github.com/confluentinc/schema-registry/blob/v8.4.0-124/protobuf-provider/src/main/java/io/confluent/kafka/schemaregistry/protobuf/ProtobufSchema.java#L271).

If you cannot register the imports as Schema Registry references, use [`schema.location`](#read-schemas-from-locations) with a self-contained `FileDescriptorSet` generated by `protoc --include_imports`.
</Note>
