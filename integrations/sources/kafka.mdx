...
During source enumerator cleanup, RisingWave attempts to delete the generated Kafka consumer groups. If the delete request fails for the whole request or for individual groups, RisingWave keeps cleanup best-effort and surfaces the failure through warning logs and the Prometheus metric `source_kafka_consumer_group_delete_failure_count{source_id,consumer_group}`.
