...
| backfill.num\_rows\_per\_split | **Optional**. Specifies number of rows per split for parallelized CDC backfill. Only effective when `backfill.parallelism` > 0. The default value is 100,000.                                                                                                          |
| backfill.as\_even\_splits | **Optional**. Whether to create even splits for parallelized CDC backfill. Only effective when `backfill.parallelism` > 0. The default value is true.                                                                                                          |
...
You can also create a CDC table with parallelized backfill enabled for better performance on large tables:
