...
- RisingWave does not currently provide geospatial functions for this `BYTEA` payload. For example, `ST_AsEWKB(locationpoint)` fails because the function expects a geometry type, not `BYTEA`.

To inspect and validate the ingested bytes, encode them to text in RisingWave:
...
For downstream systems (such as Snowflake or PostGIS), treat this value as EWKB/WKB and reconstruct the geometry there. A common pattern is to export the hex/base64-encoded value, convert it back to binary in the destination system, and then apply that system's geometry-construction function.
