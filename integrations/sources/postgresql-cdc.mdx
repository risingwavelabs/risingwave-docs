...
If the upstream PostgreSQL schema or table name uses quoted identifiers, preserve the PostgreSQL double quotes inside the string literal. For example, use `TABLE 'public."QuotedTable"'`.
...
If the upstream PostgreSQL table name is quoted, keep that quoting in the `TABLE` value. For example, the following statement ingests from `public."QuotedNote"`:
...
CREATE TABLE quoted_note (
    ...
    "authorId" text,
    content text
) FROM pg_mydb TABLE 'public."QuotedNote"';
```
...
| GEOMETRY (PostGIS)                                   | BYTEA (stored as raw EWKB bytes)                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| LTREE                                                | No support                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
### PostGIS `GEOMETRY` semantics

When you ingest PostgreSQL CDC data from PostGIS, `GEOMETRY` values are preserved as raw EWKB bytes and stored in RisingWave as `BYTEA`.

- The value may look unreadable in SQL clients. This is expected for raw binary data and does not mean the geometry is corrupted.
- RisingWave does not provide PostGIS geospatial functions for this ingested `BYTEA` value.

To inspect and validate the ingested bytes, encode them to text in RisingWave (replace `locationpoint` with your geometry column name and `cdc_table` with your CDC table name):
```sql
SELECT
  encode(locationpoint, 'hex') AS locationpoint_hex,
  encode(locationpoint, 'base64') AS locationpoint_base64
FROM cdc_table;
```

For end-to-end verification, compare RisingWave and PostgreSQL outputs:

```sql
-- In PostgreSQL (upstream)
SELECT encode(ST_AsEWKB(locationpoint), 'hex') AS locationpoint_hex
FROM source_table;

-- In RisingWave (downstream)
SELECT encode(locationpoint, 'hex') AS locationpoint_hex
FROM cdc_table;
```

For downstream systems (such as Snowflake or PostGIS), treat this value as EWKB/WKB and reconstruct the geometry there. Export `locationpoint_hex` with a RisingWave sink connector or an ETL job. (If your connector supports binary columns directly, you can export the raw `BYTEA` payload instead.) After it lands in a downstream table (for example, `cdc_exported_rows`), you can reconstruct geometry as follows:

```sql
-- Snowflake pattern: convert exported hex back to binary, then reconstruct geometry
SELECT TO_GEOMETRY(TO_BINARY(locationpoint_hex, 'HEX')) AS location_geom
FROM cdc_exported_rows;

-- PostGIS pattern
SELECT ST_GeomFromEWKB(decode(locationpoint_hex, 'hex')) AS location_geom
FROM cdc_exported_rows;
```
