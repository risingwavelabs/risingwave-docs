...
| postgres.is.aws.rds | **Optional**. Specifies whether the upstream database is AWS RDS. Set to `'true'` if using AWS RDS with a custom DNS hostname that doesn't contain `amazonaws.com`. RisingWave automatically detects AWS RDS instances from hostnames containing `amazonaws.com`, but this parameter allows explicit configuration for cases with custom DNS. Defaults to `'false'`. |
