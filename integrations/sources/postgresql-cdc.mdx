...
- RisingWave does not provide geospatial functions for this `BYTEA` payload. For example, `ST_AsEWKB(locationpoint)` fails with a function-signature error because the input is `BYTEA`, not a geometry type.

To inspect and validate the ingested bytes, encode them to text in RisingWave (replace `locationpoint` with your geometry column name):
