...
To monitor acknowledgment activity, use the **Grafana dashboard (dev)** in the **Streaming** section:

- `Source Connector Ack Successes` shows the rate of successful connector acknowledgments after checkpoint commit. It is backed by the Prometheus metric `source_connector_ack_success_count{source_name, connector_type}`.
- `Source Connector Ack Failures` shows the rate of connector acknowledgment failures after checkpoint commit. It is backed by the Prometheus metric `source_connector_ack_failure_count{source_name, connector_type, error_type}`.

For Pulsar sources, the `error_type` label can be `empty_message_id`, `channel_missing`, `channel_send_error`, `decode_error`, or `broker_error`.
