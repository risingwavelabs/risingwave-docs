...
RisingWave also exposes Prometheus metrics for source connector acknowledgments after checkpoint commit:

- `source_connector_ack_success_count{source_name, connector_type}` counts successful acknowledgments.
- `source_connector_ack_failure_count{source_name, connector_type, error_type}` counts failed acknowledgments.

For Pulsar sources, `connector_type` is `pulsar`. The `error_type` label can be one of the following values:
...
Use these metrics to diagnose backlog growth or subscription acknowledgment issues. For the full Prometheus metrics reference, see [Prometheus metrics reference](/operate/prometheus-metrics-reference).
