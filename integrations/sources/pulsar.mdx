...
### Observe acknowledgment behavior

RisingWave exposes Prometheus metrics for post-checkpoint connector acknowledgments, which helps diagnose Pulsar subscription ack backlogs or failures:

- `source_connector_ack_success_count{source_name, connector_type}` tracks successful connector acknowledgments.
- `source_connector_ack_failure_count{source_name, connector_type, error_type}` tracks failed connector acknowledgments.

For Pulsar sources, `connector_type` is `pulsar`. The `error_type` label uses bounded values:
...
If you use the RisingWave dev Grafana dashboard, check the **Source Connector Ack Successes** and **Source Connector Ack Failures** panels to monitor these metrics.
