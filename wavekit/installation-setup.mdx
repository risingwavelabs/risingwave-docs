...
This is the fastest way to try Wavekit. The `risingwavelabs/wavekit:vX.Y.Z-pgbundle` image includes a PostgreSQL server. Replace `vX.Y.Z` with the desired Wavekit version (e.g., `v0.4.0`).
...
1.  Create a `docker-compose.yaml` file:
    ...
    ```
    Note:
    *   Replace `vX.Y.Z` with the specific Wavekit version (e.g., `v0.4.0`).
...
You are now ready to start [Connecting to your RisingWave clusters](/wavekit/connect-risingwave-clusters)!