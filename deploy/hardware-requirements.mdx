...
When deploying on Kubernetes with Helm, always set resource `requests` and `limits` to the **same values** for each component. Mismatched values can cause unpredictable pod scheduling, CPU throttling, and memory pressure that degrade cluster stability and performance.

RisingWave is memory-intensive. Use a **1:4 CPU:memory ratio** when sizing components (for example, 1 CPU core to 4 GiB memory). See [Optional: Resize a node](/deploy/risingwave-k8s-helm#optional-resize-a-node) for an example configuration.
