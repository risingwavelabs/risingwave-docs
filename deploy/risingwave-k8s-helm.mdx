...
* **QoS Guaranteed class**: Pods are assigned the highest QoS class, making them less likely to be evicted under resource pressure
* **Consistent performance**: Helps provide more predictable CPU and memory availability for RisingWave pods
...
compactorComponent:
  resources:
    limits:
      ...
      memory: 4Gi
    requests:
      cpu: 2        # Should equal limits
      memory: 4Gi   # Should equal limits
```

For specific resource recommendations by component type, see [Hardware requirements](/deploy/hardware-requirements).
...
compactorComponent:
  resources:
    ...
    requests:
      # Should equal limits for production deployments
      cpu: 1
      memory: 2Gi
