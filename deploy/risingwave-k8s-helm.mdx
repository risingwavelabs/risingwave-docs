...
<Warning>
**Always set resource `requests` and `limits` to the same values for each component.** Mismatched values can cause unpredictable scheduling, CPU throttling, and memory pressure that degrade performance and stability — especially in on-premises deployments.
</Warning>
