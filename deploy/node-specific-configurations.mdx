...
Advanced streaming configurations can be set in the `[streaming.developer]` section. These are for advanced use cases and performance tuning:

| Configuration                               | Default | Description                                                                                                                                                                |
| :------------------------------------------ | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| default\_enable\_mem\_preload\_state\_table | false   | **Premium feature.** Whether to enable preloading all rows in memory for state tables by default. When enabled, all capable state tables will preload their state to memory for improved performance. Can be overridden globally with the `DEFAULT_ENABLE_MEM_PRELOAD_STATE_TABLE` environment variable. |
| mem\_preload\_state\_table\_ids\_whitelist | \[\]    | **Premium feature.** List of state table IDs to enable memory preloading for. Only takes effect when `default_enable_mem_preload_state_table` is false. |
| mem\_preload\_state\_table\_ids\_blacklist | \[\]    | **Premium feature.** List of state table IDs to disable memory preloading for. Only takes effect when `default_enable_mem_preload_state_table` is true. |
