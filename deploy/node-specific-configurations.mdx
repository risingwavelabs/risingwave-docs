...
Advanced streaming configurations can be set in the `[streaming.developer]` section:

| Configuration                               | Default | Notes                                                                                                                                                                |
| :------------------------------------------ | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| default\_enable\_mem\_preload\_state\_table | false   | **Premium feature.** Enable memory preloading for all state tables. Overrides with environment variable `DEFAULT_ENABLE_MEM_PRELOAD_STATE_TABLE=true`. |
| mem\_preload\_state\_table\_ids\_whitelist | \[\]    | **Premium feature.** State table IDs to enable memory preloading. Takes effect when `default_enable_mem_preload_state_table` is false. |
| mem\_preload\_state\_table\_ids\_blacklist | \[\]    | **Premium feature.** State table IDs to disable memory preloading. Takes effect when `default_enable_mem_preload_state_table` is true. |
