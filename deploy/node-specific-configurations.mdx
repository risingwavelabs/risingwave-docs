...
### Session initialization configurations

You can use the `[session_init]` section in `risingwave.toml` to bootstrap selected [runtime parameters](/operate/view-configure-runtime-parameters) when a cluster starts for the first time.
...
The values in `[session_init]` are seeded into the Meta store during cluster bootstrap only. On existing clusters, persisted session parameter values, such as those set with `ALTER SYSTEM SET`, take precedence. Updating `risingwave.toml` later does not retroactively change already persisted session parameters.

Currently, `[session_init]` supports the streaming parallelism family of runtime parameters: `streaming_parallelism`, `streaming_parallelism_for_backfill`, `streaming_parallelism_for_table`, `streaming_parallelism_for_source`, `streaming_parallelism_for_sink`, `streaming_parallelism_for_index`, and `streaming_parallelism_for_materialized_view`.

For details about these parameters and valid values, see [View and configure runtime parameters](/operate/view-configure-runtime-parameters).
...
| :---------------------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| in\_flight\_barrier\_nums | 10000 | The maximum number of pending barriers in each partial graph. Pending barriers include in-flight, collected but not committed, and currently completing barriers. |
| snapshot\_backfill\_barrier\_amplification\_factor | 1 | The multiplier applied to `in_flight_barrier_nums` when limiting pending barriers in a snapshot backfill partial graph. A value of 0 is treated as 1. |
| snapshot\_backfill\_finish\_max\_lagged\_barriers | 100 | The maximum number of lagged barriers allowed before a snapshot backfill job can merge into the database graph. Lower values reduce the allowed backlog before merge; higher values let backfill jobs merge sooner. |
...
RisingWave supports preloading state tables into memory to accelerate query processing and stateful operator performance. When enabled, all rows of specified state tables are loaded into memory at initialization, reducing read latency for hot tables.

Configurations can be set in `[streaming.developer]` section in the configuration file. For example:

| Configuration                            | Default | Description                             |
| :--------------------------------------- | :-----: | :-------------------------------------- |
| `default_enable_mem_preload_state_table` | `false` | Whether to enable preloading all rows in memory for state tables by default. If `true`, all capable state tables preload their state into memory. |
| `mem_preload_state_table_ids_whitelist`  | `[]`    | List of state table IDs to **enable** preloading all rows in memory. Only takes effect when `default_enable_mem_preload_state_table` is `false`. |
| `mem_preload_state_table_ids_blacklist`  | `[]`    | List of state table IDs to **disable** preloading all rows in memory. Only takes effect when `default_enable_mem_preload_state_table` is `true`. |

Using this feature involves certain configuration steps and considerations. To ensure it is set up correctly and used effectively, please contact our [support team](mailto:cloud-support@risingwave-labs.com) for guidance.

### Join hash map eviction

The following configuration controls how often the join hash map cache is evicted in hash join and as-of join executors. It can be set in the `[streaming.developer]` section of the configuration file.

| Configuration                       | Default | Description                             |
| :---------------------------------- | :-----: | :-------------------------------------- |
| `join_hash_map_evict_interval_rows` | `16`    | Number of rows processed between periodic evictions of the join hash map cache, for hash join and as-of join executors. Values smaller than `1` are clamped to `1` by the executor. |
...
You can also override cache refill behavior per materialized view by setting `streaming.developer.cache_refill_policy`:
...
| Value | Description |
| :---- | :---------- |
| `enabled` | Enable the default cache refill behavior for the materialized view. |
| `disabled` | Disable cache refill for the materialized view. |
| `streaming` | Refill cache only for streaming vnode ownership. |
| `serving` | Refill cache only for serving vnode ownership. |
| `both` | Refill cache for both streaming and serving vnode ownership. |

The override is persisted with the materialized view. After recovery, RisingWave rebuilds the cache refill runtime state from the persisted config automatically. For `serving`, `streaming`, and `both`, the runtime state also refreshes when worker ownership changes, so a full cluster recovery is not required to pick up serving worker changes.

#### Other storage configurations

Except for the above, RisingWave also provides some other storage configurations to help control the overall buffer and cache limits. Please see [Dedicated Compute Nodes](/operate/dedicated-compute-node) for more.

### Meta configurations

Meta configurations can be set in the `[meta]` section in the configuration file. For example:

```toml
[meta]
checkpoint_compression_algorithm = "zstd"
checkpoint_read_chunk_size = 134217728
checkpoint_read_max_in_flight_chunks = 4
```

The following table lists the available meta configurations related to Hummock version checkpoint:

| Configuration | Default | Description |
| :--- | :--- | :--- |
| `checkpoint_compression_algorithm` | `"zstd"` | Compression algorithm for Hummock version checkpoint data. Supported values: `"none"` (no compression), `"zstd"` (Zstandard, good balance of compression ratio and speed), `"lz4"` (faster compression with lower ratio). |
| `checkpoint_read_chunk_size` | `134217728` (128 MB) | Chunk size in bytes for reading large Hummock version checkpoints. Large checkpoints are read in parallel chunks to avoid single-request timeout issues. |
| `checkpoint_read_max_in_flight_chunks` | `4` | Maximum number of concurrent chunk reads when loading large checkpoints. Higher values may improve read throughput at the cost of increased memory usage. Memory usage is roughly `checkpoint_read_chunk_size × checkpoint_read_max_in_flight_chunks`. |

<Note>
Hummock version checkpoints are stored with an xxHash64 checksum for integrity verification. The compression format is backward-compatible: newer versions of RisingWave can read uncompressed checkpoints written by older versions, but older versions cannot read compressed checkpoints written by newer versions.
</Note>
