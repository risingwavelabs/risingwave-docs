...
<Note>
Review these prerequisites before starting the deployment. For a local test cluster, you will typically use Docker together with `kind`; for a managed Kubernetes service, keep `kubectl` and access to the target cluster ready before proceeding.
</Note>
