---
title: "Docker Compose (testing only)"
...
* Install [Docker Desktop](https://docs.docker.com/get-docker/) and ensure that it is running before you launch the cluster.
* Ensure the machine has enough CPU and memory for the containers. For sizing guidance, see [Hardware requirements](/deploy/hardware-requirements).
* Install [`psql`](/deploy/install-psql-without-postgresql) if you plan to connect from a local SQL client after deployment.

<Warning>
Docker Compose is intended for local testing and evaluation on a single machine. It is not suitable for production deployments.
</Warning>
