...
description: "This article describes how RisingWave configures fixed and adaptive streaming parallelism, including the unified `streaming_parallelism` values used for new and existing streaming jobs."
---

## Scaling policies

RisingWave supports adaptive and fixed parallelism for each streaming job, including materialized views, sinks, indexes, sources, and tables.

- Adaptive parallelism

  Adaptive parallelism is the default behavior for newly created streaming jobs. In this mode, RisingWave automatically adjusts parallelism as cluster capacity changes. You can use `adaptive` for the full worker set, `bounded(n)` to cap adaptive parallelism, or `ratio(r)` to use a fraction of available parallelism.
  ...
  To cap adaptive parallelism:

  ```sql
  ALTER TABLE t SET PARALLELISM = 'bounded(16)';
  ```
...
------+------+-------------------+-------------+----------------
 1001 | t    | table             | 4           |            128
 1002 | mv1  | materialized view | bounded(64) |            256
 1004 | idx  | index             | adaptive    |           1024
(3 rows)
```
