...
Agents can pre-compute feature vectors or aggregated state as materialized views, then query them in real time as part of tool calls. This pattern replaces inline computation with a persistent, always-fresh lookup.

In the context of online advertising, an agent pre-computes feature vectors over bidding data as a materialized view. When the agent needs to make a decision, it queries this MV as a tool call — getting an always-fresh result without any inline aggregation.
...
3. Agent queries the MV as a tool call

The agent calls this query as a tool — the result is always fresh, pre-computed, and returned in milliseconds.
...
Agents query pre-computed results from materialized views rather than computing them at call time. For example, a fraud detection tool queries a pre-maintained `suspicious_transactions` view — the agent gets a real-time fraud score without computing it inline.
...
RisingWave maintains this view incrementally — the agent never recomputes it at query time.
...
The agent calls this query to get a real-time fraud score for a specific card number — no inline aggregation, no polling.
