...
- **Serving Node**: The stateless node that parses and optimizes queries, and serves ad-hoc queries issued from users and applications in a highly concurrent and low-latency manner. It handles user connections, executes ad-hoc queries, and dispatches streaming jobs to the Streaming Nodes.
