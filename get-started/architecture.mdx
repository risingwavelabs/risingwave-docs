...
This node serves as the PostgreSQL-compatible SQL gateway. It handles user requests and delivers results with high concurrency and low latency. Clients like `psql` can connect to it seamlessly.
...
<Note>
New to barriers and checkpoints? A **barrier** is a periodic sync marker; a **checkpoint** is a global consistent snapshot created from barriers. By default, RisingWave generates one barrier every **1 second** (`barrier_interval_ms = 1000`). See [Data persistence](/reference/data-persistence#what-are-barriers-and-checkpoints).
</Note>
...
In RisingWave, a Compute Node can be a dedicated Serving Node or Streaming Node, or a hybrid one (the default) that can handle both serving and streaming tasks.
...
In RisingWave Cloud, we can use [Serverless Compaction](/cloud/serverless-compaction) to automatically scale compactors and simplify compaction resource management.
