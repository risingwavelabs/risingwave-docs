...
Beyond caching hot data in memory, RisingWave supports [**elastic disk cache**](/deploy/disk-cache), a powerful performance optimization that uses local disks or EBS for efficient data caching. This minimizes access to S3, lowering processing latency and cutting S3 access costs.
