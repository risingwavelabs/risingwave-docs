...
description: "RisingWave is a real-time event streaming platform. It offers a unified experience for real-time data ingestion, stream processing, low-latency serving, and Apache Iceberg™ management."
---
...
RisingWave treats Apache Iceberg™ as a [first-class citizen](/iceberg/overview). It continuously transforms and writes streaming data into Iceberg tables, and automates key management tasks such as compaction, small-file optimization, and schema evolution. RisingWave also supports incremental materialized views on Iceberg tables, enabling continuous view maintenance and consistent updates without external pipelines. Through a PostgreSQL-compatible interface, users can ingest, transform, query, and manage Iceberg data seamlessly within one system.
