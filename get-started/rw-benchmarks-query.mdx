...
description: "This topic provides an overview of RisingWave's ad-hoc query serving performance characteristics (latency and throughput) based on the industry-standard sysbench benchmark."
---

## Performance highlights

RisingWave demonstrates low-latency query responses and high throughput for various OLTP workloads, even under high concurrency. Key takeaways from the sysbench results include:
...
- **Hardware:**
    - One AWS EC2 instance (8 vCPUs, 16GB memory) for the RisingWave streaming node.
    - One AWS EC2 instance (8 vCPUs, 16GB memory) for the RisingWave serving node.
...
- Memory impact: When the memory is sufficient to cache all the queried data, performance remains relatively stable as the table size grows. This highlights the importance of memory capacity for OLTP workloads.
- Bottlenecks: In the `oltp_point_select` workload, the serving node becomes the primary bottleneck due to the lightweight nature of the operations. For `select_random_points` and `select_random_ranges`, both the streaming and serving nodes experience CPU saturation.
