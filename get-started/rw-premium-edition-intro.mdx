...
The following are the Premium Edition features, which we have added "Premium Edition Feature" note in the documentation.
...
For self-hosted users, you need to set up a license key to access Premium Edition features. The license key, essentially a certificate, is a JSON Web Token (JWT) that encodes information such as the license tier and expiration time. The integrity of the license key is validated using asymmetric encryption.
