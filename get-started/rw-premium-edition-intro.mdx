...
The `--license-key-file` CLI option is only available for the meta node, as the license key is propagated to other nodes through system parameters. When the `--license-key-file` option is specified, any manual configuration of the license key through system parameters (`license_key`), the initial configuration (`system.license_key`), or the `RW_LICENSE_KEY` environment variable will be rejected.
