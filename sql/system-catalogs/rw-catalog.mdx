...
| `rw_fragment_parallelism`             | Contains information about the parallelism configuration at the fragment level, including fragment IDs, parallelism, and parallelism policy that indicates the reasoning for each fragment's parallelism (e.g., `inherit(adaptive)`, `override(fixed(3))`, `upstream_fragment([1])`, or `single`).                                                                                                                                                                                                                                               |
| `rw_fragments`                        | Contains low-level information about fragments in the database, including fragment IDs, table IDs, parallelism policy, and more.                                                                                                                                                                                                                                                                 |
...
## Understanding `parallelism_policy`

The `parallelism_policy` field in `rw_fragments` and `rw_fragment_parallelism` provides insight into how each fragment's parallelism is determined. This field helps you understand the reasoning behind each fragment's parallelism configuration.

### Parallelism policy values

The `parallelism_policy` field can have the following values:

- **`inherit(adaptive)`**: The fragment inherits its parallelism from the streaming job, which uses adaptive parallelism. The parallelism automatically adjusts based on available resources.

- **`inherit(fixed(N))`**: The fragment inherits its parallelism from the streaming job, which has a fixed parallelism of N.

- **`override(fixed(N))`**: The fragment's parallelism has been manually overridden using `ALTER FRAGMENT`, setting it to a fixed value of N.

- **`upstream_fragment([ID1, ID2, ...])`**: The fragment's parallelism is determined by its upstream fragments. This typically occurs in NO_SHUFFLE scenarios where data distribution requires matching parallelism.

- **`single`**: The fragment has single parallelism, meaning it runs on a single actor. This is common for certain operator types that cannot be parallelized.

### Example usage

```sql Example
-- Create a table and materialized views
CREATE TABLE t(v int);
CREATE MATERIALIZED VIEW m AS SELECT * FROM t;
CREATE MATERIALIZED VIEW m1 AS SELECT count(*) FROM t;

-- Query fragment parallelism information
...
After manually altering a fragment's parallelism:

```sql Example
-- Override fragment parallelism
ALTER FRAGMENT 2 SET PARALLELISM = 3;
...
The `parallelism_policy` field is particularly useful for:

- **Debugging performance issues**: Understand why certain fragments have specific parallelism settings.
- **Monitoring configuration changes**: Track when and how fragment parallelism has been manually adjusted.
- **Understanding dependencies**: Identify fragments whose parallelism is constrained by upstream fragments.
