...
| schema.registry.ca_pem_path      | Path to a custom root CA certificate file (PEM) for the Schema Registry TLS connection, or `'ignore'` to skip certificate verification. Supports `SECRET ... AS FILE`. |
...
CREATE CONNECTION sr_conn WITH (
  ...
  schema.registry.ca_pem_path = SECRET sr_ca AS FILE
);
```
