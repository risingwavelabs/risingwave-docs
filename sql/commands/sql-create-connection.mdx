...
<Note>
For custom root CA certificates used by Schema Registry, configure `schema.registry.ca_pem_path` in the schema registry format and encoding options, not in `CREATE CONNECTION`. See [Parameter reference](/ingestion/formats-and-encoding-options#parameter-reference).
</Note>
