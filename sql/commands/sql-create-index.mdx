...
When creating an index, RisingWave will backfill historical data from the indexed table or materialized view. To improve isolation between the backfill phase and streaming phase, you can enable snapshot backfill by setting `SET streaming_use_snapshot_backfill=true;` before creating the index. This helps prevent resource contention between backfilling historical data and processing new streaming data. For more details, see [View and configure runtime parameters](/operate/view-configure-runtime-parameters).
