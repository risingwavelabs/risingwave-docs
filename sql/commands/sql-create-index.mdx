...
USING index_method ( index_column [ ASC | DESC ], [, ...] )
[INCLUDE ( include_column [, ...] ) ]
WITH (distance_type = '<type>');
```

<Note>
Vector index is added in v2.6.0 and is currently in **[technical preview](/changelog/product-lifecycle#product-release-lifecycle)** stage.
</Note>
...
| USING index_method | Specify the index method. Supported methods are `flat` and `HNSW`.|
...
1. Create a table with vector data. This store product information along with a semantic embedding vector that represents its features.
...
ON products USING flat (embedding)
