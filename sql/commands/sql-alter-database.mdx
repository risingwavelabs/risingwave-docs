...
| Parameter or clause | Description |
| :------------------ | :---------- |
| **SET RESOURCE_GROUP** | Sets the database's resource group metadata. |
| **RESET RESOURCE_GROUP** | Resets the database to the default resource group. |
| _resource_group_name_ | The target resource group. |
| **DEFERRED** | Required. Applies the metadata change without immediately moving inherited streaming jobs. |

```sql
-- Change the database resource group metadata
ALTER DATABASE travel SET RESOURCE_GROUP = high_priority DEFERRED;

-- Reset the database to the default resource group
ALTER DATABASE travel RESET RESOURCE_GROUP DEFERRED;
```
