...
<Note>
A `BACKFILL_RATE_LIMIT` of `0` pauses the backfill rather than removing the limit. Use `DEFAULT` (equivalent to `-1`) to remove it. See [Rate limiting](/processing/backfill#rate-limiting) for details.
</Note>
