...
```sql
ALTER SINK sink_name
    SET BACKFILL_PARALLELISM { TO | = } parallelism_number;
```
...
| _parallelism\_number_         | Can be `ADAPTIVE` or a fixed number (e.g., 1, 2, 3). Setting it to `ADAPTIVE` or `0` expands backfill parallelism to all available units. A fixed number locks it at that value. |
