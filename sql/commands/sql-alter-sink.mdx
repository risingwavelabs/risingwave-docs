...
| parallelism_number | Can be `ADAPTIVE`, `0`, a fixed number (for example, `1`, `2`, or `3`), `bounded(n)`, or `ratio(r)`. `ADAPTIVE` and `0` expand the job's parallelism to the full available worker set, a fixed number locks it at that value, `bounded(n)` caps adaptive parallelism, and `ratio(r)` uses a fraction of available parallelism. The maximum allowed value is determined by the `max_parallelism` of the job. For more information, see [Configuring maximum parallelism](/deploy/k8s-cluster-scaling#configuring-maximum-parallelism). |
...
| _parallelism\_value_           | Can be `ADAPTIVE`, `0`, a fixed number (for example, `1`, `2`, or `3`), `bounded(n)`, or `ratio(r)`. `ADAPTIVE` and `0` expand backfill parallelism to the full available worker set, a fixed number locks it at that value, `bounded(n)` caps adaptive parallelism, and `ratio(r)` uses a fraction of available parallelism. |
...
| property_name | The properties vary by connector type: <ul><li>For **ClickHouse**, **DeltaLake**, and **StarRocks**, the supported parameter is `commit_checkpoint_interval`.</li><li>For **Iceberg**, the supported parameters are `commit_checkpoint_interval` and `commit_checkpoint_size_threshold_mb`.</li><li>For **Kafka**, the supported parameters include: `properties.allow.auto.create.topics`, `properties.batch.num.messages`, `properties.batch.size`, `properties.enable.idempotence`, `properties.max.in.flight.requests.per.connection`, `properties.message.max.bytes`, `properties.message.send.max.retries`, `properties.message.timeout.ms`, `properties.queue.buffering.max.kbytes`, `properties.queue.buffering.max.messages`, `properties.queue.buffering.max.ms`, `properties.request.required.acks`, `properties.retry.backoff.ms`, `properties.receive.message.max.bytes`. </li></ul> |
