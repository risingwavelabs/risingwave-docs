...
<Note>
A `BACKFILL_RATE_LIMIT` of `0` pauses the backfill rather than removing the limit. A sink whose backfill is paused stays at `0.00%` in [`rw_ddl_progress`](/operate/monitor-statement-progress) indefinitely, with no error reported. Use `DEFAULT` (equivalent to `-1`) to remove the limit. To check the current value for each fragment, query `rw_catalog.rw_rate_limit`.
</Note>
