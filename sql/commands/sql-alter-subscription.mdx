...
| Parameter or clause | Description |
| :------------------ | :---------- |
| **SET RETENTION TO** | Changes how long incremental subscription data is retained. |
| _duration_ | An interval such as `3H` or `2D`. |

```sql
-- Retain incremental data for two days.
ALTER SUBSCRIPTION user_updates SET RETENTION TO '2D';
```
