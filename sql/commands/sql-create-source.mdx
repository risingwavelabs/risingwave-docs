...
You can expose one upstream table from a shared MySQL, PostgreSQL, or SQL Server CDC source as a catalog-only upsert source. The source itself does not create a streaming job or store the current table state. Instead, each downstream streaming job creates its own CDC scan.

First create the [shared CDC source](/ingestion/cdc-with-risingwave#shared-sources-and-transactional-consistency). Then create the table source. For example, the following source infers its columns and primary key from the upstream PostgreSQL table:
...
You can define the columns explicitly instead of using `*`. The declared columns, data types, and primary key are validated against the upstream table. The upstream table must have a primary key.

<Warning>
CDC table sources currently require exactly `WITH (snapshot = 'false')`. They do not backfill existing rows; a downstream streaming job receives only changes that occur after that job is created. You cannot query a CDC table source directly in batch mode or alter its columns or connector properties.
</Warning>
