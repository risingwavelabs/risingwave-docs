...
To completely disable it at the cluster level, go to [`risingwave.toml`](https://github.com/risingwavelabs/risingwave/blob/main/src/config/example.toml) and set `enable_shared_source = false` under `[streaming.developer]`.
