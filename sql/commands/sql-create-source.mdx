...
A passthrough sink can forward inserts, updates, and deletes without first storing the source table's current state in RisingWave. It can also apply stateless operations such as projections, casts, and filters on primary-key columns. For example, forward changes to a [PostgreSQL sink](/integrations/destinations/postgresql).

First create the destination table in the **downstream PostgreSQL database**, not in RisingWave:

```sql
CREATE TABLE public.orders_replica (
    id INT PRIMARY KEY,
    customer_id INT,
    amount BIGINT
);
...
Then run the following in **RisingWave**. This example keeps rows with a positive `id`, projects the required columns, and casts `amount` from `INT` to `BIGINT`:

```sql
CREATE SINK orders_to_postgres AS
SELECT id, customer_id, amount::BIGINT AS amount
FROM orders_cdc
WHERE id > 0
WITH (
    connector = 'jdbc',
    jdbc.url = 'jdbc:postgresql://postgres:5432/replica',
    user = 'sink_user',
    password = 'your_password',
    schema.name = 'public',
    table.name = 'orders_replica',
    type = 'upsert',
    primary_key = 'id'
);
```

Replace the JDBC URL and credentials with your downstream PostgreSQL configuration. Keep the source primary key as the sink and destination table key so that updates and deletes apply to the same downstream record. The filter uses only the primary key, which remains available even in sparse delete events. A sink requiring retractable input cannot consume this source directly.
