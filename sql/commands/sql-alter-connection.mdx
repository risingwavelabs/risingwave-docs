...
| _property\_name_    | The connector property to update. Use the same format as in [`CREATE CONNECTION`](/sql/commands/sql-create-connection). For example: `properties.sasl.username = 'dev1'`. Secrets are supported: `properties.bootstrap.server = SECRET my_secret`. |
