...
| WITH clause     | Specifies optional parameters for the materialized view. Common options include:
  ...
  - `refresh.interval.sec`: Create a batch refresh materialized view that refreshes periodically. The value must be a positive integer in seconds.
  - `cloud.serverless_backfill_enabled`: Run the initial backfill on dedicated serverless backfiller nodes for one statement. In RisingWave Cloud, enable serverless backfilling in the Cloud console first. See [Serverless backfilling](/processing/serverless-backfill).
...
4. To offload the backfill phase to dedicated serverless backfiller nodes, enable [serverless backfilling](/processing/serverless-backfill). You can enable it for the current session with `SET enable_serverless_backfill = true;` or for one statement with `WITH (cloud.serverless_backfill_enabled = true)`.
...
## Batch refresh materialized views

Use `WITH (refresh.interval.sec = <seconds>)` to create a materialized view that refreshes periodically instead of applying upstream changes continuously.

```sql
CREATE MATERIALIZED VIEW mv_up AS
SELECT * FROM t;

CREATE MATERIALIZED VIEW mv_batch
WITH (refresh.interval.sec = 60)
AS
SELECT * FROM mv_up;
```

Batch refresh materialized views currently have these constraints:

- Creation fails if `streaming_use_snapshot_backfill` is `false` or the query plan cannot use snapshot backfill.
- They must not depend directly on sources.
- Their query definition cannot be changed in place. Metadata operations such as `ALTER MATERIALIZED VIEW ... RENAME TO ...` are supported.
- They cannot be used as upstreams for downstream streaming jobs, including streaming materialized views created without `refresh.interval.sec`.
