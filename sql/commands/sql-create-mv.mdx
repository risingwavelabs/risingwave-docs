...
- They require snapshot backfill to be enabled.
- They must not depend directly on sources.
- They do not support replacing their query definition with `ALTER MATERIALIZED VIEW ... AS ...`. This does not prohibit metadata changes such as `ALTER MATERIALIZED VIEW ... RENAME TO ...`.
- They cannot be used as upstreams for downstream streaming jobs, including other materialized views.
