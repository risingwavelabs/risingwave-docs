...
3. To avoid join amplification during backfill, you can optionally specify `backfill_order` in the `WITH` clause to control the order. Use the `->` operator to define dependencies between tables, and the left table will be fully backfilled before the right begins.
    ...
    - Backfill order recovery is not supported. If a backfilling job restarts, the specified order will be lost, and all tables will backfill concurrently.
