...
| WITH clause     | Specifies optional parameters for the materialized view. Common options include:
  ...
  - `cloud.serverless_backfill_enabled`: *(RisingWave Cloud only)* Run the initial backfill on dedicated serverless backfiller nodes. Requires serverless backfilling to be enabled in the Cloud console. See [Serverless backfilling](/cloud/serverless-backfilling).
...
4. To offload the backfill phase to a dedicated controller and further reduce impact on existing streaming jobs, enable [serverless backfill](/processing/serverless-backfill) with `SET enable_serverless_backfill = true;` before creating the materialized view.
