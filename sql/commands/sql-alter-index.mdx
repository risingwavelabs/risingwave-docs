...
Use this statement to move the index's streaming job to a different resource group.

| Parameter or clause | Description |
| :------------------ | :---------- |
| **SET RESOURCE_GROUP** | Reassigns the index's streaming job to a resource group. |
...
```sql
ALTER INDEX idx_orders SET RESOURCE_GROUP = DEFAULT;
ALTER INDEX idx_orders SET RESOURCE_GROUP TO rg_batch DEFERRED;
```
...
`RESET RESOURCE_GROUP` is equivalent to resetting the index to the default resource group.

```sql
ALTER INDEX idx_orders RESET RESOURCE_GROUP;
