...
* If you only need the `UPDATE` to wait until it is included in a successful checkpoint, you can enable [`DML_WAIT_PERSISTENCE`](/sql/commands/sql-set-dml-wait-persistence). This does not provide the same immediate read-after-write visibility as `FLUSH`.
