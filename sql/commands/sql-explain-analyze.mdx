...
As [`EXPLAIN`](/sql/commands/sql-explain) shows the execution plan of a query or job before execution, `EXPLAIN ANALYZE` can execute the streaming job, including table, materialized view, sink, or index, and collects runtime statistics to help identify performance bottlenecks.
