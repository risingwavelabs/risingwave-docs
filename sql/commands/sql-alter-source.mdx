...
| :-------- | :------------------- |
| Kafka (`kafka`) | `properties.client.id`, `properties.sync.call.timeout`, `properties.enable.auto.commit`, `properties.enable.ssl.certificate.verification`, `properties.fetch.max.bytes`, `properties.fetch.queue.backoff.ms`, `properties.fetch.wait.max.ms`, `properties.message.max.bytes`, `properties.queued.max.messages.kbytes`, `properties.queued.min.messages`, `properties.receive.message.max.bytes`, `properties.statistics.interval.ms`, `properties.ssl.endpoint.identification.algorithm`, `properties.security.protocol`, `properties.sasl.mechanism`, `properties.sasl.username`, `properties.sasl.password` |
| PostgreSQL CDC (`postgres-cdc`) | `username`, `password`, `cdc.source.wait.streaming.start.timeout` |
| MySQL CDC (`mysql-cdc`) | `username`, `password`, `cdc.source.wait.streaming.start.timeout` |
| SQL Server CDC (`sqlserver-cdc`) | `username`, `password`, `cdc.source.wait.streaming.start.timeout` |
| MongoDB CDC (`mongodb-cdc`) | `username`, `password`, `cdc.source.wait.streaming.start.timeout` |

<Note>
For CDC connectors, only the credential and timeout fields above are alterable. Endpoint fields such as `hostname`, `port`, and `database.name` still require recreating the source.
</Note>
...
```sql Example: PostgreSQL CDC
-- Rotate the credentials used by a shared Postgres CDC source
