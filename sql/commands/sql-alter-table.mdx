...
| property_name | The connector property to update. For tables with connector settings, supported connectors include Kafka, Google Pub/Sub, Pulsar, MySQL CDC, PostgreSQL CDC, SQL Server CDC, and MongoDB CDC. See the [supported connector properties](/sql/commands/sql-alter-source#connector-with) for the allowed properties of each connector. For tables created with `ENGINE = iceberg`, see [Iceberg engine table properties](#iceberg-engine-table-properties). |

For tables created from a shared CDC source, update the source with `ALTER SOURCE ... CONNECTOR WITH`. `ALTER TABLE ... CONNECTOR WITH` applies to tables with their own connector settings and to Iceberg engine tables. Properties outside the supported list for the connector cannot be changed with this command.
...
#### Iceberg engine table properties

For tables created with `ENGINE = iceberg`, use `ALTER TABLE ... CONNECTOR WITH` to update the following properties without recreating the table:

| Category | Supported properties |
| :------- | :------------------- |
| Commit interval | `commit_checkpoint_interval` |
| Maintenance scheduling | `enable_compaction`, `compaction_interval_sec` |
| Snapshot expiration | `enable_snapshot_expiration`, `snapshot_expiration_max_age_millis`, `snapshot_expiration_retain_last`, `snapshot_expiration_clear_expired_files`, `snapshot_expiration_clear_expired_meta_data` |
| Manifest rewrite | `enable_manifest_rewrite`, `manifest_rewrite_target_size_bytes`, `manifest_rewrite_min_count_to_merge` |
| Compaction | `compaction.max_snapshots_num`, `compaction.small_files_threshold_mb`, `compaction.delete_files_count_threshold`, `compaction.trigger_snapshot_count`, `compaction.target_file_size_mb`, `compaction.type`, `compaction.write_parquet_compression`, `compaction.write_parquet_max_row_group_rows`, `compaction.write_parquet_max_row_group_bytes` |

Properties outside this list, such as `write_mode`, cannot be changed with this command. For maintenance configuration details and prerequisites, see [Iceberg table maintenance](/iceberg/maintenance).
...
### `SET BACKFILL_RATE_LIMIT` {#set-backfill-rate-limit}
```sql
ALTER TABLE table_name
