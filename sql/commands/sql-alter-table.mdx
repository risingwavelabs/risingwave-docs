...
* New fields added to a struct type cannot be used by existing materialized views or indexes. You must create new materialized views or indexes to reference the new fields.
* If you remove fields that are referenced by downstream queries, the new data for those fields will become `NULL` in the results.
</Note>
