...
Use the [WAIT](/sql/commands/sql-wait) command to block the current session until a specific job (or all background jobs) complete. This is useful in scripts or workflows where you need to ensure a job is done before proceeding.

```sql
-- Wait for a specific materialized view creation to finish
