---
title: "REFRESH TABLE"
description: "The `REFRESH TABLE` command manually triggers a refresh operation for tables created with the FULL_RELOAD refresh mode."
---
...
The `REFRESH TABLE` command manually triggers a full reload of data from the external source for tables configured with the `FULL_RELOAD` refresh mode. This command is useful when you need to immediately update the table data without waiting for the next scheduled refresh.
