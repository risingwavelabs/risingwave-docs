...
| _table\_name_          | A table or materialized view. In qualified wildcards such as `table_name.*`, use the relation name or alias from the `FROM` clause. For an unaliased schema-qualified relation such as `FROM schema_name.table_name`, `table_name.*` is supported.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| _sort\_expression_ | An output column name, output column ordinal number, hidden select expression, or other expression, including a scalar subquery. |
...
### Query per-row update time with `_rw_timestamp`

RisingWave exposes a hidden system column named `_rw_timestamp` on tables and materialized views. You can select it in a **batch query** to see when each row was last updated.

```sql
SELECT *, _rw_timestamp
FROM my_materialized_view
ORDER BY _rw_timestamp DESC;
```

To inspect hidden columns on a relation, use [`SHOW COLUMNS`](/sql/commands/sql-show-columns) or [`DESCRIBE`](/sql/commands/sql-describe).
