...
`CREATE SINK INTO` is useful when you want to write streaming results back into a RisingWave table instead of an external system.

- To union multiple upstreams into one table, see the example above.
- To maintain a wide table with multiple sinks and `ON CONFLICT`, see [Maintain wide table with table sinks](/processing/maintain-wide-table-with-table-sinks).
- To change streaming job logic in place by versioning sinks into a table, see [Alter a streaming job](/operate/alter-streaming#modifying-a-streaming-job-in-place).
