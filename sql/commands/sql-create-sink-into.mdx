...
3. If the target table uses `ON CONFLICT DO UPDATE IF NOT NULL`, `ON CONFLICT DO NOTHING`, or `WITH VERSION COLUMN(...)`, RisingWave preserves distinct upstream row-level changes that map to the same downstream primary key instead of compacting them into one final update per barrier. This ensures the target table's conflict handling semantics are preserved for `CREATE SINK ... INTO`.
