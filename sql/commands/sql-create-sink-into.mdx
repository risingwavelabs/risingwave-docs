...
| `WITH` clause           | You can use options such as `type = 'append-only'`, `force_append_only = 'true'`, and `preserve_row_level_changes = 'true'` to control how the sink forwards changes to the target table.                                                 |
...
<Note>
For target tables that use `ON CONFLICT DO UPDATE IF NOT NULL` or a `VERSION COLUMN`, RisingWave automatically preserves row-level changes for `CREATE SINK ... INTO` so the table can apply its conflict handling semantics correctly.
</Note>
...
2. The sink target table may have a different primary key than the upstream table. If primary keys overlap downstream, rows can be overwritten. This can make your downstream table have fewer rows than your upstream table. If you need the target table to observe every row-level change for the same target primary key within a barrier, set `preserve_row_level_changes = 'true'`. RisingWave enables this automatically for target tables that use `ON CONFLICT DO UPDATE IF NOT NULL` or a `VERSION COLUMN`.
