...
| `WITH` clause           | You can use options such as `type = 'append-only'` and `ignore_delete = 'true'` to explicitly control append-only behavior. The deprecated alias `force_append_only = 'true'` is still supported for backward compatibility.                                                 |
