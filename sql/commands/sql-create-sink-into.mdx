...
1. Sink into table aligns rows to be inserted based on column orders, rather than column names. If you discover some data correctness issues, verify that the order of selected data.
2. The table we are sinking to may have a different primary key compared to the upstream table. This may result in entries being overridden, and your upstream table having less rows than your downstream, if there are downstream primary key overlaps.
