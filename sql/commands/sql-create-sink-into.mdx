...
| `WITH` clause           | You can use options such as `type = 'append-only'` or `ignore_delete = 'true'` to control how updates and deletes are written into the target table. `force_append_only = 'true'` is kept as a backward-compatible alias of `ignore_delete = 'true'`.                                                 |
