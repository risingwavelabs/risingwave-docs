...
| `WITH` clause           | You can use options such as `type = 'append-only'`, `type = 'upsert'`, and `ignore_delete = 'true'` to explicitly control the sink behavior. The `ignore_delete` option (or its alias `force_append_only`) can be used with both append-only and upsert sinks to drop DELETE messages and convert UPDATE messages to INSERT messages.                                                 |
