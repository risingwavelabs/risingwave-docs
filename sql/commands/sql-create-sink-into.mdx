...
| `WITH` clause           | You can use options such as `type = 'append-only'`, `type = 'upsert'`, and `ignore_delete = 'true'` to explicitly control sink behavior. The `ignore_delete` option drops DELETE messages and converts UPDATE messages to INSERT messages. `force_append_only` is kept as an alias for `ignore_delete`.                                                 |
