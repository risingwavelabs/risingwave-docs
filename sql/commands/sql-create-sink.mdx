...
| _since\_timestamp_                | **Optional**. Start a direct sink from changes after the specified `timestamptz` value instead of emitting snapshot rows. This option is only supported for `CREATE SINK ... FROM table_or_mv` with `snapshot = false`. The timestamp must include an explicit time zone, for example `2024-01-01 00:00:00Z`, and must fall within the retained changelog history of the upstream table or materialized view. Create the sink in the same database as the upstream object. |
...
Use `since_timestamp` together with `snapshot = false` to create a direct sink that starts from the upstream changelog after a specific timestamp, without emitting the initial snapshot.
