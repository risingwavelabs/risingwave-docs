...
| _snapshot_                        | **Optional**. Set this parameter to `false` to skip backfilling historical data and transmit only incremental changes. This option is only applicable when using the `CREATE SINK FROM` syntax, not `CREATE SINK AS`. The default value is `true`. To replace an existing sink without historical backfill, use [`REPLACE SINK`](/sql/commands/sql-replace-sink). |
