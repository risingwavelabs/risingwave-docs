...
| _ignore\_delete_                  | **Optional**. When set to `true`, drop DELETE messages and convert UPDATE messages to INSERT messages. Works with both `append-only` and `upsert` sink types. The `force_append_only` option is kept as a backward-compatible alias. The default value is `false`. |
