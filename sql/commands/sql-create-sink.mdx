...
For upsert sinks created with `snapshot=false`, the downstream `primary_key` must normally be the same as or a subset of the key derived from the input stream. If hidden stream-key columns make that validation impossible for a compatibility scenario, you can explicitly bypass it for the current session:

```sql
SET streaming_unsafe_allow_upsert_sink_pk_mismatch = true;
```

This session variable is disabled by default. Enable it only if the sink input preserves complete old and new rows. Otherwise, a downstream primary-key change may leave stale rows in the downstream system. For more details, see [View and configure runtime parameters](/operate/view-configure-runtime-parameters).
