...
| _since\_timestamp_ | **Optional**. Start a sink from changes after the specified `timestamptz` value instead of emitting snapshot rows. Requires `snapshot = false` and an upstream table or materialized view in the same database. Supported with `CREATE SINK FROM` and compatible `CREATE SINK AS` queries, such as projection and filtering. The timestamp must include an explicit time zone, for example `2024-01-01 00:00:00Z`, and fall within the retained changelog history. |
