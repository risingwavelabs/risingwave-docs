...
| _ignore\_delete_                  | **Optional**. When set to `true`, RisingWave drops `DELETE` messages and converts `UPDATE` messages to `INSERT` messages in the sink executor. This is commonly used together with `type = 'append-only'` to force append-only output from a non-append-only upstream. The deprecated alias `force_append_only` is still accepted for backward compatibility. The default value is `false`. |
