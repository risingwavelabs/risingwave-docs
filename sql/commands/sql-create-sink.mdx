...
| _ignore\_delete_                  | **Optional**. When set to `true`, the sink drops `DELETE` messages and converts `UPDATE` messages to `INSERT` messages. You can use this with `type = 'append-only'` or `type = 'upsert'`. The default value is `false`. `force_append_only` is kept as a backward-compatible alias. |
