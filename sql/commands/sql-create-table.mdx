...
However, please note that this cleanup only applies to the data within the table itself—it does not affect downstream materialized views or other streaming jobs. If you need to clean up data in downstream MVs used in queries, you should use a [Temporal Filter](/processing/sql/temporal-filters) in the downstream stream job.
