...
| **ENGINE** | Specify the Iceberg table engine to store data natively in the Iceberg format. For more information, see [Create and manage native Iceberg tables](/iceberg/create-manage-native-iceberg-tables).                                                                                                                                                                                                                  |
...
In a `CREATE TABLE` statement, you can specify `ENGINE = iceberg` to store table data in the Apache Iceberg format on external object storage. This provides an alternative to RisingWave’s default internal row-based storage, making it easier to persist data for long-term use and cross-system access. For more information on the syntax and usage, see [Create and manage native Iceberg tables](/iceberg/create-manage-native-iceberg-tables).
