...
The record with insert operation could introduce duplicate records with the same primary key in the table. In that case, an alternative action specified by the `ON CONFLICT conflict_action` clause will be adopted. The record can come from Insert DML statement, external connectors of the table, or [sinks into the table](/sql/commands/sql-create-sink-into).

The `conflict_action` could one of the following. A [version column](#version-column) can be specified together for `DO UPDATE FULL` and `DO UPDATE IF NOT NULL`. When version column is specified, the insert operation will take effect only when the newly inserted row's version column is greater or equal than (and is not NULL) the existing row's version column.
...
2. Use an integer as a logical version number. With this technique, you can implement "zero-downtime incremental version upgrade" for data in the table: you insert a new version of data with a higher version number while keeping old data, and then after finishing the upgrade, you clean up old data with a `DELETE` statement (Upsert-then-Delete).
