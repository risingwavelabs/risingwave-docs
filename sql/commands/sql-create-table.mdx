...
- RisingWave supports **NOT NULL** constraints for tables:
  - For batch inserts and updates via `INSERT` or `UPDATE` statements, RisingWave throws an error if any row contains NULL in a NOT NULL column.
  - For tables with connectors (streaming ingestion), the NOT NULL constraint is enforced the same way as batch operations - rows with NULL values in NOT NULL columns will cause errors during ingestion.

- For sources created with `CREATE SOURCE`, NOT NULL constraints cannot be specified in the schema.
...
- To stop ingestion later while keeping the table schema and existing data, use [`ALTER TABLE ... DROP CONNECTOR`](/sql/commands/sql-alter-table#drop-connector).
...
| watermark\_clause                 | A clause that defines the watermark for a timestamp column. The syntax is `WATERMARK FOR column_name AS expr [WITH TTL]`. Watermarks enable event-time processing. `WITH TTL` enables time-based cleanup for the table based on the watermark. For details about watermarks, see [Watermarks](#watermarks) below for more.           |
| APPEND ONLY                       | When this option is specified, the table will be created as an append-only table. An append-only table cannot have primary keys. UPDATE and DELETE statements are not valid for append-only tables. See [Append only](#append-only) below for more.                                                                            |
...
| **ENGINE** | Specify the Iceberg table engine to store data natively in the Iceberg format. For more information, see [Create and manage internal Iceberg tables](/iceberg/internal-iceberg-tables).                                                                                                                                                                                                                  |
...
- Append-only tables are a good fit for append-only event streams where a primary key is not needed.


## Use Iceberg table engine

In a `CREATE TABLE` statement, you can specify `ENGINE = iceberg` to store table data in the Apache Iceberg format on external object storage. This provides an alternative to RisingWave’s default internal row-based storage, making it easier to persist data for long-term use and cross-system access. For more information on the syntax and usage, see [Create and manage internal Iceberg tables](/iceberg/internal-iceberg-tables).

## Watermarks

RisingWave supports generating watermarks when creating a streaming table. Watermarks are like markers or signals that track the progress of event time, allowing you to process events within their corresponding time windows. For more information on the syntax on how to create a watermark, see [Watermarks](/processing/watermarks).

You can add `WITH TTL` to the watermark definition to enable time-based cleanup for the table. See [Watermark-based TTL](#watermark-based-ttl-event-time) below.

<Note>
Watermarks on non-append-only tables (tables with primary keys) and the `WITH TTL` option are available since v2.8.0.
</Note>

## Watermark-based TTL (event time)

When you add `WITH TTL` to a watermark on a table, RisingWave treats rows whose event time is at or below the watermark as expired. Late changes (INSERT/UPDATE/DELETE) for those rows are ignored, and table state is cleaned based on event time. This cleanup only affects the table itself and does not delete previously computed results in downstream materialized views. If you need downstream cleanup, use a [Temporal Filter](/processing/sql/temporal-filters) in the downstream query.

<Note>
TTL cleanup is best-effort and happens asynchronously during compaction. Because watermark advancement is data-driven and non-deterministic, RisingWave does not guarantee a precise cutoff time (for example, that all data before time T is removed). Expired rows may remain visible or writable for a period of time, and the outcome of such operations is undefined.
</Note>
...
You can also implement processing-time TTL with a generated `PROCTIME()` column and a watermark on that column.

```sql
CREATE TABLE events (
  user_id VARCHAR,
  action VARCHAR,
  pt TIMESTAMPTZ AS PROCTIME(),
  WATERMARK FOR pt AS pt - INTERVAL '1' DAY WITH TTL
) APPEND ONLY
WITH (
  connector = 'kafka',
  topic = 'events',
