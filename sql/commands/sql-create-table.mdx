...
<Note>
Watermark advancement is non-deterministic, and TTL cleanup is performed asynchronously during compaction. Do not assume a strict cutoff (for example, that all data before time T has been removed). Visibility and modification of expired data are timing-dependent and therefore undefined.
</Note>
