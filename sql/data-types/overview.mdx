...
| vector(n) |     | Fixed-length vector with n dimensions for similarity search operations.  <br/>Used for storing and querying vector embeddings in machine learning applications. | Example: `vector(3)` → `'[1.0, 2.0, 3.0]'::vector(3)` |
