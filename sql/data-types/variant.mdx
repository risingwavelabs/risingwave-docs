...
* `VARIANT` cannot be used as a key type. This includes primary keys, index keys, `GROUP BY`, `DISTINCT`, `ORDER BY`, join keys, and deduplicating set operations such as `UNION`, `INTERSECT`, and `EXCEPT`. `UNION ALL` is supported.
