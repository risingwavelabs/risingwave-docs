...
RisingWave supports using `now()` as the `stop` parameter to continuously generate timestamps into a materialized view — for example, `generate_series('2025-01-01 00:00:00'::timestamptz, now(), interval '1 hour')`. The constraints are: `start` must be a constant `timestamptz` expression, `stop` must be exactly `now()`, and `step` must be a constant `interval` expression.

For examples, use cases, and limitations, see [Periodic query pattern](/processing/sql/periodic-query).
