...
Returns the unique identifier (UUID) of the current RisingWave cluster. This function is useful when configuring OAuth authentication, as the cluster ID is required in the JWT token's audience (`aud`) claim.
...
The returned UUID follows the standard format (8-4-4-4-12 hexadecimal digits separated by hyphens) and remains consistent throughout the cluster's lifetime.

For OAuth authentication setup, see [`CREATE USER`](/sql/commands/sql-create-user#oauth-audience-validation).
