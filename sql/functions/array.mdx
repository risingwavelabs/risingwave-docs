...
When working with arrays, prefer using array functions directly over unnesting in subqueries. Also, use `generate_series` with `unnest` for ordinality instead of the `WITH ORDINALITY` clause.
...
<Note>
**NULL handling:** If your array contains NULL values, `unnest` skips them while `generate_series` continues generating sequential numbers. In such cases where NULL handling is critical, consider using `WITH ORDINALITY` which maintains correct ordinality for NULL elements, or use the [`_pg_expandarray`](/sql/functions/set-returning#_pg_expandarray) function which also handles NULLs correctly.
</Note>
