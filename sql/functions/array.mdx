...
<Note>
The `generate_series` approach is generally preferred in RisingWave. However, note that if your array contains NULL values, `unnest` skips them while `generate_series` continues generating sequential numbers. In such cases where NULL handling is critical, consider using `WITH ORDINALITY` which maintains correct ordinality for NULL elements.
</Note>
