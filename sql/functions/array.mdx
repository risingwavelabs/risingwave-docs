...
**For aggregation:** If you want to unnest an array and then aggregate them back to a scalar value, consider using functions like `array_sum` against the array directly instead of using `unnest` and then `sum` to aggregate. Pure functions like `array_sum` have much better performance than relational level aggregation, which would introduce additional states.

**For unnesting:** If you want to unnest the array only, you should use `unnest` in the SELECT clause directly, instead of using it in a correlated subquery (which would introduce additional states). Especially, if you want to add ordinality to each unnested row, you can use `unnest` together with `generate_series`.
...
<Note>
In RisingWave, when multiple set-returning functions appear in the SELECT list, they are executed in a coordinated manner. This query will correctly pair each array element with its position.
</Note>
</Tab>
...
<Note>
**NULL handling:** If your array may contain NULL values, be aware that `unnest` skips them while `generate_series` continues generating sequential numbers, resulting in mismatched ordinality. For arrays with NULLs, use `WITH ORDINALITY` instead (which correctly handles NULL elements), or use the [`_pg_expandarray`](/sql/functions/set-returning#_pg_expandarray) function.
</Note>
