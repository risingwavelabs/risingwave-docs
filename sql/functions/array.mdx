...
### `array_overlaps`[](#array%5Foverlaps "Direct link to array_overlaps")

Checks whether two arrays share any non-NULL element.

Aliases: `array_is_overlap`, `array_is_intersect`

```sql
array_overlaps ( array, array ) → boolean
```

```sql
array_overlaps(array['a', 'b'], array['b', 'c']) → t

array_is_overlap(array['a', 'b'], array['c', 'd']) → f

array_is_intersect(array['a', 'b'], array['b', 'c']) → t
```
...
This operator checks if two arrays share any non-NULL element.

```sql
array[1,2,3] && array[3,4] → t

array[1,2,3] && array[4,5] → f

array[null]::int[] && array[null]::int[] → f
```
