...
<Note>
When using this approach with arrays containing NULL values, note that `unnest` skips NULL elements while `generate_series` continues generating sequential numbers, which may result in mismatched ordinality.
</Note>
</Tab>
