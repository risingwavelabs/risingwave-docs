...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 └─StreamProject { exprs: [JsonbAccessStr(t.payload, 'id':Varchar) as $expr1, array_agg(JsonbArrayElements(JsonbAccess($0, 'elements':Varchar))) filter(IsNotNull(1:Int32)), t._row_id, t.payload] }
   └─StreamHashJoin { type: LeftOuter, predicate: t.payload IS NOT DISTINCT FROM t.payload }
     ...
     └─StreamProject { exprs: [t.payload, array_agg(JsonbArrayElements(JsonbAccess($0, 'elements':Varchar))) filter(IsNotNull(1:Int32))] }
       └─StreamHashAgg { group_key: [t.payload], aggs: [array_agg(JsonbArrayElements(JsonbAccess($0, 'elements':Varchar))) filter(IsNotNull(1:Int32)), count] }
         └─StreamHashJoin { type: LeftOuter, predicate: t.payload IS NOT DISTINCT FROM t.payload }
           ├─StreamProject { exprs: [t.payload] }
           │ └─StreamHashAgg { group_key: [t.payload], aggs: [count] }
           │   └─StreamExchange { dist: HashShard(t.payload) }
           │     └─StreamTableScan { table: t, columns: [payload, _row_id] }
           └─StreamProject { exprs: [t.payload, JsonbArrayElements(JsonbAccess($0, 'elements':Varchar)), 1:Int32, projected_row_id] }
