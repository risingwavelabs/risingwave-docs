...
<Note>
If you're using `jsonb_array_elements` to do array unnesting for row-level processing, followed by an `array_agg` aggregation,
it can incur additional overhead due to subplan execution. Use `EXPLAIN` to examine your query plan, and see if there are unexpected `JOIN`s in your query.
If so, please consider rewriting with `array_transform` to perform your mapping logic to avoid the subplan execution.
</Note>

For example, given this original SQL:
```sql
create table t(key int, payload jsonb);
explain create materialized view m1 as 
  select payload->>'id' as id,
  (select array_agg(value->>'subField' order by ordinality) from jsonb_array_elements(payload->'elements') with ordinality as value) as elements
from t;
```

We can see the query plan contains a join for subplan execution:
```
                                                                                                         QUERY PLAN                                                                                                         
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 StreamMaterialize { columns: [id, elements, t._row_id(hidden), t.payload(hidden)], stream_key: [t._row_id, t.payload], pk_columns: [t._row_id, t.payload], pk_conflict: NoCheck }
 └─StreamProject { exprs: [JsonbAccessStr(t.payload, 'id':Varchar) as $expr3, array_agg($expr1 order_by($expr2 ASC)) filter(IsNotNull(1:Int32)), t._row_id, t.payload] }
   └─StreamHashJoin { type: LeftOuter, predicate: t.payload IS NOT DISTINCT FROM t.payload }
     ├─StreamExchange { dist: HashShard(t.payload) }
     │ └─StreamTableScan { table: t, columns: [payload, _row_id] }
     └─StreamProject { exprs: [t.payload, array_agg($expr1 order_by($expr2 ASC)) filter(IsNotNull(1:Int32))] }
       └─StreamHashAgg { group_key: [t.payload], aggs: [array_agg($expr1 order_by($expr2 ASC)) filter(IsNotNull(1:Int32)), count] }
         └─StreamHashJoin { type: LeftOuter, predicate: t.payload IS NOT DISTINCT FROM t.payload }
           ├─StreamProject { exprs: [t.payload] }
           │ └─StreamHashAgg { group_key: [t.payload], aggs: [count] }
           │   └─StreamExchange { dist: HashShard(t.payload) }
           │     └─StreamTableScan { table: t, columns: [payload, _row_id] }
           └─StreamProject { exprs: [t.payload, JsonbAccessStr(JsonbArrayElements(JsonbAccess($0, 'elements':Varchar)), 'subField':Varchar) as $expr1, (projected_row_id + 1:Int64) as $expr2, 1:Int32, projected_row_id] }
             └─StreamProjectSet { select_list: [$0, JsonbArrayElements(JsonbAccess($0, 'elements':Varchar))] }
               └─StreamProject { exprs: [t.payload] }
                 └─StreamHashAgg { group_key: [t.payload], aggs: [count] }
                   └─StreamExchange { dist: HashShard(t.payload) }
                     └─StreamTableScan { table: t, columns: [payload, _row_id] }
...
If we rewrite it with array_transform, the plan is greatly simplified:
```sql
explain create materialized view m1 as 
  select payload->>'id' as id,
  array_transform(jsonb_to_array(payload->'elements'), |x| x->>'subField') as elements
from t;
```

Query plan after rewrite:
```
                                                                                                          QUERY PLAN                                                                                                          
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 StreamMaterialize { columns: [id, elements, t._row_id(hidden)], stream_key: [t._row_id], pk_columns: [t._row_id], pk_conflict: NoCheck }
 └─StreamProject { exprs: [JsonbAccessStr(t.payload, 'id':Varchar) as $expr1, ArrayTransform(JsonbToArray(JsonbAccess(t.payload, 'elements':Varchar)), JsonbAccessStr(t.payload, 'subField':Varchar)) as $expr2, t._row_id] }
   └─StreamTableScan { table: t, columns: [payload, _row_id] }
(3 rows)
```
...
Converts the a JSONB array to a SQL array of JSONB elements. It is functionally equivalent to [`jsonb_array_elements`](/sql/functions/json#jsonb-array-elements) followed by `array_agg` but simpler to use. `jsonb_to_array` preserves the original element order and accurately distinguishes between `null` and an empty array.
...
```sql
SELECT jsonb_to_array('[1,true,[2,false]]'::jsonb);
------RESULT
{1,true,"[2, false]"}
```
