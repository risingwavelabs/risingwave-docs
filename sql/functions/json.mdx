...
<Note>
If you're using `jsonb_array_elements` to unnest arrays for row-level processing and then apply an `array_agg` aggregation,
it can incur additional overhead due to subplan execution. Use `EXPLAIN` to examine your query plan and check for unexpected JOINs.
If such joins appear, consider rewriting the query using `array_transform` to perform your mapping logic and avoid subplan execution.
</Note>
