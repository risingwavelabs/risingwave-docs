...
Note: If you're using `jsonb_array_elements` to do array unnesting for row-level processing, followed by an `array_agg` aggregation,
...
explain create materialized view m1 as 
  ...
  (select array_agg(value) from (select jsonb_array_elements(payload->'elements') as value)) as elements
from t;
```
