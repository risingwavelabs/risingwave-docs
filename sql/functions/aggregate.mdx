...
<Note>
Note that `approx_percentile` can be more performant than `percentile_cont` for streaming jobs where groups in the `GROUP BY` clause have high row counts.
</Note>
...
<Note>
Note that `approx_percentile` can be more performant than `percentile_disc` for streaming jobs where groups in the `GROUP BY` clause have high row counts.
</Note>
...
Returns an approximate value of the specified percentile from a numeric column. For performance-sensitive streaming percentile workloads with high-volume groups, `approx_percentile` is generally preferred over `percentile_cont` or `percentile_disc` when approximate results are acceptable.
