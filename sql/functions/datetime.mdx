...
Returns the current wall-clock date and time at expression evaluation time.
...
```sql
clock_timestamp() → '2026-07-22 07:41:48.123456+00:00'
```
...
This example is valid on current RisingWave releases. Because `proctime()` is an impure expression, a generated column defined with it cannot be part of the primary key. For details, see [Generated columns](/sql/query-syntax/generated-columns).
...
| TZH or tzh         | Time-zone hours. This pattern works only in to\_timestamp. |
