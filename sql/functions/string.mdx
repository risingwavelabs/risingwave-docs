...
* `I`: Treats the argument value as a SQL identifier.
...
Returns the given string properly quoted, so that a string can be safely used as a string literal in a SQL statement. This involves doubling any embedded single-quotes and backslashes. Note that if the input string is null, the function `quote_literal` returns null. In such cases, the function [quote\_nullable](#quote%5Fnullablestring-text) is often a better choice. Note that the quotes are part of the output string.
...
Converts the given value to text and then quotes it as a literal to be safely used in a SQL statement, similar to [`quote_literal(string text)`](#quote%5Fliteralstring-text). This involves doubling any embedded single-quotes and backslashes to ensure their proper representation.
...
Returns the given string properly quoted, so that a string can be safely used as a string literal in a SQL statement. Returns NULL if the input string is null. This involves doubling any embedded single-quotes and backslashes. When the argument is null, this function is usually more suitable than the `quote_literal` function.
