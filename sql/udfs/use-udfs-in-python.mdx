...
Then start a gRPC-aware (L7) load balancer, such as Envoy or Nginx with gRPC proxying (`grpc_pass`), that listens on port 8815 and routes requests to ports 8816-8819. The balancer must route individual gRPC calls, not TCP connections: each RisingWave process keeps a long-lived HTTP/2 connection to the balancer, so a TCP (L4) proxy pins all its calls to a single server.
