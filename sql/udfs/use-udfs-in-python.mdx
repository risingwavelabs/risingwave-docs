...
Then start a gRPC-aware (L7) load balancer, such as Envoy or Nginx with gRPC proxying (`grpc_pass`), that listens on port 8815 and routes requests to ports 8816-8819. The balancer must route individual gRPC calls, not TCP connections: when a UDF link points to one balancer address, each RisingWave process keeps a long-lived HTTP/2 connection to that address, and a TCP (L4) proxy pins calls on that connection to a single server.

On Kubernetes, you can skip the proxy: run the UDF servers as multiple pods, each listening on the same port, behind a headless Service (`clusterIP: None`) and point `USING LINK` at it, e.g. `http://<service>.<namespace>.svc.cluster.local:8815`. RisingWave resolves all pod IPs and balances calls across them on the client side, periodically refreshing DNS to track scaling.
