...
# import openAI and set your API key if you want to use the openAI API
import openai
...
# Define get_completion function to use OpenAI API
...
# Define a function to use the get_completion function

@udf(input_types=[], result_type='VARCHAR')
def get_a_joke():
    prompt = "Please give a joke in one sentence"
...
if __name__ == '__main__':
    ...
    server.add_function(get_a_joke)
    server.serve()
```
<Accordion
...
The code defines four scalar functions and one table function:
...
* The scalar function `get_a_joke`, decorated with `@udf`, provides a prompt and returns a joke.
...
CREATE FUNCTION get_a_joke() RETURNS varchar
LANGUAGE python AS get_a_joke USING LINK 'http://localhost:8815'; -- If you are running RisingWave using Docker, replace the address with 'http://host.docker.internal:8815'.
```
