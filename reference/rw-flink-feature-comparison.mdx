...
sidebarTitle: "RisingWave vs. Flink: feature comparison"
...
|:--------------------|:---------------------|:------------------------|:---------------------------------------------------|
| **Fixed-Length**    |                      |                         |                                                   |
| Fixed-Length Char   | `CHAR`               | `VARCHAR`               | RisingWave follows PostgreSQL: use `VARCHAR`, which avoids fixed-length padding and is more flexible. |
| Fixed-Length Binary | `BINARY`             | `BYTEA`                 | RisingWave uses the PostgreSQL-compatible `BYTEA` for binary data. |
| **Variable-Length** |                      |                         |                                                   |
| Variable-Length Char| `VARCHAR`/`STRING`   | `VARCHAR`/`STRING`      | Both supported. RisingWave follows the PostgreSQL standard where `VARCHAR` without a length limit is the recommended form. |
...
| 1-Byte Integer      | `TINYINT`            | `SMALLINT`              | RisingWave follows PostgreSQL: use `SMALLINT` (2-byte) for small integer values. |
| 2-Byte Integer      | `SMALLINT`           | `SMALLINT`              |                                                   |
| 4-Byte Integer      | `INT`                | `INT`                   |                                                   |
| 8-Byte Integer      | `BIGINT`             | `BIGINT`                |                                                   |
| 32-Byte Integer     | Not Supported        | `RW_INT256`             | RisingWave-specific large integer type with no Flink equivalent. |
...
| Timestamp (with timezone) | `TIMESTAMP_LTZ`      | `TIMESTAMPTZ`     | Flink uses `TIMESTAMP_LTZ` (local timezone interpretation). RisingWave uses the PostgreSQL-compatible `TIMESTAMPTZ` (stored as UTC, unambiguous). |
...
*   **Pattern Matching:** Flink provides the `MATCH_RECOGNIZE` clause for Complex Event Processing (CEP). RisingWave approaches CEP via materialized views, temporal filters, and window aggregations — patterns that are often more readable and maintainable for streaming SQL users.

| Feature           | Flink (v1.20) | RisingWave (v2.0) | Notes                                        |
|:------------------|:--------------|:------------------|:----------------------------------------------|
| `DISTINCT ON`     | Limited/No    | Supported         | PostgreSQL-style unique row selection — a concise way to get the first row per group, not directly available in Flink SQL. |
...
*   **Flink SQL (1.20)** is a stream processing framework with strong Hive integration and features like `TRY_CAST`. As a framework, it requires external state backends, separate storage, and additional tooling for serving and access control.
*   **RisingWave SQL (2.0)** offers a **PostgreSQL-compatible, integrated streaming database experience**. Key advantages include **incrementally maintained materialized views as a core architectural concept**, native `JSONB` handling, **unified SQL DDL/DML for sources, sinks, and access control**, automatic internal state management, and direct query serving via the PostgreSQL wire protocol — all in a single system.

For teams that want a complete stream processing and serving solution without managing multiple systems, RisingWave offers a significantly simpler operational model.
  ...
  <script type="application/ld+json">
    {`
    {
      ...
      "mainEntity": [
        {
          "@type": "Question",
          "name": "How does RisingWave SQL compare to Flink SQL?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "RisingWave uses PostgreSQL-compatible SQL with native JSONB support, cascading materialized views, and unified DDL for sources, sinks, and access control. Flink SQL uses its own dialect with MATCH_RECOGNIZE for complex event processing, TRY_CAST, and strong Hive catalog integration. RisingWave manages state automatically; Flink requires external state backend configuration."
          }
        },
        {
          "@type": "Question",
          "name": "Does RisingWave support the same SQL features as Flink?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Both support streaming SQL with joins, aggregations, and window functions. RisingWave has advantages in PostgreSQL compatibility, native JSONB handling, and cascading materialized views. Flink has MATCH_RECOGNIZE for complex event processing and TRY_CAST for error-tolerant type conversion. Both support user-defined functions, but with different language support."
          }
        }
      ]
    }
