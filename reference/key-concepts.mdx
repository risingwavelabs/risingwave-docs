...
Compute Nodes are responsible for ingesting data from upstream systems, executing the streaming query graph incrementally, and delivering results to downstream systems. Each Compute Node runs a set of streaming actors that process partitions of the data stream in parallel. Compute Nodes are stateless in terms of durable data — all persistent state is stored in the Hummock storage engine backed by object storage — which means they can be added or removed without data migration.
