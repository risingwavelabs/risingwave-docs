...
<Frame>
  <img src="/images/current/risingwave-flink-comparison/RisingWave-vs-Flink.png"/>
</Frame>

## Introduction

_Apache Flink is a distributed stream processing framework; RisingWave is a distributed SQL streaming database._
...
_When developing applications using Apache Flink, developers need to manage multiple systems and handle the consistency relationships between them. When using RisingWave, developers only need to manage a single system and do not need to consider any relationships between different system components._
...
<Frame>
  <img src="/images/current/risingwave-flink-comparison/stream_processing_without_risingwave.jpeg"/>
</Frame>
...
<Frame>
  <img src="/images/current/risingwave-flink-comparison/stream_processing_with_risingwave.jpeg"/>
</Frame>

## Architecture

_Apache Flink adopts a big-data style, coupled-compute-storage architecture that is optimized for scalability; RisingWave in contrast implements a cloud-native, decoupled compute-storage architecture that is optimized for cost efficiency._
<Frame>
  <img src="/images/current/risingwave-flink-comparison/architecture_comparison.png"/>
</Frame>
...
_Apache Flink offers both low-level Java, Scala, and Python APIs and a high-level SQL interface; RisingWave in contrast offers PostgreSQL-style SQL as the user interface._
...
_Apache Flink uses RocksDB to maintain local states, which are periodically checkpointed to remote storage such as S3; RisingWave by design is a database and has its own storage to persist internal state and data in S3 or equivalent storage services._
...
_Apache Flink was not designed for serving ad-hoc queries; RisingWave by design is a database and can serve concurrent ad-hoc queries._
...
_Apache Flink delivers consistent and complete results to downstream systems; RisingWave offers even higher correctness guarantee, as it not only ensures consistency and completeness, but also provides consistent snapshots for any data access._
...
_Apache Flink offers dozens of connectors to the systems in the big data world; in contrast, RisingWave can be well integrated into not only big data and cloud ecosystems but also the PostgreSQL ecosystem._
...
_Apache Flink has a steep learning curve as it offers more details in low-level control; In contrast, RisingWave has a minimal learning curve and is simple and easy to use._
...
_Apache Flink has a high maintenance cost due to its complex configuration; RisingWave has a much lower maintenance cost as it extensively leverages hosted cloud services._
...
_To achieve the target performance, Apache Flink is at a higher cost due to its heavy architectural design; On the other hand, RisingWave is much more cost-efficient with its disaggregated architecture._
...
_Apache Flink is suitable for comprehensive streaming ETL and streaming analytics applications; RisingWave not only supports streaming ETL and analytics applications, but can also do online serving with its built-in capability._
...
RisingWave offers not only streaming ETL and streaming analytics, but also online serving capabilities through its built-in batch query engine. By defining materialized views, analytical results of input streams can be gathered and served directly in RisingWave. Its distributed serving cluster supports high-concurrency queries with horizontal scalability.
  ...
  <script type="application/ld+json">
    {`
    {
      ...
      "mainEntity": [
        {
          "@type": "Question",
          "name": "What is the difference between RisingWave and Apache Flink?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "RisingWave is a PostgreSQL-compatible streaming database with built-in storage and query serving. Apache Flink is a distributed stream processing framework that requires external storage and serving layers. RisingWave uses SQL for all operations; Flink offers both SQL and Java/Scala APIs. RisingWave stores state in object storage for cost efficiency, while Flink uses RocksDB on local disk."
          }
        },
        {
          "@type": "Question",
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes, for most SQL-based stream processing use cases. RisingWave supports comprehensive SQL features including the full TPC-H benchmark, cascading materialized views, and built-in query serving — eliminating the need for a separate serving database. However, Flink may be preferred when you need low-level Java/Scala API access or deep integration with the JVM ecosystem."
          }
        },
        {
          "@type": "Question",
          "name": "Is RisingWave easier to operate than Apache Flink?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes. RisingWave is a database that manages its own storage and serves queries directly, so you deploy one system instead of Flink plus a separate database. There are no JARs to compile, no job graphs to manage, and no external state backends to configure. Streaming pipelines are created with SQL statements, not application deployments."
          }
        }
      ]
    }
