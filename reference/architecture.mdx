...
The meta node manages the cluster metadata by working with a metastore (a highly available SQL database like AWS RDS). All database objects are persisted in the meta store. The serving node retrieves the database catalog from the meta node, and caches it locally to serve the queries.
