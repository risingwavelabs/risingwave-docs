...
The **meta node** takes charge of managing the metadata of compute and compactor nodes and orchestrating operations across the system. Here metadata refers to database names, table names, column names, materialized view names, computation progress, and so on. It acts as the brain of the entire system; loss of metadata could lead to data loss or irrecoverability across the cluster. Starting with v2.1.0, RisingWave fully deprecates the etcd meta backend and uses SQL backend to scale metadata. For more information, see the [migration guide](/deploy/migrate-to-sql-backend).
