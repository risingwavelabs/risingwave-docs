---
title: "Architecture"
description: "Written for developers who want a deeper understanding of RisingWave Architecture,
              It covers the various nodes, their responsibilities and relationships with each other, according to our product architecture."
...
> If you’re new to RisingWave, it will be helpful to understand what it is first: https://docs.risingwave.com/get-started/intro.

RisingWave has several main components: Serving, Streaming, Compactor and the Meta service. At its core, RisingWave is a **unified batch and stream database**.
...
Hence it is typically recommended to keep a 2:1 ratio between compute and compactor sizes. For instance with a 4 cpu core, 16 GB memory compute node, we should be using a 2 cpu core, 8 GB compactor node. In a write heavy workload, for instance where 500K rows are written to storage per second, a 1:8 compute:compactor ratio is recommended. This can typically happen during backfill of historical data, where the ingestion size is really large. You can check the section: https://docs.risingwave.com/performance/specific-bottlenecks#write-stall on how to diagnose compaction bottlenecks.
...
- [faq-using-risingwave](./faq-using-risingwave.mdx)
- [performance-faq](./performance-faq.mdx)