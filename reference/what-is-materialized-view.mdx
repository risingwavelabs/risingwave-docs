...
- [Periodic query pattern with generate\_series](/sql/functions/set-returning#periodic-query-pattern-limited-cron-like-alternative) — Limited cron-like alternative using `generate_series(..., now(), interval ...)` inside a materialized view
