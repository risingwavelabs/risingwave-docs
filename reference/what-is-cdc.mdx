...
For PostgreSQL, MySQL, and SQL Server, the recommended approach is to create a **shared source** and then create derived tables from it:

```sql
-- 1. Create a shared CDC source
...
-- 2. Create a derived table from the shared source
-- Note: primary key must match the upstream table's primary key
CREATE TABLE orders (
...
3. **Maintains transactional consistency** — changes from the same upstream transaction are applied consistently across all derived tables.
...
A single shared source can feed multiple derived tables. This means one replication connection and one replication slot serve all the tables you want to replicate:

```sql
-- Create additional derived tables from the same shared source
CREATE TABLE customers (*) FROM pg_source TABLE 'public.customers';
CREATE TABLE products (*) FROM pg_source TABLE 'public.products';
```

Shared sources use a single replication slot on the source database, reducing the load on the upstream system. For PostgreSQL, `slot.name` is optional — RisingWave generates and manages a unique replication slot automatically when omitted.
