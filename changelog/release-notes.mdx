...
- Compaction output SST filters now default to the `blocked` layout (block-based filters) for all non-L0 levels. L5 and L6 now default to `xor8` filters, while L0 keeps the `auto` layout and `xor16` filter kind. [#25853](https://github.com/risingwavelabs/risingwave/pull/25853)
