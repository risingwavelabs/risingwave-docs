...
- Fixes incorrect results in correlated scalar aggregate queries by preserving empty aggregate rows and `LATERAL` join conditions. For batch queries, the optimizer now evaluates random samples consistently and rejects plans that would otherwise re-evaluate nondeterministic outer expressions or row limits. [#27065](https://github.com/risingwavelabs/risingwave/pull/27065)
