...
This release adds the `VARIANT` data type, periodically refreshed materialized views, partitioned gap filling, and more control over backfill and sink operations. It also includes the improvements and fixes documented in the v3.0.x patch releases below.

## Breaking changes and deprecations

- WebSocket ingestion now requires a license that includes `WebSocketIngest`. Before upgrading self-managed deployments using this feature, configure a license covering it; otherwise new or re-established WebSocket connections are rejected with HTTP 403. HTTP webhook ingestion is unaffected. [#26519](https://github.com/risingwavelabs/risingwave/pull/26519)
- Removes direct CDC table creation with the `mysql-cdc`, `postgres-cdc`, and `sqlserver-cdc` connectors. Create a shared CDC source first, then create tables with `CREATE TABLE ... FROM source TABLE '...'`. MongoDB CDC is unaffected. [#25682](https://github.com/risingwavelabs/risingwave/pull/25682)
- Changes Parquet `VARIANT` columns from their raw `struct<metadata bytea, value bytea>` representation to the native `variant` type. Update explicitly declared Parquet schemas to `variant`; declarations using the old raw struct now produce NULL for that column. `file_scan()` also infers `variant`, so queries accessing `.metadata` or `.value` must use variant functions or casts instead. [#25165](https://github.com/risingwavelabs/risingwave/pull/25165)
- Deprecates creating new jobs with no-shuffle backfill. Use snapshot backfill or locality backfill instead. [#25963](https://github.com/risingwavelabs/risingwave/pull/25963)
- Disables local filesystem connectors such as `posix_fs` and `fs` by default in release builds. Deployments that intentionally use these connectors must enable `frontend.unsafe_enable_local_fs_connector`. [#25971](https://github.com/risingwavelabs/risingwave/pull/25971)

## SQL features

- Adds the `VARIANT` data type for semi-structured values, with `to_variant`, `variant_get`, `try_variant_get`, `variant_typeof`, and casts to and from `varchar` and `jsonb`. `VARIANT` cannot be used as a key or in grouping, ordering, or join conditions. [#26080](https://github.com/risingwavelabs/risingwave/pull/26080)
- Supports materialized views that consume an upstream snapshot and refresh periodically with `refresh.interval.sec`. These views require snapshot backfill and cannot have downstream streaming jobs. [#25093](https://github.com/risingwavelabs/risingwave/pull/25093), [#25271](https://github.com/risingwavelabs/risingwave/pull/25271)
- Supports independent gap filling for each partition with `GAP_FILL(..., PARTITION_BY(...))`, including `EMIT ON WINDOW CLOSE`. The window-close implementation now preserves partition columns and keeps previous-row state separate for each partition. [#25287](https://github.com/risingwavelabs/risingwave/pull/25287), [#25350](https://github.com/risingwavelabs/risingwave/pull/25350)
- Adds `clock_timestamp()` and `gen_random_uuid()` for the current wall-clock timestamp and a random UUID v4 string, respectively. [#26362](https://github.com/risingwavelabs/risingwave/pull/26362)
- Allows runtime configuration, parallelism, resource-group, and rate-limit changes while a streaming job is still being created. Metadata changes, such as renaming an object, still require creation to finish. [#26104](https://github.com/risingwavelabs/risingwave/pull/26104)
- Supports standard HTTP and HTTPS endpoints in UDF `USING LINK` clauses. [#26792](https://github.com/risingwavelabs/risingwave/pull/26792)
- Fixes qualified wildcard resolution for schema-qualified relations and repeated scalar subqueries in `ORDER BY`. [#26419](https://github.com/risingwavelabs/risingwave/pull/26419), [#25686](https://github.com/risingwavelabs/risingwave/pull/25686)

## Connectors

- Adds `REPLACE SINK ... FROM ...` to switch an existing sink at a cut-over barrier without backfilling historical rows. Exactly-once sinks, sink-into-table, and `AS query` replacements are not supported; privileges on the old sink are not transferred. [#26039](https://github.com/risingwavelabs/risingwave/pull/26039)
- Supports projection and filter queries in sinks created with `snapshot = false`. The `since_timestamp` option can replay retained change logs from a specified timestamp when snapshot backfill is disabled; this option requires a premium license. [#26023](https://github.com/risingwavelabs/risingwave/pull/26023), [#26043](https://github.com/risingwavelabs/risingwave/pull/26043)
- Supports dynamic HTTP sink URLs derived from an upstream column. [#25699](https://github.com/risingwavelabs/risingwave/pull/25699)
- Supports Google Application Default Credentials for Pub/Sub sources and sinks in self-hosted and BYOC deployments when explicit credentials and emulator configuration are absent. RisingWave Cloud Hosted still requires explicit connector credentials. [#26808](https://github.com/risingwavelabs/risingwave/pull/26808)
- Supports PostgreSQL CDC arrays of user-defined composite types as `varchar[]`, and improves NULL handling for scalar composite values. [#25818](https://github.com/risingwavelabs/risingwave/pull/25818), [#25815](https://github.com/risingwavelabs/risingwave/pull/25815)
- Supports Pulsar message properties through `INCLUDE header` columns. [#25794](https://github.com/risingwavelabs/risingwave/pull/25794)
- Supports writing RisingWave `jsonb` values to Doris `VARIANT` columns and `timestamptz` values to supported StarRocks datetime or string columns using the sink's captured session time zone. [#25451](https://github.com/risingwavelabs/risingwave/pull/25451), [#25837](https://github.com/risingwavelabs/risingwave/pull/25837)
- Supports serverless tasks for Snowflake upsert sinks and a `num_shards` option for new Turbopuffer namespaces. [#25090](https://github.com/risingwavelabs/risingwave/pull/25090), [#26197](https://github.com/risingwavelabs/risingwave/pull/26197)

## Iceberg

- Adds primary-key-index upsert sinks for Iceberg V2 and V3 merge-on-read tables through `enable_pk_index`. V2 uses position-delete files; V3 uses deletion vectors. [#25416](https://github.com/risingwavelabs/risingwave/pull/25416), [#26116](https://github.com/risingwavelabs/risingwave/pull/26116)
- Supports reading Iceberg v3 `VARIANT` columns, including nested variants, through Iceberg sources and refreshable tables. Shredded variant columns currently read as NULL with a log warning. Writing native `VARIANT` values through Iceberg sinks is not supported. [#25165](https://github.com/risingwavelabs/risingwave/pull/25165)
- Adds per-object metadata relations named `<name>$snapshots`, `<name>$manifests`, and `<name>$files` for Iceberg Engine tables, sources, and sinks. [#26466](https://github.com/risingwavelabs/risingwave/pull/26466)
- Adds manifest rewrite maintenance and enables it by default for newly created Iceberg Engine V2 tables. [#26285](https://github.com/risingwavelabs/risingwave/pull/26285), [#26471](https://github.com/risingwavelabs/risingwave/pull/26471)
- Adds `compaction.type = 'auto'` for merge-on-read sinks to select small and delete-heavy files for compaction. Copy-on-write sinks continue to require full compaction. [#26643](https://github.com/risingwavelabs/risingwave/pull/26643), [#26898](https://github.com/risingwavelabs/risingwave/pull/26898)

## Administration and observability

- Enables automatic locality backfill selection for new clusters, with a default estimated input-size threshold of 10 GiB. Existing clusters retain their persisted settings. [#25990](https://github.com/risingwavelabs/risingwave/pull/25990)
- Adds typed fragment rate limits for source ingestion, backfill, DML, and sink throughput through `ALTER FRAGMENT`. [#26183](https://github.com/risingwavelabs/risingwave/pull/26183)
- Allows an initial cache-refill policy to be set with `streaming_cache_refill_policy` before creating a streaming job. [#26603](https://github.com/risingwavelabs/risingwave/pull/26603)
- Supports bootstrapping streaming parallelism defaults from the `[session_init]` section of `risingwave.toml`. Persisted session defaults take precedence. [#25864](https://github.com/risingwavelabs/risingwave/pull/25864)
- Reports barrier metrics by partial graph, groups diagnostic actor traces by fragment, and includes indexes in `rw_fragment_id_to_ddl`. [#26495](https://github.com/risingwavelabs/risingwave/pull/26495), [#25833](https://github.com/risingwavelabs/risingwave/pull/25833), [#25903](https://github.com/risingwavelabs/risingwave/pull/25903)
- Redacts passwords in parsed `CREATE USER` and `ALTER USER` statements in server-side query logs. This does not redact arbitrary SQL values or statements that cannot be parsed. [#25740](https://github.com/risingwavelabs/risingwave/pull/25740)

## Bug fixes

- Enforces the PostgreSQL wire-protocol authentication sequence before accepting regular client messages. [#26825](https://github.com/risingwavelabs/risingwave/pull/26825)
- Preserves CDC changes to already-scanned rows during backfill recovery startup, including while reconnecting to the upstream database. [#27031](https://github.com/risingwavelabs/risingwave/pull/27031)
- Preserves composite primary-key order during MySQL and SQL Server CDC schema discovery and snapshot reads. [#26847](https://github.com/risingwavelabs/risingwave/pull/26847), [#27160](https://github.com/risingwavelabs/risingwave/pull/27160)
- Prevents a database recovery loop when a batch-refresh materialized view is created just before a compute-node failure. [#26786](https://github.com/risingwavelabs/risingwave/pull/26786)
- Fixes missing or incorrectly updated rows when a `FULL_RELOAD` refresh deletes a row and a later refresh adds the same key again. [#27040](https://github.com/risingwavelabs/risingwave/pull/27040)
- Uses the effective database barrier interval when advancing streaming `now()`. [#27118](https://github.com/risingwavelabs/risingwave/pull/27118)
- Avoids repeated timestamp-offset lookups for Kafka partitions, reducing broker work when startup timestamps require tiered-storage reads. [#27030](https://github.com/risingwavelabs/risingwave/pull/27030)
- Fixes Pulsar acknowledgments across multiple splits and actors, and Pub/Sub streaming-pull flow-control stalls. [#26892](https://github.com/risingwavelabs/risingwave/pull/26892), [#26826](https://github.com/risingwavelabs/risingwave/pull/26826), [#26817](https://github.com/risingwavelabs/risingwave/pull/26817)
- Avoids reading unrelated table blocks during SST scans and point lookups. [#27048](https://github.com/risingwavelabs/risingwave/pull/27048)
- Fixes a temporal join crash that could occur when replicated state table writes received a projected chunk with invisible rows. [#26345](https://github.com/risingwavelabs/risingwave/pull/26345)
- Prevents a no-op `ALTER SINK ... CONNECTOR WITH` update from crashing a compute node when the sink uses a commit-decoupled KV log store. [#26412](https://github.com/risingwavelabs/risingwave/pull/26412)
- Fixes a possible meta-node crash during subscription cursor reads. If the requested start epoch is ahead of the table's committed epoch, the cursor waits for a newer committed epoch. Other requests with inverted epoch ranges are rejected instead of crashing. [#26478](https://github.com/risingwavelabs/risingwave/pull/26478)
- Validates explicitly configured `debezium.heartbeat.interval.ms` values consistently across all CDC connectors. The value must be a positive signed 32-bit integer (`1` to `2147483647` milliseconds). If omitted, connector defaults on creation and the existing value on alteration remain unchanged. [#27017](https://github.com/risingwavelabs/risingwave/pull/27017)

## Cluster configuration changes

- Bounds pending barriers in snapshot-backfill partial graphs using `in_flight_barrier_nums` and `snapshot_backfill_barrier_amplification_factor`. Their defaults are 10,000 and 1, respectively. [#26467](https://github.com/risingwavelabs/risingwave/pull/26467)
- The SSTable filter layout for compaction output now defaults to `blocked` for L1-L6, while L0 keeps `auto`. The filter kind defaults to `xor8` for L5 and L6; L0-L4 keep `xor16`. [#25853](https://github.com/risingwavelabs/risingwave/pull/25853)
- When `sstable_filter_layout[level]` is `auto`, `blocked_xor_filter_kv_count_threshold` is evaluated per output SSTable instead of per compaction task. [#25782](https://github.com/risingwavelabs/risingwave/pull/25782)
- Compaction-group SST filter configuration now uses explicit `sstable_filter_type` (`none`, `xor16`, `xor8`) and canonical `sstable_filter_layout` values (`auto`, `plain`, `blocked`). Setting `sstable_filter_type` to `none` disables SST filters for that target level, and `rw_catalog.rw_hummock_sstables` now exposes `filter_type` and `filter_layout`. [#25832](https://github.com/risingwavelabs/risingwave/pull/25832)

See the **Full Changelog** [here](https://github.com/risingwavelabs/risingwave/compare/v3.0.0...v3.1.0).

</Update>

<Update label="v3.0.4" description="2026-09-16">

## SQL features

- Streams query results incrementally over the PostgreSQL wire protocol to reduce frontend memory usage and deliver initial rows sooner. The `frontend.stream_flush_threshold_bytes` configuration controls flushing and defaults to 64 KiB. [#26782](https://github.com/risingwavelabs/risingwave/pull/26782)
- Preserves partial-update columns in `CREATE SINK ... INTO` targets when a downstream key moves between stream keys within the same barrier interval. [#26759](https://github.com/risingwavelabs/risingwave/pull/26759)
- Invalidates cached rows deleted during refresh, preventing stale materialized results when rows are later reinserted. [#27057](https://github.com/risingwavelabs/risingwave/pull/27057)

## Connectors

- Allows CDC table `snapshot.batch_size` and `snapshot.interval` options to be altered. The updated options take effect after `RECOVER`. [#26757](https://github.com/risingwavelabs/risingwave/pull/26757)
- Preserves upstream MySQL composite-primary-key order during schema discovery and handles column-name casing consistently during automatic schema changes. [#26757](https://github.com/risingwavelabs/risingwave/pull/26757)
- Skips creating an external snapshot reader when CDC backfill is disabled with `snapshot = false`, including when recovering completed backfill state. [#26983](https://github.com/risingwavelabs/risingwave/pull/26983)
- Fixes Pulsar source acknowledgments by isolating acknowledgment channels for each actor and acknowledging every split at checkpoints. [#26910](https://github.com/risingwavelabs/risingwave/pull/26910), [#26956](https://github.com/risingwavelabs/risingwave/pull/26956)
- Resolves Kafka timestamp-based startup offsets once per partition during an enumerator's lifetime, avoiding repeated broker lookups. [#27052](https://github.com/risingwavelabs/risingwave/pull/27052)
- Handles exactly-once metadata cleanup when a sink is dropped during an in-flight commit, avoiding errors caused by the removed sink state. [#27096](https://github.com/risingwavelabs/risingwave/pull/27096)

## Iceberg

- Publishes copy-on-write compaction results across partitions atomically and avoids rewriting data files unaffected by deletes. [#26736](https://github.com/risingwavelabs/risingwave/pull/26736), [#26757](https://github.com/risingwavelabs/risingwave/pull/26757)
- Adds memory-aware scheduling for dedicated Iceberg compactors. `storage.iceberg_compaction_memory_limit_mb` sets the estimated memory budget for admitting tasks; when unset, the budget is derived from available compactor memory. This is not a hard allocation limit. `storage.iceberg_compaction_max_pull_task_count` limits tasks requested per pull and defaults to `1`. [#26757](https://github.com/risingwavelabs/risingwave/pull/26757)
- Adds the `auto` compaction strategy, which selects small files and files with heavy deletes. It becomes the default when Iceberg compaction is licensed; otherwise, the default remains `full`. Copy-on-write selects its policy automatically: omit `compaction.type` when creating a copy-on-write sink; existing persisted values are ignored. [#26757](https://github.com/risingwavelabs/risingwave/pull/26757)
- Prunes old deletion-only manifests and merges manifests during copy-on-write publication, reducing metadata growth without an additional data-file rewrite. [#26990](https://github.com/risingwavelabs/risingwave/pull/26990)

## Administration

- Enforces the PostgreSQL password authentication flow before accepting regular protocol messages. [#26890](https://github.com/risingwavelabs/risingwave/pull/26890)
- Moves Hummock version checkpoint creation to a dedicated runtime to avoid blocking the main meta runtime, and reduces meta-store queries during commit-epoch validation. [#26783](https://github.com/risingwavelabs/risingwave/pull/26783), [#26784](https://github.com/risingwavelabs/risingwave/pull/26784)
- Keeps newly added workers tracked for background reconnection when initial control-stream connection attempts fail. [#27014](https://github.com/risingwavelabs/risingwave/pull/27014)

## Observability

- Adds MySQL CDC binlog file lag and retention-risk dashboard panels and an alert indicator. Fixes binlog sequence metrics for non-default file prefixes, including those used by RDS and Aurora. [#26964](https://github.com/risingwavelabs/risingwave/pull/26964), [#26728](https://github.com/risingwavelabs/risingwave/pull/26728)
- Adds metrics and dashboard panels for the Iceberg compactor's scheduling memory budget and estimated memory reserved by running tasks. [#26757](https://github.com/risingwavelabs/risingwave/pull/26757)

## Assets

- Run this version from Docker:
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v3.0.4 single_node`
- [Prebuilt all-in-one library for Linux](https://github.com/risingwavelabs/risingwave/releases/download/v3.0.4/risingwave-v3.0.4-x86_64-unknown-linux-all-in-one.tar.gz)
- [Source code (zip)](https://github.com/risingwavelabs/risingwave/archive/refs/tags/v3.0.4.zip)
- [Source code (tar.gz)](https://github.com/risingwavelabs/risingwave/archive/refs/tags/v3.0.4.tar.gz)
- [risectl - a CLI tool for managing and accessing RisingWave clusters](https://github.com/risingwavelabs/risingwave/releases/download/v3.0.4/risectl-v3.0.4-x86_64-unknown-linux.tar.gz)

See the **Full Changelog** [here](https://github.com/risingwavelabs/risingwave/compare/v3.0.3...v3.0.4).

</Update>

<Update label="v3.0.3" description="2026-08-17">

## SQL features

- Fixes Iceberg Engine table creation with `enable_compaction = false` when the license does not include `SinkAutoSchemaChange`. For Iceberg Engine tables, `ALTER TABLE ADD COLUMN` still requires this feature. [#26545](https://github.com/risingwavelabs/risingwave/pull/26545)
- Improves over-window query memory management by evicting cold partition-cache entries at chunk boundaries instead of waiting for the next barrier. [#26424](https://github.com/risingwavelabs/risingwave/pull/26424)

## Connectors

- Adds Microsoft Entra service-principal authentication for Iceberg sinks and Iceberg Engine REST catalogs on Azure Data Lake Storage Gen2. Configure `adlsgen2.tenant_id`, `adlsgen2.client_id`, and `adlsgen2.client_secret`; optionally set `adlsgen2.authority_host` for sovereign clouds. [#26124](https://github.com/risingwavelabs/risingwave/pull/26124)
- Fixes exactly-once coordinated sinks getting stuck after recovery when an empty pre-commit epoch had no durable coordinator record. [#26498](https://github.com/risingwavelabs/risingwave/pull/26498)
- Prevents a no-op `ALTER SINK ... CONNECTOR WITH` update from stopping the sink consumer and forcing database recovery. [#26484](https://github.com/risingwavelabs/risingwave/pull/26484)
- Improves cluster recovery stability when many sinks reconnect simultaneously by throttling coordinator initialization and adding retry backoff. [#26639](https://github.com/risingwavelabs/risingwave/pull/26639)
- Fixes writer panics when an Iceberg table contains malformed snapshot summary totals. [#26628](https://github.com/risingwavelabs/risingwave/pull/26628)
- Reuses Iceberg S3 and GCS storage operators per bucket, reducing repeated credential and token loading, including repeated STS calls for IRSA-backed S3 deployments. [#26634](https://github.com/risingwavelabs/risingwave/pull/26634)

## Administration

- Reconciles serving vnode mappings during startup, recovery, and dirty-job cleanup so stale fragment mappings are removed. [#26352](https://github.com/risingwavelabs/risingwave/pull/26352)

## Assets

- Run this version from Docker:
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v3.0.3 single_node`
- [Prebuilt all-in-one library for Linux](https://github.com/risingwavelabs/risingwave/releases/download/v3.0.3/risingwave-v3.0.3-x86_64-unknown-linux-all-in-one.tar.gz)
- [Source code (zip)](https://github.com/risingwavelabs/risingwave/archive/refs/tags/v3.0.3.zip)
- [Source code (tar.gz)](https://github.com/risingwavelabs/risingwave/archive/refs/tags/v3.0.3.tar.gz)
- [risectl - a CLI tool for managing and accessing RisingWave clusters](https://github.com/risingwavelabs/risingwave/releases/download/v3.0.3/risectl-v3.0.3-x86_64-unknown-linux.tar.gz)

See the **Full Changelog** [here](https://github.com/risingwavelabs/risingwave/compare/v3.0.2...v3.0.3).
...
- Streams meta snapshot backup and restore payloads to reduce memory usage. When restoring an older snapshot, decodes only the required Hummock sequence data from the latest snapshot instead of its full metadata. [#26267](https://github.com/risingwavelabs/risingwave/pull/26267), [#26193](https://github.com/risingwavelabs/risingwave/pull/26193)
...
- Run this version from Docker:
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v3.0.2 single_node`
...
- Run this version from Docker:
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v3.0.1 single_node`
...
- After a meta-node restart, exactly-once sink writers no longer replay epochs whose pre-commit metadata was already persisted, preventing duplicate key errors during recovery. [#25690](https://github.com/risingwavelabs/risingwave/pull/25690)
- **PostgreSQL CDC**:
...
- Adds `disable_sync_log_store_dispatcher` to the `[streaming.developer]` configuration section. Set it to `true` to disable the optimized dispatcher path for sync log store. The default is `false`. [#25663](https://github.com/risingwavelabs/risingwave/pull/25663)
...
- Run this version from Docker:
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v3.0.0 single_node`
...
- Run this version from Docker:
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v2.8.0 single_node`
...
- Run this version from Docker:
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v2.7.0 single_node`
...
- Run this version from Docker:<br/>
`docker run -it --pull=always -p 4566:4566 -p 5691:5691 ghcr.io/risingwavelabs/risingwave:v2.1.0 single_node`
...
*   Run this version from Docker:
    `docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v1.9.1 single_node`
*   [Prebuilt all-in-one library for Linux](https://github.com/risingwavelabs/risingwave/releases/download/v1.9.1/risingwave-v1.9.1-x86_64-unknown-linux-all-in-one.tar.gz)
...
*   Run this version from Docker:
    `docker run -it --pull=always -p 4566:4566 -p 5691:5691 risingwavelabs/risingwave:v1.8.0 single_node`
