...
- Fixes a bug where `ALTER` rate-limit changes for existing streaming jobs could revert after a later fragment reschedule, such as scaling or parallelism changes. This affected source, backfill, and sink throttling configured with `ALTER ... SET {SOURCE|BACKFILL|SINK}_RATE_LIMIT`. [#25562](https://github.com/risingwavelabs/risingwave/pull/25562)
