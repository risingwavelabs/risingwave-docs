...
- Query syntax:
    - Supports cross-database queries in both streaming and ad-hoc modes. https://github.com/risingwavelabs/risingwave/pull/20261
    - Support grouping by `window_start` and `window_end` at the same time in emit-on-window-close queries. https://github.com/risingwavelabs/risingwave/pull/19998
    - Support `ASOF JOIN` for batch queries to match the nearest record with possible variations in case of ties. https://github.com/risingwavelabs/risingwave/pull/19790

- SQL commands:
    - **Public preview:** Supports specifying `resource_group` for `CREATE DATABASE` command. https://github.com/risingwavelabs/risingwave/pull/20751
    - Allows enabling or disabling the creation of embedded Python, JavaScript, and WebAssembly UDFs. https://github.com/risingwavelabs/risingwave/pull/20893
    - Supports async and batched functions for embedded JavaScript UDFs. https://github.com/risingwavelabs/risingwave/pull/20403
    - Rejects schema-qualified CURSOR names in DECLARE statements to align with Postgres specifications. https://github.com/risingwavelabs/risingwave/pull/20221
    - `CREATE TABLE` command supports partially specifying the schema instead of using `*` to ingest all columns. https://github.com/risingwavelabs/risingwave/pull/20203
    - `DROP SCHEMA`  command supports the `CASCADE` option to drop the specified schema and all its dependencies.  https://github.com/risingwavelabs/risingwave/pull/19702
    - Supports switching databases during an active session using either `SET DATABASE TO` or `USE`. https://github.com/risingwavelabs/risingwave/pull/19786
    - Supports `SET PARALLELISM` clause for `ALTER SOURCE` command. https://github.com/risingwavelabs/risingwave/pull/20606
    - Support `IF NOT EXISTS` option for `CREATE FUNCTION`/`CREATE AGGREGATE`. https://github.com/risingwavelabs/risingwave/pull/20079
    - Supports `GRANT` or `REVOKE` privileges on view. https://github.com/risingwavelabs/risingwave/pull/20670

- SQL functions & operators:
    - Support IGNORE NULLS for first_value and last_value window functions. https://github.com/risingwavelabs/risingwave/pull/19874

- System catalog:
    - **Public preview:** Adds system catalog `rw_resource_groups`. https://github.com/risingwavelabs/risingwave/pull/20920
    - **Public preview:** Adds system catalog `rw_streaming_jobs`. https://github.com/risingwavelabs/risingwave/pull/20207

### Connectors

- **Deprecation notice:** The legacy S3 source will be fully deprecated in v2.4.0. While the properties and enumerator will be retained to ensure that the metadata can start normally, it may disrupt other streaming jobs that still rely on the legacy source. https://github.com/risingwavelabs/risingwave/pull/20658
- **Public preview:** Support ingesting data from **PostgreSQL 17** using CDC.  https://github.com/risingwavelabs/risingwave/pull/20668
- Supports creating connections for Iceberg. https://github.com/risingwavelabs/risingwave/pull/20189
- Supports AWS S3 Tables integration for the Iceberg source, sink, and table engine. https://github.com/risingwavelabs/risingwave/pull/21018
- Supports creating Iceberg sources and sinks with GCS. https://github.com/risingwavelabs/risingwave/pull/19999
- Supports generated columns for tables on CDC sources. https://github.com/risingwavelabs/risingwave/pull/20380
- Supports reading Parquet files from GCS and Azure blob. https://github.com/risingwavelabs/risingwave/pull/19974, https://github.com/risingwavelabs/risingwave/pull/20046
- Removes schema from DML statements for JDBC sinks if not specified one.  https://github.com/risingwavelabs/risingwave/pull/20632
- Adds `partition_by` to enable partitioning using column names or transformations for Iceberg sinks.  https://github.com/risingwavelabs/risingwave/pull/20495
- Adds `jdbc.auto.commit` for JDBC sinks to automatically commit transactions. https://github.com/risingwavelabs/risingwave/pull/20597
- Adds `snowflake`  catalog for Iceberg source. https://github.com/risingwavelabs/risingwave/pull/20421
- Adds `aws.msk.signer_timeout_sec` to set timeout limit for loading AWS credentials of AWS MSK. https://github.com/risingwavelabs/risingwave/pull/20610
- `jdbc.query.timeout` for JDBC sinks is reduced from 600s to 60s. https://github.com/risingwavelabs/risingwave/pull/20641

### Installation and deployment

- For the production cluster, a license key with `Paid` tier and CPU limit of 4 is set by default to allow the free trial of paid features on a small scale. https://github.com/risingwavelabs/risingwave/pull/20296
- Add a function `rw_license()` to retrieve the license information. https://github.com/risingwavelabs/risingwave/pull/20629

### Cluster configuration changes

- Adds runtime parameter `sink_rate_limit` . https://github.com/risingwavelabs/risingwave/pull/19660
