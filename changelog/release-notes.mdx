...
- Compaction output SST filters now default to the `blocked` layout (block-based filters) for L1–L6, while L0 keeps the `auto` layout. The filter kind defaults to `xor8` for L5 and L6; L0–L4 keep `xor16`. [#25853](https://github.com/risingwavelabs/risingwave/pull/25853)
