...
- **Breaking fix:** JSONB string values now hash consistently with plain `&str`. If your cluster uses JSONB columns as distribution keys, reshuffle affected data after upgrading using your standard redistribution workflow, because vnode mapping for JSONB string values is corrected to match `&str` hashing. [#25369](https://github.com/risingwavelabs/risingwave/pull/25369)
