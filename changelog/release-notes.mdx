...
- `enable_compaction_group_normalize` now defaults to `true` (previously `false`). Overlap normalization for compaction groups is now triggered only before merge scheduling, not before regular split scheduling. This reduces latency on the split path. The description for `periodic_scheduling_compaction_group_split_interval_sec` is updated to reflect that it does not disable merge-triggered normalize splits. [#25334](https://github.com/risingwavelabs/risingwave/pull/25334)
