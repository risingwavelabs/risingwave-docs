...
- SQL commands:
    - Fixes a bug where `ALTER SOURCE ... source_rate_limit` and analogous backfill, sink, and DML rate-limit changes could silently revert after a later fragment reschedule. For example, a source resumed with `ALTER SOURCE ... SET source_rate_limit TO DEFAULT` could become paused again after scaling or parallelism changes. [#25562](https://github.com/risingwavelabs/risingwave/pull/25562)
- System catalog:
