...
- Prevents a possible meta-node crash when a subscription cursor requests changes starting after the table's latest committed epoch. The cursor now waits for the committed epoch to reach the requested start epoch before reading the change log. [#26478](https://github.com/risingwavelabs/risingwave/pull/26478)
