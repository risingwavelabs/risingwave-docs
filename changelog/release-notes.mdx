...
<Update label="v2.9.0" description="Upcoming">

## SQL features

- System catalog:
    - Adds column `schema_name` to `rw_catalog.rw_materialized_views` and `rw_catalog.rw_subscriptions` so you can filter by schema without extra joins. [#25248](https://github.com/risingwavelabs/risingwave/pull/25248)
...
- Iceberg sink and Iceberg engine tables: Adds `commit_checkpoint_size_threshold_mb` to trigger size-based early commits. When uncommitted write data exceeds this threshold (in MB), RisingWave commits at the next checkpoint barrier regardless of `commit_checkpoint_interval`. Default: `128` MB. [#25058](https://github.com/risingwavelabs/risingwave/pull/25058)
