...
- Updates SST filter layout behavior for Hummock compaction output. `sstable_filter_layout` now accepts `blocked` in addition to `auto` and `plain`, and its default changes to `["auto", "blocked", "blocked", "blocked", "blocked", "blocked", "blocked"]`, so non-L0 levels use block-based filters by default. When `sstable_filter_layout[level]` is `auto`, `blocked_xor_filter_kv_count_threshold` is now evaluated per output SST instead of per compaction task. [#25782](https://github.com/risingwavelabs/risingwave/pull/25782)
