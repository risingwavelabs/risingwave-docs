...
- **Breaking change:** Automatic compaction-group scheduling now measures successful Hummock ingress over actual commit intervals. Splits use the latest observed rate, while merges require a full observed history whose peak stays at or below both throughput thresholds. The new `[meta]` option `table_write_throughput_retention_seconds` controls this shared history and defaults to `300` seconds. The following options are deprecated and ignored: `table_stat_throuput_window_seconds_for_split`, `table_stat_throuput_window_seconds_for_merge`, `table_stat_high_write_throughput_ratio_for_split`, and `table_stat_low_write_throughput_ratio_for_merge`. Remove them when upgrading; their window and ratio values have no one-to-one mapping to the new policy. Throughput thresholds and scheduling intervals are unchanged. [#27044](https://github.com/risingwavelabs/risingwave/pull/27044)
