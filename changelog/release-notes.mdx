...
- Compaction-group SST filter settings now use `sstable_filter_type` (`none`, `xor16`, `xor8`) and `sstable_filter_layout` (`auto`, `plain`, `blocked`). Setting `sstable_filter_type` to `none` disables SST filters for that target level, and `plain` is now the canonical non-blocked layout value. `rw_catalog.rw_hummock_sstables` also exposes `filter_type` and `filter_layout`. [#25832](https://github.com/risingwavelabs/risingwave/pull/25832)
