...
- **New UI:** v0.6.3 redesigned the Console and login page.
- **License requirement:** The v0.7.0 tag introduced startup license verification; v0.7.1 is the first GitHub release listed here that requires a signed license with the `premium` feature. A missing key or a `free` tier key without `premium` does not start the release image. v0.7.5 added a recovery flow that lets Console start with an expired, otherwise valid key in a limited mode. For an older, no-key evaluation image, see [v0.5.1 in the license guide](/web-ui/risingwave-console/license-management#try-without-a-license-key). This is not a free tier of the current release.
- **SSO:** v0.7.1 introduced the login flow and Dex foundation; v0.7.3 added browser login and connector administration; v0.7.4 added deployment overlays and follow-up fixes.
