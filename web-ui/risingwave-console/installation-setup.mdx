...
Production builds require the signed RisingWave license key at startup. This is the same license key used by the RisingWave database. Provide it with either `RW_LICENSE_KEY` or `RW_LICENSE_KEY_PATH`. If you want to try Console without configuring a license key, follow the [v0.5.1 trial steps](/web-ui/risingwave-console/license-management#try-without-a-license-key). It is an older evaluation version and does not include all current capabilities.
...
docker run -d -p 8020:8020 --name risingwave-console \
  ...
  risingwavelabs/risingwave-console:v0.7.9-pgbundle
```
...
services:
  risingwave-console:
    image: risingwavelabs/risingwave-console:v0.7.9
    ports:
      - "8020:8020"
    environment:
