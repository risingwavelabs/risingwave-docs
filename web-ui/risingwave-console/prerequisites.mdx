...
If you only want to try Console without configuring a license key, follow the [v0.5.1 trial steps](/web-ui/risingwave-console/license-management#try-without-a-license-key). This is an older evaluation version and does not include all current Console capabilities, so some features described in this documentation may not exist in that version.
