...
If you only want to try RisingWave Console without configuring a license key, use `v0.5.1`. On an x86-64 (amd64) computer, run:

```shell
docker run --rm -p 127.0.0.1:8020:8020 --name risingwave-console \
  -e RCONSOLE_ROOT_PASSWORD=choose_a_password \
  risingwavelabs/risingwave-console:v0.5.1-pgbundle
```

On an ARM64 computer, including Apple Silicon, use the ARM64 image instead:

```shell
docker run --rm -p 127.0.0.1:8020:8020 --name risingwave-console \
  -e RCONSOLE_ROOT_PASSWORD=choose_a_password \
  risingwavelabs/risingwave-console:0.5.1-pgbundle-arm64
```

Replace `choose_a_password` with your own password. Keep the container running, open `http://localhost:8020`, and sign in as `root` with that password. To explore data, you need a running RisingWave cluster; use **Clusters** → **Import** to connect it. See [Connect to your RisingWave clusters](/web-ui/risingwave-console/connect-risingwave-clusters) for the required endpoints.
...
Production Console builds verify the signed RisingWave license key during startup. Console exits if the key is missing, invalid, or does not include the `premium` feature. Since v0.7.5, an expired but otherwise valid key lets Console start in a limited mode: read-only flows and license replacement remain available, while premium actions are blocked. Earlier licensed releases exit when the key has expired.
