...
Open `http://localhost:8020` and sign in as `root` with the password you set (default `root`).
...
data:
  config.yaml: |
    ...
    # which the StatefulSet already enforces (replicas: 1). Moving Dex
    # to the shared external Postgres is tracked as CLOUD-4823.
    storage:
      type: sqlite3
      config:
        file: /dex-data/dex.db
    ...
    # password auth stays in the console/Anclax); it's enabled only to
