...
description: "View and use the cloud metadata values exposed in the RisingWave Cloud Console, including the IAM role ARN, PrivateLink Principal, and egress public IPs for your cluster."
---

RisingWave Cloud exposes a set of environment-specific metadata values for each cluster. These values are required when setting up cross-account access (IAM role assume) and PrivateLink connections. All values are read-only and are generated automatically when a cluster is created.
