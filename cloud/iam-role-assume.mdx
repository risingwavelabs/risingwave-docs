...
Add your IAM role ARN to your cluster's **Allowed Principals** list in the RisingWave Cloud Console so the cluster authorizes RisingWave Cloud to assume it. See [Managing allowed principals](#managing-allowed-principals-in-risingwave-cloud) below for the exact steps.
...
Click **Add principal** to add an IAM Role ARN that is allowed to access this cluster's resources via IAM Assume Role — credential-free access from your cloud environment.
