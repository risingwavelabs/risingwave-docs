...
The **Resources** page also includes a **Backfillings** tab where you can configure dedicated serverless nodes for materialized view backfill jobs. Serverless backfiller nodes scale to 0 when idle, so you only pay for compute while a backfill is running.
