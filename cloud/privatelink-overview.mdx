...
In addition to connecting RisingWave Cloud to services in your VPC, RisingWave Cloud also supports **Serving PrivateLink** — a reverse path that lets you connect *to* RisingWave Cloud privately from your own AWS VPC.

With Serving PrivateLink, RisingWave Cloud creates an AWS endpoint service. You then create an [Interface VPC Endpoint](https://docs.aws.amazon.com/vpc/latest/privatelink/create-interface-endpoint.html) in your AWS account to connect to it. All traffic between your VPC and RisingWave Cloud stays on the AWS internal network.
