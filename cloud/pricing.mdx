...
- **Disk cache**: Every project includes a default disk cache — a local disk layer that sits between memory and object storage to reduce latency and object storage access. Pro plan projects can customize the cache size and performance tier. See [Manage disk cache](/cloud/manage-disk-cache) for the sizing defaults and tier options.
