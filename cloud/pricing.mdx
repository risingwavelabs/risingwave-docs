...
RisingWave Cloud charges the cost of each cluster individually based on the plan selected.

| Plan          | Hosting                       | Pricing model                          |
|:--------------|:------------------------------|:----------------------------------------|
| [Basic](#basic-plan)       | Fully hosted (AWS, GCP, Azure) | Pay-as-you-go (after 7-day free trial) |
| [Pro](#pro-plan)           | Fully hosted, BYOC, or BYOK8s   | Pay-as-you-go or Annual contract       |


### Basic plan

The Basic plan is designed to help you launch streaming workloads fast with zero setup. It is fully hosted on the Cloud provider of your choice (AWS, GCP, or Azure).
...
The Pro plan offers elastic scale, flexibility, and advanced capabilities. This plan is suitable for clusters requiring high scalability or specific deployment requirements.

- **Deployment**: Available as Fully hosted, Bring Your Own Cloud (BYOC), or Bring Your Own Kubernetes (BYOK).
- **Pricing**: Options for Pay-as-you-go or an Annual contract.
- **Features**: Includes premium features and SLA. Premium support is available.
...
Your total charge is based on the RWU-hours consumed by all components in your cluster. For instance, using 1 RWU for 1 hour equals 1 RWU-hour.
...
- **Compute resources**: Compute resources are measured in RisingWave Unit (RWU) hours used across all clusters in the organization. See the [explanation of RWU](#risingwave-unit-rwu) above. In each RisingWave cluster, the usages are tracked for all four components:
   - Serving Node
   - Streaming Node
   - Meta Node
   - Compactor Node
...
- **Disk cache**: Every cluster includes a default disk cache — a local disk layer that sits between memory and object storage to reduce latency and object storage access. Pro clusters can customize the cache size and performance tier. See [Manage disk cache](/cloud/manage-disk-cache) for the sizing defaults and tier options.
...
In total, the cluster utilizes 38 RWUs and stores 20GB of data. The cluster operated for 700 hours in the past month. The cluster also transferred 1TB (1024GB) of public ingress data and 20GB of public egress data.
