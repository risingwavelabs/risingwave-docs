...
RWU is calculated as follows: 

RWU = MAX(vCPU, (Memory in GB)/4)

The compute unit (RWU) is determined by taking the larger value between the number of virtual CPUs (vCPU) and one-fourth of the memory size in GB. This ensures that the compute unit reflects the greater of the two resource requirements.
