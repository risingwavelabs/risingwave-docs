...
The **Backfiller SKU** defines the compute size of each backfiller node. Choose a larger SKU for faster backfill throughput on large upstream tables, or a smaller SKU to minimize cost.

### Replicas

**Replicas** sets the backfill capacity available to RisingWave Cloud when scheduling serverless backfill jobs:
...
1. When a new serverless backfill job is submitted, RisingWave Cloud creates a dynamic resource group for that job.
