...
This guide covers WebSocket ingestion for **managed RisingWave Cloud** clusters, where connections are routed through the managed RWProxy ingest endpoint.
...
In managed RisingWave Cloud, WebSocket ingest connections are accepted by RWProxy over WSS and forwarded to your cluster's frontend ingest listener.
...
1. A running RisingWave Cloud cluster.
2. The tenant identifier of your cluster (for example, `rwc-g1huxxxxxx-mycluster`).
...
1. Go to the [RisingWave Cloud dashboard](https://cloud.risingwave.com/dashboard), open your cluster, and select **Data Catalog** in the cluster navigation.
