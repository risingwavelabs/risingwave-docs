...
- Each DML batch is sent as one WebSocket text frame. On RisingWave Cloud, RWProxy caps each frame at 10 MiB by default (its `ingestMaxMessageSize` setting, which RisingWave Cloud operators can override); an oversized frame closes the connection with WebSocket code `1009` before any ack is sent. A self-hosted frontend has a larger 16 MiB cap, which is the default of the `tungstenite` WebSocket library that RisingWave uses (RisingWave does not override it).
...
- Keep each batch below 8 MiB to stay safely under both the 10 MiB Cloud limit and the 16 MiB self-hosted frontend limit.
