...
description: "This article describes how to use metrics API to export metrics from a RisingWave Cloud cluster to various monitoring systems like Prometheus, DataDog, and InfluxDB. The metrics include all major components such as `etcd`, `compactor`, and `meta`."
---
...
* Update the `NS_ID` with the specific UUID of your RisingWave cluster. To find the UUID, hover over the tag next to your cluster name.
...
To set up dashboards with **Grafana** and **Prometheus**, import one of the pre-generated RisingWave dashboards into Grafana:

1. [Dev dashboard (`risingwave-dev-dashboard.json`)](https://github.com/risingwavelabs/risingwave/blob/main/grafana/risingwave-dev-dashboard.json) - recommended for most monitoring needs.
2. [User dashboard (`risingwave-user-dashboard.json`)](https://github.com/risingwavelabs/risingwave/blob/main/grafana/risingwave-user-dashboard.json)
...
The Python scripts under [`grafana/`](https://github.com/risingwavelabs/risingwave/tree/main/grafana) are the source of truth used by RisingWave developers with `grafanalib` to generate these JSON dashboards. Most users can import the JSON above and ignore the Python.

If you need to customize the dashboards, such as enabling multiple namespace support or regenerating them programmatically, follow the [Grafana dashboard build instructions](https://github.com/risingwavelabs/risingwave/blob/main/grafana/README.md).
