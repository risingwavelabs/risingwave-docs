...
- **Region**: The AWS region where your EKS cluster and supporting resources run. It can differ from the RisingWave Cloud control-plane region when you configure the cross-region PrivateLink requirements in [Network connectivity](#7-network-connectivity).
...
The required top-level `region` field in the BYOK8s configuration identifies the region of your Kubernetes cluster and supporting cloud resources. The region cannot be changed after you register the environment; create a new environment to move to another region.
</Tab>
<Tab title="GCP">
...
<Warning>
Cross-region BYOK8s environments are currently in preview.
</Warning>

If the EKS cluster is in a different region from the RisingWave Cloud control plane:

- Configure both VPC Endpoint Services to support the control-plane region. The [reference `base_env` module](https://github.com/risingwavelabs/risingwave-byok-terraform-example/tree/main/aws/base_env) does this when you set `control_plane_region` to the region selected by your `rwc` context.
- Allow the AWS principal that applies the endpoint-service configuration to use the [`vpce:AllowMultiRegion`](https://docs.aws.amazon.com/vpc/latest/privatelink/security_iam_service-with-iam.html#security_iam_service-with-iam-id-based-policies-actions) permission-only action. Ensure that your AWS Organizations service control policies do not deny this action.
...
<Note>
BYOK8s environment management is available in the RisingWave Cloud portal under **BYOK**. Cluster creation still uses the two-phase `rwc` flow below because the cluster IAM role depends on the namespace and service account allocated during initial creation.
</Note>
