...
The batching strategy ensures that the contents within a chunk are not further split, which enables the decoupling of the file sink. The batching strategy is available for Parquet, JSON, and CSV encode. 
