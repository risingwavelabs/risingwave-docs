...
If the stream key matches the sink primary key exactly, or if the sink is configured as `append-only` or with `ignore_delete = 'true'`, no buffering is performed and the sink emits changes immediately as they arrive.
...
<Note>
File sink currently supports only append-only mode, so please set `type = 'append-only'`. If the upstream is not append-only, also set `ignore_delete = 'true'` in the sink options.
</Note>
