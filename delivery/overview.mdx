...
You can use `partition_by` to partition files by time units (month, day, or hour). When enabled, files are organized into corresponding subdirectories based on their creation time.


Regarding file naming rules, currently, files follow the naming pattern `/Option<partition_by>/executor_id + timestamp.suffix`. `Timestamp` differentiates files batched by the rollover interval. 
