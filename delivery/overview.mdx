...
| [HTTP](/integrations/destinations/http) | `connector = 'http'` |
...
- **Delta Lake**: Supports exactly-once coordination when `is_exactly_once = true`. This deduplicates replayed checkpoint commits to prevent duplicate file appends during recovery.
