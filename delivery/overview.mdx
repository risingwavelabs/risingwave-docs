...
You can use the `ignore_delete` option to prevent DELETE operations from being sent to the downstream sink. When enabled, the sink will process INSERT and UPDATE operations normally but ignore DELETE operations. This is useful to prevent accidental data deletion in downstream systems.

The `ignore_delete` option is supported for:
- `append-only` sinks (specified with `type = 'append-only'` or `FORMAT PLAIN`)
- `upsert` sinks (specified with `type = 'upsert'` or `FORMAT UPSERT`)

The `ignore_delete` option is **NOT** supported for:
- `debezium` (retract) sinks

To enable this option, add `ignore_delete = 'true'` in the `WITH` clause when creating a sink:
...
<Note>
The `force_append_only` option is a legacy alias for `ignore_delete` and is maintained for backward compatibility. You should only specify one of these options, not both.
</Note>
...
If the stream key matches the sink primary key exactly, or if the sink is configured as `append-only` or with `ignore_delete` enabled, no buffering is performed and the sink emits changes immediately as they arrive.
...
WITH (
    ...
    type = 'append-only',
    ignore_delete='true'
) FORMAT PLAIN ENCODE PARQUET(ignore_delete='true');
```
