...
For decoupled sinks, RisingWave persists incoming updates to an internal synced log store before handing them to the downstream connector. This allows RisingWave to keep polling upstream executors and advancing sink progress even when the downstream system is slow or blocked by backpressure.
