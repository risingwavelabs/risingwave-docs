...
If you set `ignore_delete = 'true'`, the sink drops `DELETE` messages and converts `UPDATE` messages to `INSERT` messages. This option is supported for both `append-only` and `upsert` sink types. `force_append_only` remains available as a backward-compatible alias.
