...
You can set the `ignore_delete` option to `true` to drop DELETE messages and convert UPDATE messages to INSERT messages in both `append-only` and `upsert` sink types. This is useful when the downstream system cannot handle deletes or when you want to preserve all historical states.
