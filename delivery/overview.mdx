...
You can set the `ignore_delete` option to `true` to drop DELETE messages and convert UPDATE messages to INSERT messages in both `append-only` and `upsert` sink types. This is useful when the downstream system cannot handle deletes or when you want to maintain an append-only history of changes.

The `force_append_only` option is kept as a backward-compatible alias for `ignore_delete`.

Example:

```sql
CREATE SINK sink_name FROM source_name WITH (
  connector = 'kafka',
...
If the stream key matches the sink primary key exactly, or if the sink is configured as `append-only` or has `ignore_delete` enabled, no buffering is performed and the sink emits changes immediately as they arrive.
