...
- For coordinated exactly-once sinks, RisingWave now durably records every acknowledged pre-commit epoch, including epochs with no downstream commit metadata, so recovery resumes from a consistent sink progress point.
